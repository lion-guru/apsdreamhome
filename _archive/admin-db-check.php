<?php
/**
 * Admin Panel — DB Verification Script
 * Access: http://localhost/apsdreamhome/public/admin-db-check.php
 * LOCALHOST ONLY — DELETE AFTER USE
 */
...
echo "<span style='color:#38bdf8;font-size:16px;'>=== APS DREAM HOME — DB VERIFICATION REPORT ===</span>\n\n";
...
if ($hasBookings && !$hasPlotBookings) {
    echo "\n<span style='color:#f87171'>❌ CRITICAL: Only 'bookings' exists but code uses 'plot_bookings'!</span>\n";
} elseif (!$hasBookings && $hasPlotBookings) {
    echo "\n<span style='color:#fb923c'>⚠️  'bookings' table missing — BookingController will fail!</span>\n";
    echo "   FIX: CREATE VIEW bookings AS SELECT * FROM plot_bookings;\n";
} elseif ($hasBookings && $hasPlotBookings) {
    echo "\n<span style='color:#4ade80'>✅ Both exist — check if data is consistent</span>\n";
