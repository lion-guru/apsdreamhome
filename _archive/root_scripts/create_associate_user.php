...
try {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        // Check if associate user already exists
        $stmt = $pdo->prepare("SELECT id FROM users WHERE email = ? AND user_type = 'associate' LIMIT 1");
        $stmt->execute(['associate@apsdreamhome.com']);
        
        if ($stmt->rowCount() > 0) {
            echo "Associate user already exists in users table.\n";
        } else {
