...
$idx = $r.Content.IndexOf('DEBUG START')
if ($idx -ge 0) { Write-Host "DEBUG START found at $idx" }
$idx = $r.Content.IndexOf('Top Performers')
if ($idx -ge 0) { Write-Host "Top Performers found at $idx" }
$idx = $r.Content.IndexOf('CFO Dashboard')
if ($idx -ge 0) { Write-Host "CFO Dashboard found at $idx" }
