...
foreach ($rows as $r) echo "  {$r['id']}: plot={$r['plot_id']} cust={$r['customer_id']} assoc={$r['associate_id']} [{$r['status']}] ₹{$r['total_plot_value']}" . PHP_EOL;
...
foreach ($rows as $r) echo "  {$r['id']}: {$r['plot_number']} ({$r['block']}) ₹{$r['price_per_sqft']}/sqft = ₹{$r['total_price']} [{$r['status']}]" . PHP_EOL;
