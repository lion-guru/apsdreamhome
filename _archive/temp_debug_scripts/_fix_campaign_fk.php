...
try {
    ...
    echo "âœ“ Dropped old FK: fk_marketing_campaign_recipients_campaign_id\n";
} catch (PDOException $e) {
    echo "âš  Could not drop old FK (may not exist): " . $e->getMessage() . "\n";
}
...
if ($orphans > 0) {
    $pdo->exec("
        ...
        WHERE NOT EXISTS (SELECT 1 FROM marketing_campaigns mc WHERE mc.id = mcr.campaign_id)
    ");
    echo "âœ“ Deleted $orphans orphaned rows\n";
}
...
try {
    ...
    ");
    echo "âœ“ Added new FK â†’ marketing_campaigns.id\n";
} catch (PDOException $e) {
    echo "âœ— Could not add new FK: " . $e->getMessage() . "\n";
}
...
echo "\nVerification â€” FKs on marketing_campaign_recipients:\n";
foreach ($verify as $row) {
    echo "  {$row['CONSTRAINT_NAME']} â†’ {$row['REFERENCED_TABLE_NAME']}.{$row['REFERENCED_COLUMN_NAME']}\n";
}
