<?php
/**
 * Phase 14: Wrap+drop 3-ref tables
 */
$config = require __DIR__ . '/../config/database.php';
$pdo = new PDO("mysql:host={$config['host']};port={$config['port']};dbname={$config['database']};charset=utf8mb4", $config['username'], $config['password'], [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]);

$skip = ['points_rules', 'tier_benefits', 'wallet_configuration', 'notification_templates',
         'error_logs', 'comparison_criteria', 'lead_scoring_history', 'lead_scoring_rules'];
...
foreach ($tables as $t) {
    ...
    $targets[] = ['name' => $t, 'rows' => $rows, 'paths' => $refPaths];
}

usort($targets, fn($a,$b) => $b['rows'] <=> $a['rows']);

echo "=== PHASE 14: Wrap+Drop 3-Ref Tables ===\n";
