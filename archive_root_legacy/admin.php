...
class PerfectSecurity {
    private static $csrfTokenName = 'perfect_admin_csrf_token';
    ...
    public static function validateCSRFToken($token) {
        return isset($_SESSION[self::$csrfTokenName]) && hash_equals($_SESSION[self::$csrfTokenName], $token);
    }
...
class PerfectAuth {
    ...
    public function __construct() {
        $this->db = PerfectAdminDatabase::getInstance()->getConnection();
    ...
    public function authenticate($username, $password) {
        try {
            $stmt = $this->db->prepare("
                ...
                WHERE u.username = :username AND u.status = 'active' AND u.is_deleted = 0
                LIMIT 1
            ");
            ...
            if ($user && $this->security->verifyPassword($password, $user['password'])) {
                // Update last login
                $updateStmt = $this->db->prepare("UPDATE users SET last_login = NOW(), login_attempts = 0 WHERE id = :id");
    ...
    public function isLoggedIn() {
        return isset($_SESSION['admin_user']) && isset($_SESSION['admin_logged_in']) && $_SESSION['admin_logged_in'] === true;
    }
    
    public function hasPermission($permission) {
        if (!$this->isLoggedIn()) return false;
        
        $permissions = json_decode($_SESSION['admin_user']['permissions'] ?? '[]', true);
...
class PerfectAnalytics {
    ...
    public function getDashboardStats() {
        try {
            ...
            // Property stats
            $stmt = $this->db->query("SELECT COUNT(*) as total FROM properties WHERE is_deleted = 0");
            $stats['total_properties'] = $stmt->fetch()['total'];
            
            $stmt = $this->db->query("SELECT COUNT(*) as total FROM properties WHERE status = 'available' AND is_deleted = 0");
            $stats['available_properties'] = $stmt->fetch()['total'];
            
            $stmt = $this->db->query("SELECT COUNT(*) as total FROM properties WHERE status = 'sold' AND is_deleted = 0");
            $stats['sold_properties'] = $stmt->fetch()['total'];
            
            // User stats
            $stmt = $this->db->query("SELECT COUNT(*) as total FROM users WHERE is_deleted = 0");
            $stats['total_users'] = $stmt->fetch()['total'];
            
            $stmt = $this->db->query("SELECT COUNT(*) as total FROM users WHERE role_id = 2 AND is_deleted = 0");
            $stats['total_agents'] = $stmt->fetch()['total'];
            
            $stmt = $this->db->query("SELECT COUNT(*) as total FROM users WHERE role_id = 3 AND is_deleted = 0");
            $stats['total_customers'] = $stmt->fetch()['total'];
            
            // Booking stats
            $stmt = $this->db->query("SELECT COUNT(*) as total FROM bookings WHERE status = 'pending'");
            $stats['pending_bookings'] = $stmt->fetch()['total'];
            
            $stmt = $this->query("SELECT COUNT(*) as total FROM bookings WHERE status = 'confirmed'");
            $stats['confirmed_bookings'] = $stmt->fetch()['total'];
            
            // Revenue stats
            $stmt = $this->db->query("SELECT SUM(amount) as total FROM bookings WHERE status = 'confirmed' AND created_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)");
            ...
            $stmt = $this->db->query("SELECT COUNT(*) as total FROM properties WHERE DATE(created_at) = CURDATE()");
            $stats['today_properties'] = $stmt->fetch()['total'];
            
            $stmt = $this->db->query("SELECT COUNT(*) as total FROM bookings WHERE DATE(created_at) = CURDATE()");
    ...
    public function getChartData() {
        try {
            ...
            $stmt = $this->db->query("
                ...
                ORDER BY month
            ");
            $propertyChartData = $stmt->fetchAll();
            ...
            $stmt = $this->db->query("
                ...
                ORDER BY month
            ");
            $bookingChartData = $stmt->fetchAll();
...
class PerfectAdminService {
    ...
    public function getUserList($filters = [], $page = 1, $limit = 20) {
        try {
            $whereConditions = ["u.is_deleted = 0"];
            $params = [];
            
            if (!empty($filters['search'])) {
                $whereConditions[] = "(u.username LIKE :search OR u.first_name LIKE :search OR u.last_name LIKE :search OR u.email LIKE :search)";
            ...
            $query = "
                SELECT u.*, r.role_name, 
                       ...
                       DATE_FORMAT(u.last_login, '%d %b %Y %H:%i') as last_login_formatted
                FROM users u
                LEFT JOIN roles r ON u.role_id = r.id
                WHERE $whereClause
    ...
    public function getPropertyList($filters = [], $page = 1, $limit = 20) {
        try {
            ...
            // Get properties
            $query = "
                SELECT p.*, pt.type_name, u.first_name, u.last_name,
...
if (!$auth->isLoggedIn() && $action !== 'login' && $action !== 'authenticate') {
    $action = 'login';
}
    ...
    if (!$security->validateCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($username) || empty($password)) {
            ...
            if ($user) {
                ...
                header('Location: PERFECT_ADMIN.php');
                exit;
            } else {
...
if ($auth->isLoggedIn() && (time() - ($_SESSION['last_activity'] ?? 0) > 1800)) {
    $auth->logout();
    header('Location: PERFECT_ADMIN.php?timeout=1');
    exit;
}
                ...
                <div class="header-right">
                    <div class="dropdown">
                        <div class="user-profile dropdown-toggle" data-bs-toggle="dropdown">
                            ...
                            <div>
                                <div class="fw-semibold"><?php echo htmlspecialchars($_SESSION['admin_user']['first_name'] . ' ' . $_SESSION['admin_user']['last_name']); ?></div>
                                <small class="text-muted"><?php echo htmlspecialchars($_SESSION['admin_user']['role_name']); ?></small>
                            </div>
                        </div>
