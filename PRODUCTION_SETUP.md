...
-- Grant privileges
GRANT ALL PRIVILEGES ON apsdreamhomefinal.* TO 'aps_prod_user'@'localhost';

-- Create read-only user for reports
CREATE USER 'aps_readonly'@'localhost' IDENTIFIED BY 'ReadOnlyPassword456!';
GRANT SELECT ON apsdreamhomefinal.* TO 'aps_readonly'@'localhost';
...
sudo cp -r /path/to/your/local/apsdreamhomefinal/* /var/www/apsdreamhomefinal/
...
sudo chmod -R 777 /var/www/apsdreamhomefinal/logs
```
...
# Database backup
mysqldump -u aps_prod_user -p'YourStrongProdPassword123!' apsdreamhomefinal > $BACKUP_DIR/db_backup_$DATE.sql

# File backup
tar -czf $BACKUP_DIR/files_backup_$DATE.tar.gz /var/www/apsdreamhomefinal --exclude=/var/www/apsdreamhomefinal/uploads/temp
...
mysql -u aps_readonly -p'ReadOnlyPassword456!' -e "SELECT 1" apsdreamhomefinal > /dev/null 2>&1
DB_STATUS=$?
...
mysql -u aps_prod_user -p apsdreamhomefinal -e "SHOW TABLES;"
...
mysql -u aps_readonly -p apsdreamhomefinal -e "SELECT COUNT(*) FROM properties; SELECT COUNT(*) FROM leads;"
```
