...
class ConfigManager {
    ...
    private function loadConfigurations() {
        ...
        if (!file_exists($envPath)) {
            error_log("Configuration file not found: $envPath");
            return;
        }

        // Parse .env file with robust error handling
        try {
            $env = $this->parseEnvFile($envPath);
            if (empty($env)) {
                throw new Exception('No configurations found in .env file');
            }
        } catch (Exception $e) {
            error_log('Configuration parsing error: ' . $e->getMessage());
            // Fallback to default configurations
            $env = $this->getDefaultConfigurations();
        }
