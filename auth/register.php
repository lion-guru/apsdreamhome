    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $csrf_token) {
        ...
        if (empty($errors)) {
            try {
                ...
                // Insert user data
                $insert_query = "INSERT INTO users (first_name, last_name, email, phone, password, role, status, verification_token, token_expires, created_at, updated_at)
                                VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?, NOW(), NOW())";

                $insert_stmt = $conn->prepare($insert_query);
                $insert_stmt->bind_param('ssssssss', $first_name, $last_name, $email, $phone, $hashed_password, $user_role, $verification_token, $token_expires);
