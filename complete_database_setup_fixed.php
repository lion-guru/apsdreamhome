<?php
// APS Dream Home - Complete Database Setup (Fixed)
...
// Create database
$db->query("DROP DATABASE IF EXISTS apsdreamhomefinal_complete");
$db->query("CREATE DATABASE apsdreamhomefinal_complete CHARACTER SET utf8mb4");
$db->select_db('apsdreamhomefinal_complete');
...
$tables = [
    ...
    "associates" => "CREATE TABLE associates (
        id INT AUTO_INCREMENT PRIMARY KEY,
...
foreach ($tables as $name => $sql) {
    if ($db->query($sql)) {
        echo "<p style='color: green;'>✅ $name table created</p>";
    } else {
        echo "<p style='color: red;'>❌ Error creating $name table: " . $db->error . "</p>";
    }
}
...
$constraints = [
    "ALTER TABLE plots ADD CONSTRAINT plots_project_fk FOREIGN KEY (project_id) REFERENCES projects(id)",
    "ALTER TABLE plots ADD CONSTRAINT plots_customer_fk FOREIGN KEY (customer_id) REFERENCES users(id)",
    "ALTER TABLE plots ADD CONSTRAINT plots_associate_fk FOREIGN KEY (associate_id) REFERENCES associates(id)",
    "ALTER TABLE bookings ADD CONSTRAINT bookings_property_fk FOREIGN KEY (property_id) REFERENCES properties(id)",
    "ALTER TABLE bookings ADD CONSTRAINT bookings_customer_fk FOREIGN KEY (customer_id) REFERENCES customers(id)",
    "ALTER TABLE transactions ADD CONSTRAINT transactions_customer_fk FOREIGN KEY (customer_id) REFERENCES customers(id)",
    "ALTER TABLE transactions ADD CONSTRAINT transactions_property_fk FOREIGN KEY (property_id) REFERENCES properties(id)"
];
...
foreach ($constraints as $constraint) {
    if ($db->query($constraint)) {
        ...
        echo "<p style='color: orange;'>⚠️ Constraint failed: " . $db->error . "</p>";
    }
}
...
echo "<p>✅ Database: apsdreamhomefinal_complete</p>";
