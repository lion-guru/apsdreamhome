...
try {
    ...
    // 1. Setup Associate Plan Based on Image
    ...
    // Add Associate Plan
    $db->exec("INSERT INTO mlm_plans (name, type, description, joining_fee, commission_rates, bonus_structure, rank_requirements, status) VALUES (
        'Associate Plan',
        'associate',
        'Complete Associate MLM Plan with Direct Business Salary and Junior Business Commission. 7 levels with progressive commission rates.',
        1000.00,
        '{\"level_1\": 10, \"level_2\": 12, \"level_3\": 15, \"level_4\": 18, \"level_5\": 22, \"level_6\": 25, \"level_7\": 30}',
        '{\"direct_business\": \"10%\", \"junior_business\": \"5%\", \"team_override\": \"3%\", \"leadership_bonus\": \"2%\"}',
        '{\"level_1\": \"2 associates, ₹50,000 business\", \"level_2\": \"4 associates, ₹1,00,000 business\", \"level_3\": \"6 associates, ₹2,00,000 business\", \"level_4\": \"8 associates, ₹3,50,000 business\", \"level_5\": \"10 associates, ₹5,00,000 business\", \"level_6\": \"15 associates, ₹7,50,000 business\", \"level_7\": \"20 associates, ₹10,00,000 business\"}',
        'active'
    )");
    ...
    $commissionLevels = [
        [1, 'Associate', 10.00, 2, 10.00, 50000, 200000],
        [2, 'Senior Associate', 12.00, 4, 12.00, 100000, 400000],
        [3, 'Team Leader', 15.00, 6, 15.00, 200000, 800000],
        [4, 'Senior Team Leader', 18.00, 8, 18.00, 350000, 1500000],
        [5, 'Manager', 22.00, 10, 22.00, 500000, 2500000],
        [6, 'Senior Manager', 25.00, 15, 25.00, 750000, 5000000],
        [7, 'Director', 30.00, 20, 30.00, 1000000, 10000000]
    ];
    
    foreach ($commissionLevels as $level) {
        $db->exec("INSERT INTO mlm_commission_levels (plan_id, level, name, commission_rate, min_associates, direct_percentage, min_business, max_business) VALUES (
            $planId, {$level[0]}, '{$level[1]}', {$level[2]}, {$level[3]}, {$level[4]}, {$level[5]}, {$level[6]}
        )");
    }
    
    // 2. Setup MLM Salary Plans (create new table)
    echo "💼 Setting up MLM Salary Plans...\n";
    
    // Drop existing mlm_salary_plans if exists
    $db->exec("DROP TABLE IF EXISTS mlm_salary_plans");
    
    // Create mlm_salary_plans table
    $db->exec("CREATE TABLE mlm_salary_plans (
    ...
    $salaryPlans = [
        ['Associate', 5000.00, 10.00, 'Direct Business Salary - ₹5,000 + 10% commission'],
        ['Senior Associate', 8000.00, 12.00, 'Direct Business + Team Commission - ₹8,000 + 12%'],
        ['Team Leader', 12000.00, 15.00, 'Team Leadership Salary - ₹12,000 + 15%'],
        ['Senior Team Leader', 18000.00, 18.00, 'Senior Leadership - ₹18,000 + 18%'],
        ['Manager', 25000.00, 22.00, 'Management Salary - ₹25,000 + 22%'],
        ['Senior Manager', 35000.00, 25.00, 'Senior Management - ₹35,000 + 25%'],
        ['Director', 50000.00, 30.00, 'Director Level - ₹50,000 + 30%']
    ];
    
    foreach ($salaryPlans as $salary) {
        $db->exec("INSERT INTO mlm_salary_plans (level_name, base_salary, commission_percent, description, is_active) VALUES (
            '{$salary[0]}', {$salary[1]}, {$salary[2]}, '{$salary[3]}', 1
        )");
    }
    
    // 3. Setup Commission Structure (using correct column names)
    echo "💰 Setting up Commission Structure...\n";
    
    // Clear existing data
    $db->exec("DELETE FROM mlm_commission_plans");
    
    // Add different commission types using correct column names
    $commissionPlans = [
        ['Direct Business Commission', 'DIRECT', 10.00, 'Commission on personal sales - 10%'],
        ['Junior Business Commission', 'JUNIOR', 5.00, 'Commission on junior associate business - 5%'],
        ['Team Override Commission', 'TEAM', 3.00, 'Override commission on team - 3%'],
        ['Leadership Bonus', 'LEADERSHIP', 2.00, 'Leadership performance bonus - 2%'],
        ['Director Override', 'DIRECTOR', 1.00, 'Director level override - 1%']
    ];
    
    foreach ($commissionPlans as $plan) {
        $db->exec("INSERT INTO mlm_commission_plans (plan_name, plan_code, description, plan_type, status) VALUES (
            '{$plan[0]}', '{$plan[1]}', '{$plan[3]}', '{$plan[2]}', 'active'
        )");
    }
    
    // 4. Add Sample Associates (using correct column names)
    echo "👥 Adding Sample Associates...\n";
    
    // Check if mlm_associates has the required columns
    $stmt = $db->query("DESCRIBE mlm_associates");
    $columns = $stmt->fetchAll();
    $columnNames = array_column($columns, 'Field');
    
    if (!in_array('name', $columnNames)) {
        $db->exec("ALTER TABLE mlm_associates ADD COLUMN name VARCHAR(200) AFTER id");
    }
    
    if (!in_array('email', $columnNames)) {
        $db->exec("ALTER TABLE mlm_associates ADD COLUMN email VARCHAR(100) AFTER name");
    }
    
    if (!in_array('phone', $columnNames)) {
        $db->exec("ALTER TABLE mlm_associates ADD COLUMN phone VARCHAR(20) AFTER email");
    }
    ...
    foreach ($sampleAssociates as $index => $associate) {
        $sponsorId = $associate[5] ? $associate[5] : 'NULL';
        $userId = $index + 1; // Create user_id as sequential
        
        $db->exec("INSERT INTO mlm_associates (name, email, phone, level_id, status, sponsor_id, joining_date, user_id, plan_id, rank, total_earnings, monthly_earnings, team_size, direct_referrals) VALUES (
            '{$associate[0]}', '{$associate[1]}', '{$associate[2]}', {$associate[3]}, '{$associate[4]}', $sponsorId, '{$associate[6]}', $userId, $planId, '{$associate[0]}', 0.00, 0.00, 0, 0
        )");
    }
    
    // 5. Setup Network Tree
    ...
    // 6. Add Sample Commission Records
    ...
    $commissionRecords = [
        [1, 5000.00, 'direct', 'Direct business commission', '2024-04-01', 'paid'],
        [1, 2500.00, 'junior', 'Junior business commission', '2024-04-01', 'paid'],
        [2, 7500.00, 'direct', 'Direct business commission', '2024-04-01', 'paid'],
        [2, 3000.00, 'team', 'Team override commission', '2024-04-01', 'paid'],
        [3, 6000.00, 'direct', 'Direct business commission', '2024-04-01', 'paid'],
        [4, 4000.00, 'leadership', 'Leadership bonus', '2024-04-01', 'paid'],
        [1, 1500.00, 'junior', 'Junior business commission from Bob', '2024-04-05', 'pending'],
        [2, 2000.00, 'team', 'Team override from Alice', '2024-04-05', 'pending']
    ];
    ...
    // 7. Create MLM Admin Controller if not exists
...
class MLMController 
{
    ...
    // 8. Create MLM Views if not exist
