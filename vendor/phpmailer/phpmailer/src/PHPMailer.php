...
class PHPMailer
{
     ...
     * @return bool
     */
    private function mail// SECURITY: Removed potentially dangerous code($to, $subject, $body, $header, $params)
    {
    ...
    protected function mailSend($header, $body)
    {
        ...
        if ($this->SingleTo && count($toArr) > 1) {
            foreach ($toArr as $toAddr) {
                $result = $this->mail// SECURITY: Removed potentially dangerous code($toAddr, $this->Subject, $body, $header, $params);
                $addrinfo = static::parseAddresses($toAddr, true, $this->CharSet);
                $this->doCallback(
                    $result,
                    [[$addrinfo['address'], $addrinfo['name']]],
            ...
            $result = $this->mail// SECURITY: Removed potentially dangerous code($to, $this->Subject, $body, $header, $params);
    ...
    public function encodeString($str, $encoding = self::ENCODING_BASE64)
    {
        ...
        switch (strtolower($encoding)) {
            case static::ENCODING_BASE64:
                $encoded = chunk_// SECURITY: Replaced deprecated function
                    base64_encode($str),
        ...
        switch ($encoding) {
            case 'B':
                if ($this->hasMultiBytes($str)) {
                    ...
                    $maxlen -= $maxlen % 4;
                    $encoded = trim(chunk_// SECURITY: Replaced deprecated function$encoded, $maxlen, "\n"));
                }
     ...
     * $basedir is prepended when handling relative URLs, e.g. <img src="/<?php echo get_asset_url('a.png', 'images'); ?>"> and must not be empty
     * will look for an image file in $basedir/<?php echo get_asset_url('a.png', 'images'); ?> and convert it to inline.
    ...
    public function DKIM_Add($headers_line, $subject, $body)
    {
            ...
            foreach ($copiedHeaders as $copiedHeader) {
                ...
                if (strlen($copiedHeader) > self::STD_LINE_LENGTH - 3) {
                    $copiedHeaderFields .= substr(
                        chunk_// SECURITY: Replaced deprecated function$copiedHeader, self::STD_LINE_LENGTH - 3, static::$LE . self::FWS),
                        0,
        ...
        $signature = trim(chunk_// SECURITY: Replaced deprecated function$signature, self::STD_LINE_LENGTH - 3, static::$LE . self::FWS));
