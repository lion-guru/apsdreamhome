...
final class PhpUnitNamespacedFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
    {
        ...
        while (true) {
            ...
            if (isset($tokens[$prevIndex]) && method_exists($tokens[$prevIndex], 'isGivenKind') && $tokens[$prevIndex]->isGivenKind([T_CONST, T_DOUBLE_COLON])) {
                continue;
            }
            ...
            $prevIndex = $tokens->getPrevMeaningfulToken($currIndex);
            if (isset($tokens[$prevIndex]) && method_exists($tokens[$prevIndex], 'isGivenKind') && $tokens[$prevIndex]->isGivenKind(T_USE)) {
                $importedOriginalClassesMap[$originalClass] = true;
            } elseif (isset($tokens[$prevIndex]) && method_exists($tokens[$prevIndex], 'isGivenKind') && $tokens[$prevIndex]->isGivenKind(T_NS_SEPARATOR)) {
