 ...
 * Fixes database connection and table issues
 */
...
<html>
<head>
    <title>Database Connection Fix - APS Dream Home</title>
    <style>
...
<body>
    <div class='container'>
        <h1>🗄️ APS Dream Home - Database Connection Fix</h1>";
...
try {
    ...
    if ($conn->query($sql) === TRUE) {
        echo "<div class='success'>✅ Properties table created/verified successfully</div>";
    } else {
    ...
    if ($conn->query($sql) === TRUE) {
        echo "<div class='success'>✅ Site settings table created/verified successfully</div>";
    } else {
    ...
    foreach ($default_settings as $setting) {
        $check_setting = $conn->query("SELECT id FROM site_settings WHERE setting_key = '{$setting[0]}' LIMIT 1");
    ...
    $check_properties = $conn->query("SELECT id FROM properties LIMIT 1");
    if ($check_properties->num_rows == 0) {
        $sample_properties = [
        ...
        foreach ($sample_properties as $property) {
            ...
            $stmt->bind_param("ssdssiidssi", $property[0], $property[1], $property[2], $property[3], $property[4], $property[5], $property[6], $property[7], $property[8], $property[9]);
            $stmt->execute();
        }
        
        echo "<div class='success'>✅ Sample properties inserted</div>";
    } else {
        echo "<div class='info'>ℹ️ Properties already exist</div>";
    }
    ...
    echo "<p><strong>Solution:</strong> Please ensure XAMPP MySQL is running and try again.</p>";
    echo "<p><strong>Check:</strong> <a href='system_health_check.php'>Run System Health Check</a></p></div>";
}
