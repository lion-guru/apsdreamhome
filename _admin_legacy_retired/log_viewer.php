...
<div class="container">
    ...
    <form class="row g-3 mb-3" method="get">
        <div class="col-md-3">
            <input type="text" name="search" class="form-control" placeholder="Keyword (action, user, details)" value="<?= htmlspecialchars($search) ?>">
        </div>
        <div class="col-md-2">
        ...
        <div class="col-md-3">
            <select name="action_type" class="form-select">
                ...
                <?php foreach ($action_types as $atype): ?>
                    <option value="<?= htmlspecialchars($atype) ?>"<?= ($action_filter === $atype) ? ' selected' : '' ?>><?= htmlspecialchars($atype) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="col-md-2">
            <select name="user" class="form-select">
                ...
                <?php foreach ($usernames as $uname): ?>
                    <option value="<?= htmlspecialchars($uname) ?>"<?= ($user_filter === $uname) ? ' selected' : '' ?>><?= htmlspecialchars($uname) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="col-md-1">
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-striped log-table">
            ...
            <?php foreach ($entries_to_show as $i => [$timestamp, $action, $details]): ?>
                <tr>
                    <td><?= $start + $i + 1 ?></td>
                    <td class="log-entry"><?= htmlspecialchars($timestamp) ?></td>
                    <td class="log-entry"><?= htmlspecialchars($action) ?></td>
                    <td class="log-entry"><?= htmlspecialchars($details) ?></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
