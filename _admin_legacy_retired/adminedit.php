...
$roles_data = $db->fetch('SELECT name FROM roles');
$roles = array_column($roles_data, 'name');

// Fetch admin details
$user = $db->fetch('SELECT id, auser, aemail, apass, adob, aphone, role, status FROM admin WHERE id=:id', ['id' => $edit_id], false);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $updated = $db->execute(
        'UPDATE admin SET auser=:auser, aemail=:aemail, apass=:apass, adob=:adob, aphone=:aphone, role=:role, status=:status WHERE id=:id',
        [
            'auser' => $auser,
            'aemail' => $aemail,
            'apass' => $apass,
