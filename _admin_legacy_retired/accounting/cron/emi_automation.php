...
function updateInstallmentStatus() {
    ...
    $db->execute("UPDATE emi_installments 
...
function checkDefaultedPlans() {
    ...
    foreach ($results as $row) {
        ...
        $db->update('emi_plans', 
            ['status' => 'defaulted', 'updated_at' => date('Y-m-d H:i:s')], 
            ['id' => $row['id']]
        );
        ...
        // Get admin users
        $admins = $db->fetch("SELECT id FROM users WHERE role = 'admin'");
...
function generateMonthlyReport() {
    ...
    $query = "SELECT 
              ...
              WHERE MONTH(ei.due_date) = :month AND YEAR(ei.due_date) = :year";
    
    $stats = $db->fetch($query, ['month' => $month, 'year' => $year], false);
