...
// Get users using prepared statement
$stmt = $conn->prepare("SELECT id, name FROM employees ORDER BY name");
$stmt->execute();
$users = $stmt->get_result();
$stmt->close();

// Get roles using prepared statement
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check if role already exists using prepared statement
...
// List all user-role assignments using prepared statement
$stmt = $conn->prepare("SELECT e.id as user_id, e.name as user, r.id as role_id, r.name as role FROM user_roles ur JOIN employees e ON ur.user_id=e.id JOIN roles r ON ur.role_id=r.id ORDER BY e.name, r.name");
$stmt->execute();
...
<div class="container py-4">
    ...
    <h4 class="mt-5">Current Assignments</h4>
        ...
        <tbody>
            <?php while($a = $assignments->fetch_assoc()): ?>
                <tr>
                    ...
                    <td>
                        <a href="manage_user_roles.php?remove_user=<?php echo $a['user_id']; ?>&remove_role=<?php echo $a['role_id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Remove this role from user?')">Remove</a>
                    </td>
                </tr>
            <?php endwhile; ?>
        </tbody>
