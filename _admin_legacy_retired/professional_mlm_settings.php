...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'update_commission_settings':
                ...
                foreach ($commission_types as $type) {
                    ...
                    $stmt = $conn->prepare("
                        ...
                        updated_at = NOW()
                    ");
                    $stmt->bind_param("sidd", $type, $enabled, $percentage, $max_amount);
                    $stmt->execute();
                    $stmt->close();
                }
                $_SESSION['success'] = "Commission settings updated successfully!";
                break;
            ...
            case 'update_rank_settings':
                ...
                $stmt = $conn->prepare("
                    ...
                    WHERE level_id = ?
                ");
                $stmt->bind_param("ddiidi", $joining_fee, $monthly_maintenance, $team_size_required, 
                               $direct_referrals, $monthly_target, $level_id);
                
                if ($stmt->execute()) {
                    $_SESSION['success'] = "Rank settings updated successfully!";
                } else {
                    $_SESSION['error'] = "Failed to update rank settings.";
                }
                $stmt->close();
                break;
            ...
            case 'update_bonus_settings':
                ...
                foreach ($bonus_types as $type) {
                    ...
                    $stmt = $conn->prepare("
                        ...
                        updated_at = NOW()
                    ");
                    $stmt->bind_param("sidis", $type, $enabled, $amount, $validity_days, $qualification_criteria);
                    $stmt->execute();
                    $stmt->close();
                }
                $_SESSION['success'] = "Bonus settings updated successfully!";
                break;
        ...
        redirectTo('professional_mlm_settings.php');
    }
}

// Get current settings
$commission_settings = [];
$result = $conn->query("SELECT * FROM mlm_commission_settings");
while ($row = $result->fetch_assoc()) {
    $commission_settings[$row['setting_name']] = $row;
}

$bonus_settings = [];
$result = $conn->query("SELECT * FROM mlm_bonus_settings");
while ($row = $result->fetch_assoc()) {
...
$mlm_levels = $conn->query("SELECT * FROM mlm_levels ORDER BY level_order");
                                ...
                                <div class="accordion-item">
                                    <h2 class="accordion-header" id="rank<?php echo $rank['level_id']; ?>">
                                        <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" 
                                            ...
                                            <strong><?php echo htmlspecialchars($rank['level_name']); ?></strong>
                                            <span class="ms-auto">
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <input type="text" class="form-control form-control-sm" 
                                                           ...
                                                           value="<?php echo htmlspecialchars($setting['qualification_criteria']); ?>">
                                                </td>
                                                <td><small class="text-muted"><?php echo $description; ?></small></td>
                                            </tr>
                                            <?php endforeach; ?>
