...
function createMLMPlan($data)
{
    ...
    try {
        ...
        $plan_query = "INSERT INTO mlm_commission_plans
                      ...
                      VALUES (:plan_name, :plan_code, :plan_type, :description, :joining_fee, :monthly_target, 'active', :created_by, NOW())";

        $db->execute($plan_query, [
            ...
            foreach ($data['levels'] as $index => $level) {
                if (!empty($level['level_name'])) {
                    $level_query = "INSERT INTO mlm_plan_levels
                                   ...
                                   VALUES (:plan_id, :level_name, :level_order, :direct_commission, :team_commission,
                                    :level_bonus, :matching_bonus, :leadership_bonus, :performance_bonus,
                                    :monthly_target, NOW())";
                    ...
                    $db->execute($level_query, [
                        ...
                        'level_order' => ($index + 1),
                        'direct_commission' => $level['direct_commission'] ?? 0,
                        'team_commission' => $level['team_commission'] ?? 0,
                        'level_bonus' => $level['level_bonus'] ?? 0,
                        'matching_bonus' => $level['matching_bonus'] ?? 0,
                        'leadership_bonus' => $level['leadership_bonus'] ?? 0,
                        'performance_bonus' => $level['performance_bonus'] ?? 0,
                        'monthly_target' => $level['monthly_target'] ?? 0
                    ]);
                }
            }
        }
            ...
            foreach ($data['bonuses'] as $bonus) {
                if (!empty($bonus['bonus_name'])) {
                    $bonus_query = "INSERT INTO mlm_plan_bonuses
                                   ...
                                   VALUES (:plan_id, :bonus_name, :bonus_type, :bonus_percentage, :min_achievement,
                                    :max_achievement, 1, NOW())";

                    $db->execute($bonus_query, [
                        'plan_id' => $plan_id,
                        'bonus_name' => $bonus['bonus_name'],
                        'bonus_type' => $bonus['bonus_type'] ?? 'percentage',
                        'bonus_percentage' => $bonus['bonus_percentage'] ?? 0,
                        'min_achievement' => $bonus['min_achievement'] ?? 0,
                        'max_achievement' => $bonus['max_achievement'] ?? 0
                    ]);
                }
            }
        }
...
$db = \App\Core\App::database();
try {
    $existing_plans = $db->fetch("SELECT * FROM mlm_commission_plans WHERE status = 'active' ORDER BY created_at DESC");
} catch (Exception $e) {
    error_log("Error getting existing plans: " . $e->getMessage());
}
?>
