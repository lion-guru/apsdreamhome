<?php
/**
 * Enhanced Security Property Type Management
 * Provides secure property type creation with comprehensive security measures
...
ini_set('error_log', __DIR__ . '/logs/add_property_type_security.log');
...
// Rate limiting for property type operations
$max_property_type_operations = 10; // operations per hour
...
if (isset($_SESSION['last_activity']) &&
    ...
    session_unset();
    session_destroy();
    logSecurityEvent('Add Property Type Session Timeout', [
        'ip_address' => $_SERVER['REMOTE_ADDR'] ?? 'UNKNOWN',
...
// Rate limiting check
$rate_limit_key = 'property_type_operations_' . md5($ip_address);
...
if ($current_time - $rate_limit_data['first_operation'] < 3600) {
    ...
    if ($rate_limit_data['operations'] > $max_property_type_operations) {
        logSecurityEvent('Property Type Operations Rate Limit Exceeded', [
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Too many property type operations. Please slow down.',
...
function logSecurityEvent($event, $context = []) {
    ...
    if ($logFile === null) {
        ...
        $logFile = $logDir . '/add_property_type_security.log';
    }
...
function validateRequestHeaders() {
    ...
    if (empty($user_agent) || strlen($user_agent) < 10) {
        logSecurityEvent('Suspicious User Agent in Add Property Type', [
...
if (!isset($_SESSION['auser'])) {
    logSecurityEvent('Unauthorized Access Attempt to Add Property Type', [
        'ip_address' => $_SERVER['REMOTE_ADDR'] ?? 'UNKNOWN',
        'user_agent' => $_SERVER['HTTP_USER_AGENT'] ?? 'UNKNOWN'
    ]);
...
if (!validateRequestHeaders()) {
    logSecurityEvent('Invalid Request Headers in Add Property Type', [
        'ip_address' => $_SERVER['REMOTE_ADDR'] ?? 'UNKNOWN'
    ]);
...
// Log page access
logSecurityEvent('Add Property Type Page Accessed', [
    'ip_address' => $_SERVER['REMOTE_ADDR'] ?? 'UNKNOWN',
    'user_agent' => $_SERVER['HTTP_USER_AGENT'] ?? 'UNKNOWN',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
            logSecurityEvent('CSRF Token Mismatch in Add Property Type', [
                'ip_address' => $_SERVER['REMOTE_ADDR'] ?? 'UNKNOWN',
        ...
        // Validate and sanitize inputs with enhanced validation
        $type_name = validateInput($_POST['type_name'] ?? '', 'property_type_name', $max_name_length);
        $available_plots = validateInput($_POST['available_plots'] ?? '', 'integer');

        // Validate required fields
        if ($type_name === false) {
            throw new Exception('Property type name is required and must contain only letters, numbers, spaces, hyphens, underscores, and parentheses.');
        }

        if ($available_plots === false || $available_plots < 0 || $available_plots > $max_plots_limit) {
            throw new Exception("Available plots must be a number between 0 and $max_plots_limit.");
        }

        // Check if property type already exists with prepared statement
        $check_stmt = $con->prepare("SELECT id FROM property_types WHERE type_name = ?");
        if (!$check_stmt) {
        ...
        // Insert property type with prepared statement
        $stmt = $con->prepare("INSERT INTO property_types (type_name, available_plots) VALUES (?, ?)");
        if (!$stmt) {
            throw new Exception('Failed to prepare insert statement: ' . $con->error);
        }

        $stmt->bind_param("si", $type_name, $available_plots);

        if (!$stmt->execute()) {
            throw new Exception('Failed to insert property type: ' . $stmt->error);
        }

        $new_type_id = $con->insert_id;
        $stmt->close();

        // Log successful property type creation
        logSecurityEvent('Property Type Created Successfully', [
            'ip_address' => $_SERVER['REMOTE_ADDR'] ?? 'UNKNOWN',
            'admin_user' => $_SESSION['auser'],
            'type_name' => $type_name,
        ...
        if (function_exists('log_admin_activity')) {
            log_admin_activity('add_property_type', 'Added property type: ' . $type_name . ' with ' . $available_plots . ' available plots');
        }

        $success = 'Property type added successfully!';
        header("Location: /admin/property_types.php?msg=" . urlencode('Property type added successfully.'));
        exit();

    } catch (Exception $e) {
        logSecurityEvent('Property Type Creation Error', [
            'error_message' => $e->getMessage(),
            'ip_address' => $_SERVER['REMOTE_ADDR'] ?? 'UNKNOWN',
            'user_agent' => $_SERVER['HTTP_USER_AGENT'] ?? 'UNKNOWN',
            'admin_user' => $_SESSION['auser'],
            'type_name' => $_POST['type_name'] ?? 'UNKNOWN'
        ]);
...
<html lang="en">
<head>
    <title>Add Property Type - Secure</title>
    <meta charset="utf-8">
            ...
            <div class="card shadow-sm">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <h2 class="mb-0">Add New Property Type</h2>
                        <a href="/admin/property_types.php" class="btn btn-outline-secondary">
                            <i class="fas fa-arrow-left me-2"></i>Back to Property Types
                        </a>
                    </div>
                    ...
                    <div class="alert alert-info d-flex justify-content-between align-items-center mb-4">
                        ...
                        <div class="text-end">
                            <small class="text-muted">
                                ...
                                Rate limit: <?php echo $max_property_type_operations - $rate_limit_data['operations']; ?>/<?php echo $max_property_type_operations; ?> remaining
                            </small>
                        </div>
                    </div>
                    ...
                    <form method="POST" action="" class="needs-validation" novalidate>
                        ...
                        <div class="form-floating mb-3 position-relative">
                            <input type="text" class="form-control" id="type_name" name="type_name"
                                   placeholder="Property Type" required maxlength="<?php echo $max_name_length; ?>"
                                   value="<?php echo htmlspecialchars($_POST['type_name'] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
                                   pattern="[A-Za-z0-9\s\-_()]+"
                                   title="Only letters, numbers, spaces, hyphens, underscores, and parentheses allowed">
                            <label for="type_name"><i class="fa fa-building"></i> Property Type Name *</label>
                            <div class="invalid-feedback">Please enter a valid property type name (letters, numbers, spaces, hyphens, underscores, parentheses only).</div>
                            <div class="form-text">Enter the name of the property type (e.g., Apartment, Villa, Plot)</div>
                        </div>

                        <div class="form-floating mb-4 position-relative">
                            <input type="number" class="form-control" id="available_plots" name="available_plots"
                                   placeholder="Available Plots" required min="0" max="<?php echo $max_plots_limit; ?>"
                                   value="<?php echo htmlspecialchars($_POST['available_plots'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                            <label for="available_plots"><i class="fa fa-th-list"></i> Available Plots *</label>
                            <div class="invalid-feedback">Please enter a valid number of available plots (0-<?php echo $max_plots_limit; ?>).</div>
                            <div class="form-text">Number of plots available for this property type</div>
                        </div>

                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-primary btn-lg">
                                <i class="fa fa-plus me-2"></i>Add Property Type
                            </button>
                            <a href="/admin/property_types.php" class="btn btn-outline-secondary">
            ...
            <div class="card mt-3 border-info">
                <div class="card-body">
                    ...
                    <div class="small">
                        ...
                        <div class="mb-1">
                            <i class="fas fa-check text-success me-2"></i>Rate limiting (<?php echo $max_property_type_operations; ?> operations/hour)
                        </div>
                        <div class="mb-1">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const plotsInput = document.getElementById('available_plots');
    ...
    // Real-time validation for property type name
    if (typeNameInput) {
    ...
    // Real-time validation for available plots
    if (plotsInput) {
        plotsInput.addEventListener('input', function() {
            const value = parseInt(this.value);
            const maxPlots = <?php echo $max_plots_limit; ?>;

            if (this.value !== '' && (isNaN(value) || value < 0 || value > maxPlots)) {
                this.setCustomValidity(`Please enter a number between 0 and ${maxPlots}`);
            } else {
                this.setCustomValidity('');
            }
        });
    ...
    if (form) {
        form.addEventListener('submit', function(event) {
            ...
            const plots = plotsInput ? plotsInput.value.trim() : '';
            ...
            if (!plots || isNaN(plots) || plots < 0) {
                event.preventDefault();
                alert('Please enter a valid number of available plots');
                return false;
            }

            // Log form submission attempt
            logSecurityEvent('Property Type Form Submission Attempt', {
                type_name: typeName,
    ...
    $(document).ajaxError(function(event, xhr, settings, thrownError) {
        logSecurityEvent('AJAX Error in Add Property Type', {
            url: settings.url,
    ...
    logSecurityEvent('Add Property Type Page Loaded', {
        max_operations: <?php echo $max_property_type_operations; ?>,
        max_name_length: <?php echo $max_name_length; ?>,
        max_plots: <?php echo $max_plots_limit; ?>
    });
});
</script>
</body>
