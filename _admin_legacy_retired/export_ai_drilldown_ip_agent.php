<?php
// Export drilldown results for a specific IP or user agent as CSV
session_start();
...
$ip = isset($_GET['ip']) ? mysqli_real_escape_string($con, $_GET['ip']) : null;
$agent = isset($_GET['agent']) ? mysqli_real_escape_string($con, $_GET['agent']) : null;
...
$res = mysqli_query($con, "SELECT created_at, action, suggestion_text, feedback, notes, ip_address, user_agent FROM ai_interactions WHERE $where ORDER BY created_at DESC LIMIT 100");
