 ...
 * Provides secure role creation with comprehensive security measures
...
ini_set('error_log', __DIR__ . '/logs/add_role_security.log');
...
$max_role_operations = 20; // operations per hour
...
if (
    ...
    session_unset();
    session_destroy();
    logSecurityEvent('Add Role Session Timeout', [
        'ip_address' => $_SERVER['REMOTE_ADDR'] ?? 'UNKNOWN',
...
// Rate limiting check
$rate_limit_key = 'role_operations_' . md5($ip_address);
...
if ($current_time - $rate_limit_data['first_operation'] < 3600) {
    ...
    if ($rate_limit_data['operations'] > $max_role_operations) {
        logSecurityEvent('Role Operations Rate Limit Exceeded', [
...
function logSecurityEvent($event, $context = [])
{
    ...
    if ($logFile === null) {
        ...
        $logFile = $logDir . '/add_role_security.log';
    }
...
function validateRequestHeaders()
{
    ...
    if (empty($user_agent) || strlen($user_agent) < 10) {
        logSecurityEvent('Suspicious User Agent in Add Role', [
...
if (!isset($_SESSION['auser'])) {
    logSecurityEvent('Unauthorized Access Attempt to Add Role', [
        'ip_address' => $_SERVER['REMOTE_ADDR'] ?? 'UNKNOWN',
        'user_agent' => $_SERVER['HTTP_USER_AGENT'] ?? 'UNKNOWN'
    ]);
...
if (!validateRequestHeaders()) {
    logSecurityEvent('Invalid Request Headers in Add Role', [
        'ip_address' => $_SERVER['REMOTE_ADDR'] ?? 'UNKNOWN'
    ]);
...
// Log page access
logSecurityEvent('Add Role Page Accessed', [
    'ip_address' => $_SERVER['REMOTE_ADDR'] ?? 'UNKNOWN',
    'user_agent' => $_SERVER['HTTP_USER_AGENT'] ?? 'UNKNOWN',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
            logSecurityEvent('CSRF Token Mismatch in Add Role', [
                'ip_address' => $_SERVER['REMOTE_ADDR'] ?? 'UNKNOWN',
        ...
        // Validate role data
        $name = validateInput($_POST['name'] ?? '', 'role_name', $max_role_name_length);
        $description = validateInput($_POST['description'] ?? '', 'role_description', $max_description_length, false);
        ...
        $existing = $db->fetch("SELECT id FROM roles WHERE name = :name", ['name' => $name], false);
        if ($existing) {
        ...
        logSecurityEvent('Role Created Successfully', [
            'ip_address' => $_SERVER['REMOTE_ADDR'] ?? 'UNKNOWN',
            'admin_user' => $_SESSION['auser'],
            'role_name' => $name,
        ...
        if (function_exists('log_admin_activity')) {
            log_admin_activity('add_role', 'Added role: ' . $name . ' - ' . $description);
        }

        $msg = 'Role added successfully!';
        header("Location: /admin/roles.php?msg=" . urlencode('Role added successfully.'));
        exit();
    } catch (Exception $e) {
        logSecurityEvent('Role Creation Error', [
            'error_message' => $e->getMessage(),
            'ip_address' => $_SERVER['REMOTE_ADDR'] ?? 'UNKNOWN',
            'user_agent' => $_SERVER['HTTP_USER_AGENT'] ?? 'UNKNOWN',
            'admin_user' => $_SESSION['auser'],
            'role_name' => $_POST['name'] ?? 'UNKNOWN',
            'role_description' => $_POST['description'] ?? 'UNKNOWN'
        ]);
                    ...
                    <div class="card-body p-4">
                        ...
                        <form method="POST" action="" class="needs-validation" novalidate>
                            ...
                            <div class="form-floating mb-3 position-relative">
                                ...
                                <label for="name"><i class="fa fa-user-tag"></i> Role Name *</label>
                                ...
                                <div class="form-text">Enter a unique name for the role (e.g., Manager, Sales Rep)</div>
                            </div>

                            <div class="form-floating mb-4 position-relative">
                                <textarea class="form-control" id="description" name="description"
                                    placeholder="Role Description" rows="3" maxlength="<?php echo $max_description_length; ?>"
                                    style="height: 100px;"><?php echo htmlspecialchars($_POST['description'] ?? '', ENT_QUOTES, 'UTF-8'); ?></textarea>
                                <label for="description"><i class="fa fa-align-left"></i> Description</label>
                                <div class="invalid-feedback">Please enter a valid description (letters, numbers, spaces, and basic punctuation only).</div>
                                <div class="form-text">Brief description of the role's responsibilities and permissions</div>
                            </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (form) {
                form.addEventListener('submit', function(event) {
                    ...
                    logSecurityEvent('Role Form Submission Attempt', {
                        role_name: name,
                        role_description: description
                    });
            ...
            $(document).ajaxError(function(event, xhr, settings, thrownError) {
                logSecurityEvent('AJAX Error in Add Role', {
                    url: settings.url,
            ...
            logSecurityEvent('Add Role Page Loaded', {
