...
try {
    ...
    // 6. Add Marketing Campaign
    echo "📈 Adding Marketing Campaign for Raghunath Nagri...\n";
    $db->exec("INSERT INTO marketing_campaigns (colony_id, campaign_name, campaign_type, description, target_amount, raised_amount, start_date, end_date, status, features, terms_conditions, contact_person, contact_phone, contact_email, is_featured, is_active) VALUES (
        {$raghunathColony['id']},
    ...
    // 7. Add Investment Plans
    echo "💰 Adding Investment Plans for Raghunath Nagri...\n";
    
    // Plan 1: Zero Interest EMI
    $db->exec("INSERT INTO investment_plans (colony_id, plan_name, plan_type, min_investment, max_investment, interest_rate, tenure_months, processing_fee, down_payment_percent, emi_per_lakh, features, eligibility, documents_required, status, valid_till, is_popular) VALUES (
        {$raghunathColony['id']},
    ...
    // 8. Add Special Offers
    echo "🎁 Adding Special Offers for Raghunath Nagri...\n";
    
    // Offer 1: Early Bird Discount
    $db->exec("INSERT INTO special_offers (colony_id, offer_title, offer_type, description, discount_percent, max_discount_amount, min_investment, valid_from, valid_till, terms_conditions, is_limited_time, is_active) VALUES (
        {$raghunathColony['id']},
    ...
    foreach ($sampleInvestors as $investor) {
        $db->exec("INSERT INTO funding_investors (campaign_id, investor_name, investor_phone, investor_email, investment_amount, investment_date, payment_method, transaction_id, status, notes) VALUES (
            $campaignId,
            '{$investor[0]}',
            '{$investor[1]}',
            '{$investor[2]}',
    ...
    // 10. Update Raghunath Nagri with Marketing Info
    echo "🏰 Updating Raghunath Nagri with Marketing Information...\n";
    $db->exec("UPDATE colonies SET description = 'Premium residential colony in Gorakhpur named after Lord Raghunath. Modern amenities with traditional values and excellent location. Special funding campaign available with zero-interest EMI plans and early bird discounts.', amenities = '\"Temple\", \"Park\", \"School\", \"Hospital\", \"Market\", \"24/7 Security\", \"Wide Roads\", \"Underground Drainage\", \"Street Lights\", \"Water Supply\", \"Community Hall\", \"Jogging Track\", \"Children Play Area\", \"Senior Citizen Corner\"', total_plots = 85, available_plots = 25, starting_price = 2000000 WHERE id = {$raghunathColony['id']}");
