...
class AccessControlMiddleware {
    ...
    public static function checkIPRestrictions($userId = null) {
        ...
        try {
            ...
            // No IP restrictions
            if (!$userData || empty($userData['allowed_ips'])) {
                return true;
            }

            // Parse allowed IPs
            $allowedIPs = json_decode($userData['allowed_ips'], true);
            $currentIP = $_SERVER['REMOTE_ADDR'] ?? '';

            // Check IP against allowed list
            $ipAllowed = self::isIPInAllowedList($currentIP, $allowedIPs);

            // Log IP check
            AdminLogger::log('IP_ACCESS_CHECK', [
                'user_id' => $userId,
                'current_ip' => $currentIP,
                'status' => $ipAllowed ? 'ALLOWED' : 'BLOCKED'
            ]);
