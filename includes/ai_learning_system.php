...
class AILearningSystem {
    ...
    private function storeUserPreference($category, $key, $value) {
        ...
        $stmt->execute([$this->user_id, $category, $key, json_encode($value)]);
    }
    ...
    private function reinforceCurrentPersonality($feedback_type) {
        ...
        $stmt->execute([json_encode($current_traits), $this->ai_personality['id']]);
    }
    ...
    private function updateUserProfile() {
        ...
        $stmt = $this->pdo->prepare("SELECT COUNT(*) as total_interactions FROM ai_user_interactions WHERE user_id = ?");
        ...
        $stmt->execute([$total_interactions, $this->user_id]);
    }
    ...
    private function loadAIPersonality() {
        $stmt = $this->pdo->prepare("SELECT * FROM ai_agent_personality WHERE active = 1 LIMIT 1");
        $stmt->execute();
    ...
    private function generateSessionId() {
        return 'ai_session_' . uniqid() . '_' . time();
    }
    ...
    public function getLearningStats() {
        ...
        // Total interactions
        $stmt = $this->pdo->prepare("SELECT COUNT(*) as total FROM ai_user_interactions WHERE user_id = ?");
        $stmt->execute([$this->user_id]);
        $stats['total_interactions'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];

        // Knowledge base size
        $stmt = $this->pdo->prepare("SELECT COUNT(*) as total FROM ai_knowledge_base");
        $stmt->execute();
        $stats['knowledge_entries'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];

        // Workflow patterns
        $stmt = $this->pdo->prepare("SELECT COUNT(*) as total FROM ai_workflow_patterns");
        $stmt->execute();
        $stats['workflow_patterns'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];

        // User preferences
        $stmt = $this->pdo->prepare("SELECT COUNT(*) as total FROM ai_user_preferences WHERE user_id = ?");
        $stmt->execute([$this->user_id]);
        $stats['user_preferences'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
