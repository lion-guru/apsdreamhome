...
if (file_exists($mainDbFile)) {
    ...
    echo "<p><strong>Import Command:</strong> <code>mysql -u root -p apsdreamhome < {$mainDbFile}</code></p>";
    echo "</div>";
} else {
...
if (isset($_POST['restore_database'])) {
    try {
        ...
        $backupCommand = "mysqldump -u root apsdreamhome > database/current_backup_$(date +%Y%m%d_%H%M%S).sql";
...
echo "mysqldump -u root apsdreamhome > database/backup_before_restore.sql<br><br>";

echo "# Step 2: Drop and recreate database<br>";
echo "mysql -u root -e \"DROP DATABASE IF EXISTS apsdreamhome; CREATE DATABASE apsdreamhome CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;\"<br><br>";

echo "# Step 3: Import main database<br>";
echo "mysql -u root apsdreamhome < database/apsdreamhomes.sql<br><br>";

echo "# Step 4: Verify restoration<br>";
echo "mysql -u root -e \"USE apsdreamhome; SHOW TABLES;\" | wc -l<br>";
echo "</div>";

echo "<h3>Expected Results:</h3>";
echo "<ul style='font-size: 16px;'>";
echo "<li>✅ Database: apsdreamhome</li>";
