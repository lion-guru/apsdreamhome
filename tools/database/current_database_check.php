...
try {
    ...
    if ($conn->connect_error) {
        ...
        echo "<p style='color: green;'>✅ Database: apsdreamhome</p>";
...
try {
    ...
    $result = $conn->query("SELECT COUNT(*) as count FROM information_schema.tables WHERE table_schema = 'apsdreamhome' AND table_type = 'BASE TABLE'");
    if ($result) {
...
echo "<ul style='font-size: 16px;'>";
echo "<li><strong>Database Name:</strong> apsdreamhome</li>";
...
if ($totalTables == 0) {
    ...
    echo "<li><strong>Command:</strong> <code>mysql -u root -p apsdreamhome < database/apsdreamhomes.sql</code></li>";
