...
echo "<p><strong>Server Software:</strong> " . h($_SERVER['SERVER_SOFTWARE'] ?? 'Unknown') . "</p>";
echo "<p><strong>Document Root:</strong> " . h($_SERVER['DOCUMENT_ROOT']) . "</p>";
echo "<p><strong>Current File:</strong> " . h(__FILE__) . "</p>";
echo "<p><strong>Current Time:</strong> " . h(date('Y-m-d H:i:s')) . "</p>";
echo "</div>";
...
foreach ($critical_files as $file) {
    ...
    echo "<div class='$status'>" . h($file) . " - " . ($exists ? '✅ EXISTS' : '❌ MISSING') . "</div>";
}
echo "</div>";
echo "</div>";
...
try {
    ...
    echo "<div class='info'>Database: " . h(DB_NAME) . " on " . h(DB_HOST) . "</div>";

    $conn->close();
} catch (Exception $e) {
    echo "<div class='error'>❌ Database Error: " . h($e->getMessage()) . "</div>";
}
echo "</div>";
...
try {
    ...
    echo "<div class='error'>❌ Security Manager Error: " . h($e->getMessage()) . "</div>";
    echo "<div class='info'>Error occurred in: " . h($e->getFile()) . " at line " . h($e->getLine()) . "</div>";
}
echo "</div>";
...
foreach ($directories as $dir) {
    if (is_dir($dir)) {
        ...
        echo "<div class='$status'>" . h($dir) . " - " . ($writable ? '✅ WRITABLE' : '⚠️ NOT WRITABLE') . "</div>";
    } else {
        echo "<div class='error'>" . h($dir) . " - ❌ MISSING</div>";
    }
}
echo "</div>";
echo "</div>";
...
try {
    ...
    if (strlen($output) > 0) {
        ...
        echo "<div class='code'>" . h(substr($output, 0, 500)) . "...</div>";
        echo "</div>";
    } else {
        echo "<div class='warning'>⚠️ Index.php loaded but produced no output</div>";
    }
} catch (Exception $e) {
    echo "<div class='error'>❌ Index.php Error: " . h($e->getMessage()) . "</div>";
    echo "<div class='info'>Error in file: " . h($e->getFile()) . " at line " . h($e->getLine()) . "</div>";
    echo "<div class='code'>Stack trace: " . h($e->getTraceAsString()) . "</div>";
}
echo "</div>";
...
foreach ($php_settings as $setting) {
    ...
    echo "<tr><td style='padding: 8px; border: 1px solid #ddd;'>" . h($setting) . "</td><td style='padding: 8px; border: 1px solid #ddd;'>" . h($value) . "</td></tr>";
}
echo "</table>";
echo "</div>";
...
try {
    ...
    echo "<div class='code'>" . h(implode("\n", $recent_errors)) . "</div>";
    echo "</div>";
} catch (Exception $e) {
    echo "<div class='warning'>⚠️ Could not read error log: " . h($e->getMessage()) . "</div>";
}
echo "</div>";
