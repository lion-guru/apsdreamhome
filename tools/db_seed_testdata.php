...
try {
  ...
  // Customer seed (users table - used by CustomerAuthController)
  if ($db->query("SHOW TABLES LIKE 'users'")->rowCount() > 0) {
    ...
    if (!$stmt->fetchColumn()) {
      $cols = $db->query("SHOW COLUMNS FROM users LIKE 'name'");
      $hasName = $cols && $cols->rowCount() > 0;
      if ($hasName) {
        try {
          $db->prepare("INSERT INTO users (name, email, phone, password, status, created_at) VALUES (:n, :e, :p, :pw, 'active', NOW())")
            ->execute([':n'=>'Test User', ':e'=>'testuser@example.com', ':p'=>'+9199999999', ':pw'=>$hashUser]);
        } catch (Exception $e) {
          $db->prepare("INSERT INTO users (name, email, phone, password, created_at) VALUES (:n, :e, :p, :pw, NOW())")
            ->execute([':n'=>'Test User', ':e'=>'testuser@example.com', ':p'=>'+9199999999', ':pw'=>$hashUser]);
        }
      } else {
        $db->prepare("INSERT INTO users (email, phone, password, created_at) VALUES (:e, :p, :pw, NOW())")
          ->execute([':e'=>'testuser@example.com', ':p'=>'+9199999999', ':pw'=>$hashUser]);
      }
      echo "Seeded test user.\n";
    } else {
      echo "Test user already exists.\n";
    }
  } else {
    echo "users table not found. Skipping user seed.\n";
  }
  ...
  if ($db->query("SHOW TABLES LIKE 'user_properties'")->rowCount() > 0) {
    // Attempt to find test user id from users table
    $uidStmt = $db->prepare("SELECT id FROM users WHERE email = :e LIMIT 1");
