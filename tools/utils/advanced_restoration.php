<?php
/**
 * APS Dream Home - Advanced System Restoration
 * Smart import that handles mysqldump output and errors
 */

echo "<h1>🚀 APS Dream Home - Advanced System Restoration</h1>";
...
foreach ($sql_files as $file) {
    ...
    // Split into lines and filter out non-SQL content
    $lines = explode("\n", $sql);
    $clean_queries = [];
    ...
    foreach ($lines as $line) {
        ...
        // Skip empty lines, comments, and mysqldump headers/errors
        if (empty($line) ||
            strpos($line, '--') === 0 ||
            strpos($line, '/*') === 0 ||
        ...
        if (substr($line, -1) === ';') {
            ...
            if (!empty($query)) {
                ...
                if ($conn->query($query)) {
                    ...
                    echo "<p style='color: red; margin: 2px 0;'>❌ Query " . $file_queries . ": $error</p>";
                }
            }
            $query = '';
        }
