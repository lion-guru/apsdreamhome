...
if ($idx -ge 0) { Write-Host "Found 'KYC' at $idx: " + $r.Content.Substring($idx, 100) }
# Check for error
$idx = $r.Content.IndexOf('Fatal error')
if ($idx -ge 0) { Write-Host "FATAL ERROR at $idx: " + $r.Content.Substring($idx, 300) }
$idx = $r.Content.IndexOf('Parse error')
if ($idx -ge 0) { Write-Host "PARSE ERROR at $idx: " + $r.Content.Substring($idx, 300) }
$idx = $r.Content.IndexOf('Error')
if ($idx -ge 0) { Write-Host "ERROR at $idx: " + $r.Content.Substring($idx, 200) }
