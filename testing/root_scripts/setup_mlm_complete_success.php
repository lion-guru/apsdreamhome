...
try {
    ...
    // 1. Check and remove foreign key constraint if exists
    echo "🔧 Checking foreign key constraints...\n";
    $stmt = $db->query("SELECT CONSTRAINT_NAME FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'apsdreamhome' AND TABLE_NAME = 'mlm_commission_levels' AND REFERENCED_TABLE_NAME IS NOT NULL");
    $constraints = $stmt->fetchAll();
    
    foreach ($constraints as $constraint) {
        $db->exec("ALTER TABLE mlm_commission_levels DROP FOREIGN KEY {$constraint['CONSTRAINT_NAME']}");
        echo "✅ Dropped constraint: {$constraint['CONSTRAINT_NAME']}\n";
    }
    
    // 2. Add proper foreign key to mlm_plans
    $db->exec("ALTER TABLE mlm_commission_levels ADD CONSTRAINT fk_plan_levels_mlm_plans FOREIGN KEY (plan_id) REFERENCES mlm_plans(id) ON DELETE CASCADE");
    echo "✅ Added foreign key to mlm_plans\n";
    ...
    // 4. Setup MLM Salary Plans (create new table)
