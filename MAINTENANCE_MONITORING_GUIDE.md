...
tail -50 /var/www/apsdreamhomefinal/storage/logs/security.log

# Count today's security events
grep "$(date +%Y-%m-%d)" /var/www/apsdreamhomefinal/storage/logs/security.log | wc -l

# Check for failed login attempts
grep "login_failed" /var/www/apsdreamhomefinal/storage/logs/security.log | tail -10

# Monitor suspicious activity
grep -E "(sql_injection|unauthorized|attack)" /var/www/apsdreamhomefinal/storage/logs/security.log
```
...
php -r "
try {
    \$pdo = new PDO('mysql:host=localhost;dbname=apsdreamhomefinal', 'username', 'password');
...
# Monitor error logs
tail -20 /var/log/apache2/apsdreamhomefinal_error.log
```
...
php /var/www/apsdreamhomefinal/scripts/security-audit.php

# Run security test suite
php /var/www/apsdreamhomefinal/scripts/security-test-suite.php
...
mysqlcheck -o apsdreamhomefinal -u[username] -p[password]
...
php /var/www/apsdreamhomefinal/scripts/security-validation.php

# Check for file integrity changes
find /var/www/apsdreamhomefinal/ -type f -exec md5sum {} + | sort > /tmp/current_checksums.txt
diff /var/www/apsdreamhomefinal/checksums.md5 /tmp/current_checksums.txt
...
ls -la /var/backups/apsdreamhomefinal_*.sql

# Verify backup completeness
head -5 /var/backups/apsdreamhomefinal_backup_$(date +%Y%m%d).sql

# Test file system backup
ls -la /var/www/apsdreamhomefinal/storage/uploads/ | wc -l
```
...
sudo ls -la /var/www/apsdreamhomefinal/
```
...
tail -f /var/www/apsdreamhomefinal/storage/logs/security.log | grep -E "(attack|injection|breach)"
...
sudo chmod 000 /var/www/apsdreamhomefinal/storage/uploads/
...
find /var/www/apsdreamhomefinal/ -name "*.php~" -delete
find /var/www/apsdreamhomefinal/ -name "*.bak" -delete

# Clean up suspicious uploads
sudo rm -rf /var/www/apsdreamhomefinal/storage/uploads/suspicious/*

# Reset compromised passwords
php /var/www/apsdreamhomefinal/scripts/reset-compromised-passwords.php

# Update all security components
php /var/www/apsdreamhomefinal/scripts/update-security.php
```

#### **Step 4: Recovery**
```bash
# Restore from clean backup
mysql -u[username] -p[password] apsdreamhomefinal < /var/backups/clean_backup.sql

# Restore file system
sudo cp -r /var/backups/files_backup/* /var/www/apsdreamhomefinal/
...
php /var/www/apsdreamhomefinal/scripts/log-incident.php \
  --type=INCIDENT_TYPE \
...
nano /var/www/apsdreamhomefinal/SECURITY_POLICIES.md
...
# Find large files
find /var/www/apsdreamhomefinal/ -type f -size +10M -exec ls -lh {} \;
...
php /var/www/apsdreamhomefinal/scripts/emergency-notification.php
```
...
# Security monitoring
watch -n 5 'tail -5 /var/www/apsdreamhomefinal/storage/logs/security.log'
...
```bash
# Create alert script
cat > /var/www/apsdreamhomefinal/scripts/security-alert.php << 'EOF'
<?php
...
```bash
# Daily log analysis
cat > /var/www/apsdreamhomefinal/scripts/daily-log-analysis.sh << 'EOF'
#!/bin/bash
LOG_DIR="/var/www/apsdreamhomefinal/storage/logs"
...
chmod +x /var/www/apsdreamhomefinal/scripts/daily-log-analysis.sh
```
