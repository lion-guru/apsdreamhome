...
echo "<h2>🔍 Step 1: Check MySQL Status</h2>";
echo "<div style='background: #f8f9fa; padding: 15px; border-radius: 8px; margin: 10px 0;'>";

try {
    $mysqlPath = 'C:\\xampp\\mysql\\bin\\mysql.exe';
    if (file_exists($mysqlPath)) {
        echo "<p style='color: green;'>✅ MySQL executable found: {$mysqlPath}</p>";
    } else {
        echo "<p style='color: red;'>❌ MySQL executable not found</p>";
    }
    ...
    if ($conn->connect_error) {
        ...
        echo "<p style='color: orange;'>💡 Solution: Start MySQL service in XAMPP Control Panel</p>";
    } else {
        echo "<p style='color: green;'>✅ MySQL Connection: Successful</p>";
        $conn->close();
    }
} catch (Exception $e) {
    echo "<p style='color: red;'>❌ MySQL Check Error: " . $e->getMessage() . "</p>";
}
echo "</div>";

// Step 2: Database Files Check
echo "<h2>📁 Step 2: Database Files Check</h2>";
...
foreach ($databaseFiles as $file => $description) {
    if (file_exists($file)) {
        ...
        echo "<p style='color: green;'>✅ {$description}: {$file} ({$sizeMB} MB)</p>";
    } else {
...
if (file_put_contents($configFile, $configContent)) {
    echo "<p style='color: green;'>✅ Database configuration created: {$configFile}</p>";
} else {
    echo "<p style='color: red;'>❌ Failed to create config file</p>";
}
echo "</div>";

// Step 4: Database Creation and Import
echo "<h2>🗃️ Step 4: Database Creation & Import</h2>";
...
try {
    ...
    if ($conn->error) {
        echo "<p style='color: orange;'>⚠️ Database creation warning: " . $conn->error . "</p>";
    } else {
    ...
    echo "<p style='color: green;'>✅ Using database: {$databaseName}</p>";

    // Check if database has tables
    $result = $conn->query("SHOW TABLES");
    $tableCount = $result ? $result->num_rows : 0;

    echo "<p style='color: blue;'>📊 Current tables: {$tableCount}</p>";

    if ($tableCount < 10) {
        echo "<p style='color: orange;'>⚠️ Database appears to need data import</p>";

        // Import main database file
        $mainDbFile = 'database/apsdreamhomes.sql';
        if (file_exists($mainDbFile)) {
            echo "<p style='color: blue;'>📥 Importing main database file...</p>";
            ...
            if ($sqlContent) {
                ...
                foreach ($statements as $statement) {
                    if (!empty($statement) && !stripos($statement, 'CREATE DATABASE') && !stripos($statement, 'USE ')) {
                        if ($conn->query($statement)) {
                            ...
                            echo "<p style='color: orange;'>⚠️ Statement failed: " . substr($statement, 0, 100) . "...</p>";
                        }
                    }
                }
                ...
                echo "<p style='color: green;'>✅ Final table count: {$finalTableCount}</p>";
            } else {
        ...
        echo "<p style='color: green;'>✅ Database already has sufficient tables ({$tableCount})</p>";
    }
...
try {
    ...
    if ($conn) {
        ...
        if ($result) {
            ...
            echo "<p style='color: green;'>✅ Basic query test: " . $row['test'] . "</p>";
        }
        ...
        if ($result) {
            ...
            echo "<p style='color: green;'>✅ Tables accessible: " . count($tables) . " tables</p>";
        ...
        if ($result) {
            $row = $result->fetch_assoc();
            echo "<p style='color: green;'>✅ Users table accessible: {$row['count']} users</p>";
        }
...
// Update status
$status['MySQL Connection'] = '✅ Connected';
$status['Database Created'] = '✅ Ready';
$status['Configuration'] = file_exists('includes/config.php') ? '✅ Created' : '❌ Failed';
$status['Connection Test'] = '✅ Working';
...
try {
    ...
    if ($conn->connect_error) {
        ...
        $result = $conn->query("SHOW TABLES");
        $tableCount = $result ? $result->num_rows : 0;
        $status['Tables Imported'] = $tableCount > 10 ? '✅ ' . $tableCount . ' Tables' : '⚠️ ' . $tableCount . ' Tables';
        $conn->close();
    }
} catch (Exception $e) {
    $status['MySQL Connection'] = '❌ Error: ' . $e->getMessage();
}
...
$allGood = !in_array('❌', array_column(array_chunk($status, 1), 0));
if ($allGood) {
    echo "<h3 style='color: green; text-align: center;'>🎉 Database Setup Complete! ✅</h3>";
} else {
...
// Step 7: Next Steps
echo "<h2>📋 Step 7: Next Steps & Testing</h2>";
echo "<div style='background: #f8f9fa; padding: 15px; border-radius: 8px; margin: 10px 0;'>";
...
echo "<a href='index.php' style='background: #007bff; color: white; padding: 12px 20px; text-decoration: none; border-radius: 5px;'>🏠 Website</a>";
...
echo "<h4>🔧 Manual Database Import (if needed):</h4>";
