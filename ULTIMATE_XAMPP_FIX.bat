...
:: 1. Fix MySQL Port
echo [ACTION] Setting MySQL to Port 3306...
...
echo [ACTION] Configuring phpMyAdmin for Port 3306...
powershell -Command "$content = Get-Content '%PMA_CONFIG%'; $content = $content -replace \"'host'\\].*\", \"'host'] = '127.0.0.1';\"; if ($content -notmatch \"'port'\") { $content = $content -replace \"'host'\] = '127.0.0.1';\", \"'host'] = '127.0.0.1';`n`$cfg['Servers'][`$i]['port'] = '3306';\" } else { $content = $content -replace \"'port'\] = '.*'\", \"'port'] = '3306'\" }; $content | Set-Content '%PMA_CONFIG%'"
