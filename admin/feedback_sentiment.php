...
$feedbacks = $conn->query("SELECT * FROM feedback ORDER BY created_at DESC LIMIT 50");
?><!DOCTYPE html>
<html lang='en'>
...
<body><div class='container py-4'><h2>Automated Feedback & Sentiment Analysis</h2><form method='post'><div class='mb-3'><label>Your Feedback</label><textarea name='feedback' class='form-control'></textarea></div><button class='btn btn-success'>Submit Feedback</button></form><table class='table table-bordered mt-4'><thead><tr><th>User</th><th>Feedback</th><th>Sentiment*</th><th>Date</th></tr></thead><tbody><?php while($f = $feedbacks->fetch_assoc()): ?><tr><td><?= htmlspecialchars($f['user_email']) ?></td><td><?= htmlspecialchars($f['feedback']) ?></td><td><?= htmlspecialchars($f['sentiment']) ?></td><td><?= $f['created_at'] ?></td></tr><?php endwhile; ?></tbody></table><p class='mt-3'>*Sentiment auto-analysis ready. Integrate with AI/ML API for real-time feedback insights.</p></div></body></html>
