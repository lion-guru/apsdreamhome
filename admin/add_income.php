...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO income (uid, amount, source, date, description) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("idsss", $uid, $amount, $source, $date, $description);

    if ($stmt->execute()) {
        header("Location: dashboard.php?msg=".urlencode('Income added successfully.'));
        exit();
    } else {
            ...
            <div class="col-lg-6">
                <div class="card shadow-sm">
                    <div class="card-body p-4">
                        <h2 class="mb-4 text-center">Add New Income</h2>
                        <form method="POST" action="" class="needs-validation" novalidate>
                            ...
                            <div class="d-grid">
                                <button type="submit" class="btn btn-primary btn-lg rounded-pill"><i class="fa fa-plus"></i> Add Income</button>
                            </div>
                        </form>
                    </div>
                </div>
