...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['create_bonus'])) {
        ...
        $stmt = $conn->prepare("
            INSERT INTO mlm_special_bonuses (
        ...
        $stmt->bind_param("ssddssssssssdddiidi", 
            $bonus_name, $bonus_type, $bonus_amount, $bonus_percentage, $qualifying_rank,
            $valid_from, $valid_to, $description, $is_active, $auto_calculate,
            $frequency, $target_criteria, $minimum_sales, $minimum_team_sales,
        ...
        if ($stmt->execute()) {
            ...
            $error = "Error creating bonus program: " . $stmt->error;
        }
        $stmt->close();
    }
    ...
    if (isset($_POST['create_reward'])) {
        ...
        $stmt = $conn->prepare("
            INSERT INTO mlm_rewards_recognition (
        ...
        $stmt->bind_param("ssdsdddisssissi", 
            $reward_name, $reward_type, $reward_value, $qualifying_rank,
            $minimum_sales, $minimum_team_sales, $minimum_direct_referrals,
            $valid_from, $valid_to, $description, $is_active, $auto_distribute,
        ...
        if ($stmt->execute()) {
            ...
            $error = "Error creating reward program: " . $stmt->error;
        }
        $stmt->close();
    }
}
...
$levels = $conn->query("SELECT * FROM mlm_levels ORDER BY level_order");
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        ...
                                        <br><small class="text-muted"><?php echo htmlspecialchars($bonus['description']); ?></small>
                                    </td>
                                    <td>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <strong><?php echo htmlspecialchars($reward['reward_name']); ?></strong>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" id="reward_qualifying_rank" name="reward_qualifying_rank" required>
                                        <?php 
                                        $levels->data_seek(0); // Reset the result pointer
