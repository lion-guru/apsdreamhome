...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($stmt->execute()) {
            ...
            $notify_query = "
                SELECT u.uname, u.uemail, u.uphone, kv.status 
                FROM kyc_verification kv
                JOIN user u ON kv.associate_id = u.uid
                WHERE kv.id = ?
            ";
            $notify_stmt = $conn->prepare($notify_query);
            $notify_stmt->bind_param('i', $kyc_id);
            $notify_stmt->execute();
            $notify_result = $notify_stmt->get_result()->fetch_assoc();
            ...
            if ($notify_result) {
                ...
                send_sms($notify_result['uphone'], $message);
                send_email($notify_result['uemail'], 'KYC Verification Status', $message);
            }
...
$pending_kyc_list = $kyc_result ? $kyc_result->fetch_all(MYSQLI_ASSOC) : [];
?>
                    ...
                    <div class="col-md-4 mb-4">
                        <div class="card">
                            <div class="card-header">
                                <h5 class="card-title"><?php echo htmlspecialchars($kyc['uname']); ?></h5>
                                <small class="text-muted"><?php echo htmlspecialchars($kyc['uemail']); ?></small>
                            </div>
                            <div class="card-body">
                                <div class="documents-preview">
                                    <div class="row">
                                        <div class="col-4">
                                            <img src="../uploads/kyc/<?php echo $kyc['associate_id'] . '/' . htmlspecialchars($kyc['aadhar_doc']); ?>" 
                                                 class="kyc-document-preview" 
                                                 data-document-type="Aadhar Card"
                                                 data-document-src="../uploads/kyc/<?php echo $kyc['associate_id'] . '/' . htmlspecialchars($kyc['aadhar_doc']); ?>">
                                        </div>
                                        <div class="col-4">
                                            <img src="../uploads/kyc/<?php echo $kyc['associate_id'] . '/' . htmlspecialchars($kyc['pan_doc']); ?>" 
                                                 class="kyc-document-preview" 
                                                 data-document-type="PAN Card"
                                                 data-document-src="../uploads/kyc/<?php echo $kyc['associate_id'] . '/' . htmlspecialchars($kyc['pan_doc']); ?>">
                                        </div>
                                        <div class="col-4">
                                            <img src="../uploads/kyc/<?php echo $kyc['associate_id'] . '/' . htmlspecialchars($kyc['address_doc']); ?>" 
                                                 class="kyc-document-preview" 
                                                 data-document-type="Address Proof"
                                                 data-document-src="../uploads/kyc/<?php echo $kyc['associate_id'] . '/' . htmlspecialchars($kyc['address_doc']); ?>">
                                        </div>
                                    </div>
                                </div>
                                <hr>
