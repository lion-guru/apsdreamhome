...
function getCRMStats() {
    ...
    $stats['total_leads'] = $result ? $result->fetch_assoc()['total'] : 0;

    // New leads this month
    $query = "SELECT COUNT(*) as new_this_month FROM leads WHERE created_at >= DATE_SUB(NOW(), INTERVAL 1 MONTH)";
    $result = $conn->query($query);
    $stats['new_leads_month'] = $result ? $result->fetch_assoc()['new_this_month'] : 0;

    // Converted leads
    $query = "SELECT COUNT(*) as converted FROM leads WHERE status = 'converted'";
    $result = $conn->query($query);
    $stats['converted_leads'] = $result ? $result->fetch_assoc()['converted'] : 0;

    // Active customers
    $query = "SELECT COUNT(*) as active FROM customers WHERE status = 'active'";
    $result = $conn->query($query);
    $stats['active_customers'] = $result ? $result->fetch_assoc()['active'] : 0;

    // Monthly revenue
    $query = "SELECT SUM(amount) as monthly_revenue FROM payments WHERE payment_date >= DATE_SUB(NOW(), INTERVAL 1 MONTH) AND status = 'completed'";
    $result = $conn->query($query);
    $stats['monthly_revenue'] = $result ? $result->fetch_assoc()['monthly_revenue'] : 0;

    // Average deal size
    $query = "SELECT AVG(amount) as avg_deal FROM payments WHERE status = 'completed'";
    $result = $conn->query($query);
    $stats['avg_deal_size'] = $result ? $result->fetch_assoc()['avg_deal'] : 0;
...
function getSalesForecast() {
    ...
    $result = $conn->query($query);
    $historical = $result ? $result->fetch_all(MYSQLI_ASSOC) : [];
...
function getCustomerInsights() {
    ...
    $result = $conn->query($query);
    $top_customers = $result ? $result->fetch_all(MYSQLI_ASSOC) : [];
    ...
    $result = $conn->query($query);
    $acquisition_sources = $result ? $result->fetch_all(MYSQLI_ASSOC) : [];
