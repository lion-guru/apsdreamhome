...
$escalations = $conn->query("SELECT e.name, al.user_id, COUNT(*) as denials FROM audit_log al LEFT JOIN employees e ON al.user_id = e.id WHERE al.action='Permission Denied' AND al.created_at >= NOW() - INTERVAL 1 DAY GROUP BY al.user_id HAVING denials >= 3 ORDER BY denials DESC");
?>
<!DOCTYPE html>
<html lang='en'>
