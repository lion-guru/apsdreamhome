    ...
    try {
        if (isset($data['plan_id']) && !empty($data['plan_id'])) {
            ...
            $query = "UPDATE mlm_commission_plans SET
                      plan_name = ?, plan_code = ?, description = ?, plan_type = ?, updated_at = NOW()
                      WHERE id = ?";

            $stmt = $conn->prepare($query);
            $stmt->bind_param("ssssi", $data['plan_name'], $data['plan_code'], $data['description'], $data['plan_type'], $data['plan_id']);
            $stmt->execute();
                ...
                if (!empty($level['level_name'])) {
                    if (isset($level['level_id']) && !empty($level['level_id'])) {
                        ...
                        $level_query = "UPDATE mlm_plan_levels SET
                                       level_name = ?, level_order = ?, direct_commission = ?, team_commission = ?,
                                       level_bonus = ?, matching_bonus = ?, leadership_bonus = ?, performance_bonus = ?, monthly_target = ?
                                       WHERE id = ?";

                        $level_stmt = $conn->prepare($level_query);
                        $level_stmt->bind_param("sddddddddi", $level['level_name'], ($index + 1), $level['direct_commission'],
                                             $level['team_commission'], $level['level_bonus'], $level['matching_bonus'],
                                             $level['leadership_bonus'], $level['performance_bonus'], $level['monthly_target'], $level['level_id']);
                        $level_stmt->execute();
                    } else {
                        // Create new level
                        ...
                        $level_stmt = $conn->prepare($level_query);
                        $level_stmt->bind_param("isdddddddd", $plan_id, $level['level_name'], ($index + 1), $level['direct_commission'],
                                             $level['team_commission'], $level['level_bonus'], $level['matching_bonus'],
                                             $level['leadership_bonus'], $level['performance_bonus'], $level['monthly_target']);
                        $level_stmt->execute();
                    }
                }
            }
...
function getPlanById($plan_id) {
    ...
    $query = "SELECT * FROM mlm_commission_plans WHERE id = ?";
    $stmt = $conn->prepare($query);
...
function getPlanLevels($plan_id) {
    ...
    $query = "SELECT * FROM mlm_plan_levels WHERE plan_id = ? ORDER BY level_order";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("i", $plan_id);
    $stmt->execute();
