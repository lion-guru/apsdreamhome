...
function isAdmin() {
    try {
        ...
        if (!isset($_SESSION['admin_logged_in']) || $_SESSION['admin_logged_in'] !== true) {
            ...
            global $pdo;
            $stmt = $pdo->prepare("SELECT id, role, status FROM admin WHERE auser = ?");
            $stmt->execute([$_SESSION['admin_session']['username']]);
...
function logAdminAction($logData) {
    ...
    try {
        global $pdo;
        $stmt = $pdo->prepare("INSERT INTO admin_action_logs (action, details, ip_address, created_at) VALUES (?, ?, ?, NOW())");
        $stmt->execute([
            $logEntry['action'], 
            $logJson, 
            $logEntry['ip_address']
