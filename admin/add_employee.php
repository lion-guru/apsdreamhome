...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Handle new role creation (inline, via modal or quick add)
    if (isset($_POST['new_role']) && trim($_POST['new_role']) !== '') {
        ...
        $stmt->bind_param('ss', $new_role, $new_desc);
        $stmt->execute();
        $stmt->close();
...
<body><div class="container py-4"><h2>Add New Employee</h2><?php if($msg): ?><div class="alert alert-info"><?= $msg ?></div><?php endif; ?>
<form method="POST">
    <div class="mb-3">
        <label>Name</label>
    ...
    <div class="mb-3">
        ...
        <div class="input-group">
            <select name="role_id" class="form-control">
                <option value="">-- // SECURITY: Removed potentially dangerous code</option>
                <?php while($r = $roles->fetch_assoc()): ?>
                    <option value="<?= $r['id'] ?>"><?= htmlspecialchars($r['name']) ?></option>
                <?php endwhile; ?>
            </select>
            <button type="button" class="btn btn-outline-secondary" data-bs-toggle="collapse" data-bs-target="#addRoleBox">+ New Role</button>
        </div>
        <div class="collapse mt-2" id="addRoleBox">
            <div class="card card-body">
                <input type="text" name="new_role" class="form-control mb-2" placeholder="New Role Name">
                <input type="text" name="new_desc" class="form-control mb-2" placeholder="Description (optional)">
