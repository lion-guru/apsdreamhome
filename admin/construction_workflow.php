    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'add_resource':
                ...
                $stmt->bind_param("issis", $project_id, $name, $type, $quantity, $status);
            ...
            case 'add_daily_log':
                ...
                $sql = "INSERT INTO daily_logs (project_id, date, description, weather_conditions, work_completed, issues_faced) 
                        VALUES (?, ?, ?, ?, ?, ?)";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("isssss", $project_id, $date, $description, $weather, $work_completed, $issues);

                if ($stmt->execute()) {
                    $success = 'Daily log added successfully';
                } else {
                    $error = 'Error adding daily log: ' . $conn->error;
                }
                break;
            ...
            case 'add_checklist':
                ...
                $sql = "INSERT INTO quality_checklists (project_id, title, description, status, completion_date) 
                        VALUES (?, ?, ?, ?, ?)";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("issss", $project_id, $title, $description, $status, $completion_date);

                if ($stmt->execute()) {
                    $success = 'Quality checklist added successfully';
                } else {
                    $error = 'Error adding checklist: ' . $conn->error;
                }
                break;
            ...
            case 'add_contractor':
                ...
                $sql = "INSERT INTO contractors (name, specialization, contact_info, status) 
                        VALUES (?, ?, ?, ?)";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("ssss", $name, $specialization, $contact_info, $status);

                if ($stmt->execute()) {
                    $success = 'Contractor added successfully';
                } else {
                    $error = 'Error adding contractor: ' . $conn->error;
                }
                break;
            ...
            case 'assign_contractor':
                ...
                $sql = "INSERT INTO contractor_assignments (contractor_id, project_id, start_date, end_date, payment_terms, status) 
                        VALUES (?, ?, ?, ?, ?, ?)";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("iissss", $contractor_id, $project_id, $start_date, $end_date, $payment_terms, $status);

                if ($stmt->execute()) {
                    $success = 'Contractor assigned successfully';
                } else {
                    $error = 'Error assigning contractor: ' . $conn->error;
                }
                break;
        }
    }
...
$sql = "SELECT * FROM contractors ORDER BY name ASC";
$result = $conn->query($sql);
