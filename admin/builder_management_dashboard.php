...
if (!isset($_SESSION['admin_id']) || $_SESSION['admin_role'] !== 'Company Owner') {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    switch ($_POST['action']) {
        case 'add_builder':
            try {
                $stmt = $conn->prepare("INSERT INTO builders (name, email, mobile, address, license_number, experience_years, specialization, rating, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active', NOW())");
                $stmt->bind_param("sssssissd", $_POST['builder_name'], $_POST['builder_email'], $_POST['builder_mobile'], $_POST['builder_address'], $_POST['license_number'], $_POST['experience_years'], $_POST['specialization'], $_POST['rating']);
                $stmt->execute();
        ...
        case 'add_project':
            try {
                $stmt = $conn->prepare("INSERT INTO construction_projects (project_name, builder_id, site_id, project_type, start_date, estimated_completion, budget_allocated, status, description, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, 'planning', ?, NOW())");
                $stmt->bind_param("siisssis", $_POST['project_name'], $_POST['builder_id'], $_POST['site_id'], $_POST['project_type'], $_POST['start_date'], $_POST['estimated_completion'], $_POST['budget_allocated'], $_POST['description']);
                $stmt->execute();
        ...
        case 'update_progress':
            try {
                $stmt = $conn->prepare("INSERT INTO project_progress (project_id, progress_percentage, milestone_achieved, work_description, amount_spent, next_milestone, updated_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())");
                $stmt->bind_param("iissdsi", $_POST['project_id'], $_POST['progress_percentage'], $_POST['milestone_achieved'], $_POST['work_description'], $_POST['amount_spent'], $_POST['next_milestone'], $_SESSION['admin_id']);
                $stmt->execute();
                
                $stmt2 = $conn->prepare("UPDATE construction_projects SET progress_percentage = ?, last_updated = NOW() WHERE id = ?");
                $stmt2->bind_param("ii", $_POST['progress_percentage'], $_POST['project_id']);
                $stmt2->execute();
        ...
        case 'process_payment':
            try {
                $stmt = $conn->prepare("INSERT INTO builder_payments (project_id, builder_id, payment_amount, payment_type, payment_date, payment_method, description, paid_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())");
                $stmt->bind_param("iidssss", $_POST['project_id'], $_POST['builder_id'], $_POST['payment_amount'], $_POST['payment_type'], $_POST['payment_date'], $_POST['payment_method'], $_POST['payment_description'], $_SESSION['admin_id']);
                $stmt->execute();
...
$builders_result = $conn->query("SELECT * FROM builders ORDER BY name");
$projects_result = $conn->query("SELECT cp.*, b.name as builder_name, s.site_name FROM construction_projects cp LEFT JOIN builders b ON cp.builder_id = b.id LEFT JOIN sites s ON cp.site_id = s.id ORDER BY cp.created_at DESC");
$sites_result = $conn->query("SELECT * FROM sites ORDER BY site_name");
?>
<!DOCTYPE html>
<html lang="en">
<head>
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-3">
                                <div class="stats-card text-center">
                                    ...
                                    <h4>₹<?php echo number_format($conn->query("SELECT SUM(budget_allocated) as total FROM construction_projects")->fetch_assoc()['total'] ?? 0); ?></h4>
                                    <p>Total Budget</p>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="stats-card text-center">
                                    ...
                                    <h4><?php echo $conn->query("SELECT COUNT(*) as count FROM construction_projects WHERE status = 'in_progress'")->fetch_assoc()['count']; ?></h4>
                                    <p>In Progress</p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="tab-pane fade show active" id="pills-builders" role="tabpanel">
                                <div class="table-responsive">
                                    <table class="table table-custom">
                                        ...
                                        <tbody>
                                            <?php 
                                            $builders_result->data_seek(0);
                                            while ($builder = $builders_result->fetch_assoc()): 
                                            ?>
                                            <tr>
                                                <td><strong><?php echo htmlspecialchars($builder['name']); ?></strong></td>
                                                <td>
                                                    <?php echo htmlspecialchars($builder['email']); ?><br>
                                                    <small class="text-muted"><?php echo htmlspecialchars($builder['mobile']); ?></small>
                                                </td>
                                                <td><?php echo htmlspecialchars($builder['license_number']); ?></td>
                                                <td><?php echo $builder['experience_years']; ?> years</td>
                                                <td><?php echo htmlspecialchars($builder['specialization']); ?></td>
                                                <td>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="builder_id" class="form-select" required>
                                    ...
                                    while ($builder = $builders_result->fetch_assoc()): 
                                    ?>
                                    <option value="<?php echo $builder['id']; ?>"><?php echo htmlspecialchars($builder['name']); ?></option>
                                    <?php endwhile; ?>
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
