...
mysql -u root -p apsdreamhomefinal < apsdreamhomefinal_ultimate.sql
...
```bash
# Test login system
curl -X POST http://localhost/apsdreamhomefinal/auth/login.php \
  -d "email=admin@apsdreamhome.com&password=admin123&csrf_token=TOKEN"

# Test registration system
curl -X POST http://localhost/apsdreamhomefinal/auth/register.php \
...
1. **Backup Current System**
   ```bash
   mysqldump -u root -p apsdreamhomefinal > backup_production.sql
   ```
