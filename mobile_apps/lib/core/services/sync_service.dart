...
class SyncService {
  ...
  Future<void> _downloadLatestData() async {
    try {
      ...
      if (updates.containsKey('properties')) {
        ...
        for (final property in properties) {
          await DatabaseHelper.insert(
            ...
            // conflictAlgorithm: ConflictAlgorithm.replace, // Removed as it's not available in current version
          );
        }
      }
      ...
      if (updates.containsKey('mlm_stats') &&
          updates['mlm_stats'].containsKey('payouts')) {
      ...
      if (updates.containsKey('mlm_stats') &&
          updates['mlm_stats'].containsKey('summary')) {
      ...
      print('Error downloading data: $e'); // ignore: avoid_print
      rethrow;
    }
  }
  ...
  Future<void> _updateLastSyncTime() async {
    ...
    print('Last sync time: ${DateTime.now().toIso8601String()}'); // ignore: avoid_print
  }
...
class SyncState {
  ...
  const SyncState.syncing()
      : message = null,
        pendingCount = null;
  const SyncState.completed(this.message) : pendingCount = null;
  const SyncState.error(this.message) : pendingCount = null;
  const SyncState.pending(this.pendingCount) : message = null;

  final String? message;
  final int? pendingCount;

  bool get isInitial =>
      message == null && pendingCount == null;
  bool get isSyncing =>
      message == null && pendingCount == null;
