...
log_errors = On
error_log = /var/log/php/apsdreamhomefinal_errors.log
```
...
sudo mkdir -p /var/www/apsdreamhomefinal
sudo chown -R www-data:www-data /var/www/apsdreamhomefinal

# Set proper permissions
sudo chmod -R 755 /var/www/apsdreamhomefinal
sudo chmod -R 777 /var/www/apsdreamhomefinal/uploads
sudo chmod -R 777 /var/www/apsdreamhomefinal/backups
```

### Step 2: Database Setup
```sql
-- Create database
CREATE DATABASE apsdreamhomefinal CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- Create user
CREATE USER 'apsuser'@'localhost' IDENTIFIED BY 'YourStrongPassword123!';

-- Grant privileges
GRANT ALL PRIVILEGES ON apsdreamhomefinal.* TO 'apsuser'@'localhost';
...
git clone https://github.com/yourusername/apsdreamhomefinal.git /var/www/apsdreamhomefinal

# Option 2: FTP/SFTP upload
# Upload all files from your local apsdreamhomefinal directory
```
...
find /var/www/apsdreamhomefinal -type f -name "*.php" -exec chmod 644 {} \;
find /var/www/apsdreamhomefinal -type d -exec chmod 755 {} \;
chmod -R 777 uploads/ backups/ logs/ cache/
```
...
GRANT SELECT ON apsdreamhomefinal.* TO 'apsro'@'localhost';
```
...
sudo nano /etc/logrotate.d/apsdreamhomefinal

/var/log/apache2/apsdreamhomefinal_*log {
    daily
    missingok
    rotate 52
    compress
...
mysqldump -u apsuser -p'YourStrongPassword123!' apsdreamhomefinal > /var/backups/apsdreamhomefinal_$DATE.sql
tar -czf /var/backups/apsdreamhomefinal_files_$DATE.tar.gz /var/www/apsdreamhomefinal --exclude=/var/www/apsdreamhomefinal/uploads/temp
