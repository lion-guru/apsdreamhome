<?php
// Seed demo data for ERP dashboards
...
$tdsStatuses = ['pending', 'filed', 'paid'];

for ($i = 0; $i < 15; $i++) {
    $month = date('Y-m', strtotime("-" . rand(0, 12) . " months"));
    $status = $tdsStatuses[array_rand($tdsStatuses)];
    $amount = rand(5000, 50000);
    $section = ['194C', '194I', '194J', '194H'][array_rand([0,1,2,3])];
    
    try {
        $db->execute(
            "INSERT INTO tds_register (deduction_month, section_code, amount, status, deductee_name, deductee_pan, filed_date, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())",
            [$month, $section, $amount, $status, 'Deductee ' . rand(1, 100), 'ABCDE' . rand(1000, 9999) . 'F', $status === 'filed' ? date('Y-m-d') : null]
        );
    } catch (\Exception $e) {}
}
echo "  Done.\n";
...
try {
    // Update some pending to paid for current month
    $db->execute(
    ...
    // Also insert new ones for current month if none exist
    ...
    if ($count < 5) {
        for ($i = 0; $i < 5; $i++) {
            $db->execute(
                "INSERT INTO mlm_commission_ledger (booking_id, beneficiary_user_id, source_user_id, commission_type, amount, status, created_at, paid_at) VALUES (?, ?, ?, ?, ?, 'paid', NOW(), NOW())",
                [rand(1, 200), rand(1, 10), rand(1, 10), ['direct_sale', 'override', 'generation_bonus', 'matching_bonus'][array_rand([0,1,2,3])], rand(1000, 50000)]
            );
        }
    }
...
for ($i = 0; $i < 20; $i++) {
    ...
    try {
        $db->execute(
            "INSERT INTO employee_leave_requests (employee_id, leave_type, status, from_date, to_date, reason, applied_at, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())",
...
for ($i = 0; $i < 50; $i++) {
    ...
    try {
        $db->execute(
            ...
            [$date, $type, 'Operation description for ' . $type, $status, rand(1, 50)]
        );
    } catch (\Exception $e) {}
}
echo "  Done.\n";
...
for ($i = 0; $i < 30; $i++) {
    ...
    try {
        $db->execute(
            ...
            [$dept, 'Request ' . rand(1, 1000), 'Description for request', $status, $priority, rand(1, 50), rand(1, 50), $due]
        );
    } catch (\Exception $e) {}
}
echo "  Done.\n";

// 10. Update booking_payment_schedules - add some overdue/pending for current period
echo "Updating booking_payment_schedules for current period...\n";
try {
    // Ensure some are overdue (due date in past)
    $db->execute(
    ...
    // Add some due dates for current/next month
    $db->execute(
