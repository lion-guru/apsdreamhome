<?php
/**
 * Standalone Test Suite for APS Dream Home
 * This is a custom test framework that works without PHPUnit dependencies
 */
...
class StandaloneTestSuite
{
    ...
    public function runProjectTests()
    {
        ...
        try {
            ...
            $this->assertEquals($testProject[0], $project['title'], 'Project title should match');
            $this->assertEquals($testProject[2], $project['developer'], 'Project developer should match');
        ...
        $filters = ['status', 'developer', 'location'];
        foreach ($filters as $filter) {
            $stmt = $this->pdo->prepare("SELECT COUNT(*) as count FROM projects WHERE {$filter} IS NOT NULL");
            $stmt->execute();
            $count = $stmt->fetch()['count'];
            $this->assertTrue($count > 0, "Should find projects by {$filter}");
        }
    }
    ...
    public function runUserTests()
    {
        ...
        foreach ($roles as $role) {
            ...
            $stmt->execute([$role]);
            $count = $stmt->fetch()['count'];
            $this->assertTrue($count > 0, "Should find users with {$role} role");
        }
    }
    ...
    public function runInquiryTests()
    {
        ...
        foreach ($types as $type) {
            ...
            $stmt->execute([$type]);
            $count = $stmt->fetch()['count'];
            $this->assertTrue($count > 0, "Should find inquiries of type {$type}");
        }
    }
    ...
    {
        echo "<h1>🧪 APS Dream Home - Standalone Test Suite</h1>\n";
        echo "<p>Running comprehensive tests without PHPUnit dependencies...</p>\n";
    ...
    private function printSummary()
    {
        ...
        echo "<hr>\n";
        echo "<p><a href='javascript:history.back()' style='text-decoration: none; padding: 8px 16px; background-color: #007bff; color: white; border-radius: 4px;'>← Go Back</a></p>\n";
    }
}

// Run the test suite
$testSuite = new StandaloneTestSuite();
