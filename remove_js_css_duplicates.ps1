...
$duplicateLog = "C:\xampp\htdocs\apsdreamhomefinal\duplicate_removal_log.txt"
$backupDir = "C:\xampp\htdocs\apsdreamhomefinal\backup_duplicates_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
...
function Remove-Duplicates {
    ...
    $duplicateFiles = Get-ChildItem -Path "C:\xampp\htdocs\apsdreamhomefinal" -Recurse -Include "*.$FileExtension" | 
    ...
    foreach ($group in $duplicateFiles) {
        ...
        foreach ($file in $group.Group) {
            if ($keepFirst) {
                ...
                $preferredPath = Join-Path "C:\xampp\htdocs\apsdreamhomefinal\vendor" $file.Name
