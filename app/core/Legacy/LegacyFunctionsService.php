...
function getBookingById($conn, $id) {
    $sql = "SELECT b.*, u.name as customer_name, p.title as property_title
            FROM bookings b
            LEFT JOIN users u ON u.id = (SELECT c.user_id FROM users c WHERE c.id = b.customer_id)
...
// Merged from: C:\xampp\htdocs\apsdreamhome\app\Controllers/..\Services\Legacy\functions.php
