                        ...
                        <?php foreach ($stats['types'] as $type => $count): ?>
                            <div class="d-flex align-items-center justify-content-between mb-3">
                                <div class="d-flex align-items-center gap-2">
                                    <div class="rounded-circle bg-<?= actColor($type) ?> bg-opacity-10">
                                        <i class="fas fa-<?= actIcon($type) ?> text-<?= actColor($type) ?>"></i>
                                    </div>
                                    <div>
                                        <div class="fw-semibold small text-dark"><?= ucfirst(htmlspecialchars($type ?? '')) ?></div>
                                        <div class="text-muted"><?= $count ?> activities</div>
                                    </div>
                                </div>
                                <span class="badge bg-<?= actColor($type) ?> bg-opacity-10 text-<?= actColor($type) ?>"><?= $count ?></span>
                            </div>
                            <div class="emp-act-type-bar mb-3">
                                <div class="emp-act-type-bar-fill bg-<?= actColor($type) ?>"></div>
                            </div>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </div>
