    ...
    <div class="card border-0 shadow-sm mt-4">
        ...
        <div class="card-body aps-cp-card-body">
            ...
            <?php else: ?><ul class="list-unstyled mb-0"><?php foreach ($history as $h): ?>
                <li class="py-2 border-bottom"><i class="fas fa-circle text-<?= $h['type'] === 'created' ? 'success' : ($h['type'] === 'inquiry' ? 'info' : 'secondary') ?> me-2"></i><strong><?= htmlspecialchars(ucfirst($h['type'] ?? 'event')) ?></strong> — <?= htmlspecialchars($h['description'] ?? '') ?> <span class="float-end text-muted small"><?= htmlspecialchars($h['date'] ?? '') ?></span></li>
