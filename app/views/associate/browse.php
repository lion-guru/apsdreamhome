<?php
$page_title = $page_title ?? 'Browse Properties';
...
<div class="container-fluid px-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-0"><i class="fas fa-search text-primary me-2"></i>Browse Properties</h4>
            <small class="text-muted"><?= number_format($total) ?> properties available</small>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-body py-3">
            <form method="GET" action="<?= $base ?>/associate/browse" class="row g-2 align-items-end">
                <div class="col-md-3">
                    <input type="text" class="form-control form-control-sm" name="q" placeholder="Search properties..." value="<?= htmlspecialchars($current_filters['q'] ?? '') ?>">
                </div>
                <div class="col-md-2">
                    <select class="form-select form-select-sm" name="type">
                        <option value="">All Types</option>
                        <?php foreach (['plot','house','flat','shop','farmhouse','villa','land'] as $t): ?>
                            <option value="<?= $t ?>" <?= ($current_filters['type'] ?? '') === $t ? 'selected' : '' ?>><?= ucfirst($t) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-md-2">
                    <select class="form-select form-select-sm" name="listing">
                        ...
                        <option value="sell" <?= ($current_filters['listing'] ?? '') === 'sell' ? 'selected' : '' ?>>For Sale</option>
                        <option value="rent" <?= ($current_filters['listing'] ?? '') === 'rent' ? 'selected' : '' ?>>For Rent</option>
                    </select>
                </div>
                <div class="col-md-2">
                    <select class="form-select form-select-sm" name="sort">
                        <option value="newest" <?= ($current_filters['sort'] ?? 'newest') === 'newest' ? 'selected' : '' ?>>Newest First</option>
                ...
                <div class="col-md-1">
                    <a href="<?= $base ?>/associate/browse" class="btn btn-outline-secondary btn-sm w-100">Reset</a>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="col-lg-4 col-md-6 mb-4">
            <div class="card border-0 shadow-sm h-100 property-card" style="transition: transform 0.2s, box-shadow 0.2s;">
                ...
                <div class="card-body d-flex flex-column">
                    <h6 class="card-title fw-bold"><?= htmlspecialchars($property['name'] ?? 'Untitled') ?></h6>
                    ...
                    <div class="row small text-center border-top border-bottom py-2 mb-3 g-1">
                        ...
                        <div class="col">
                            ...
                            <strong><?= number_format($property['area_sqft']) ?></strong> sqft
                        </div>
                        <?php endif; ?>
                        <div class="col">
                    ...
                    <div class="mt-auto">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <button class="btn btn-sm btn-primary interest-btn"
                                ...
                                <i class="fas fa-hand-pointer me-1"></i>I'm Interested
                            </button>
                        </div>
                        <small class="text-muted"><i class="fas fa-eye me-1"></i><?= (int)($property['views'] ?? 0) ?> views</small>
                    </div>
                </div>
            </div>
        </div>
    ...
    <nav aria-label="Property pagination" class="mt-3">
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-body text-center py-5">
            ...
            <a href="<?= $base ?>/associate/browse" class="btn btn-primary">View All Properties</a>
        </div>
    </div>
    <?php endif; ?>
</div>
