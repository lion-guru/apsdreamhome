...
<div class="d-flex justify-content-between align-items-center mb-3">
    <div>
        <h4 class="mb-1"><i class="fas fa-map-marked-alt"></i> <?= __('assoc_cm_title', [], 'Plot Map') ?> à¢€—� <?= htmlspecialchars($colony['name'] ?? '') ?></h4>
...
<div class="map-stats-bar" id="mapStats">
    <span class="map-stat" data-status="available"><span class="dot"></span> <?= __('assoc_cm_available', [], 'Available') ?>: <strong id="statAvail">0</strong></span>
    <span class="map-stat" data-status="booked"><span class="dot"></span> <?= __('assoc_cm_booked', [], 'Booked') ?>: <strong id="statBooked">0</strong></span>
    <span class="map-stat" data-status="sold"><span class="dot"></span> <?= __('assoc_cm_sold', [], 'Sold') ?>: <strong id="statSold">0</strong></span>
    <span class="map-stat" data-status="hold"><span class="dot"></span> <?= __('assoc_cm_hold', [], 'Hold') ?>: <strong id="statHold">0</strong></span>
...
<div class="map-filter-bar mb-3">
    ...
    <button class="btn btn-sm" data-filter="available" ><?= __('assoc_cm_available', [], 'Available') ?></button>
    <button class="btn btn-sm" data-filter="booked" ><?= __('assoc_cm_booked', [], 'Booked') ?></button>
    <button class="btn btn-sm" data-filter="sold" ><?= __('assoc_cm_sold', [], 'Sold') ?></button>
    <button class="btn btn-sm" data-filter="hold" ><?= __('assoc_cm_hold', [], 'Hold') ?></button>
</div>
<div id="plotMap"></div>
...
(function() {
    ...
    legend.onAdd = function() {
        ...
        div.innerHTML = '<div >' +
            '<div><span ></span><?= __('assoc_cm_available', [], 'Available') ?></div>' +
            '<div><span ></span><?= __('assoc_cm_booked', [], 'Booked') ?></div>' +
            '<div><span ></span><?= __('assoc_cm_sold', [], 'Sold') ?></div>' +
            '<div><span ></span><?= __('assoc_cm_hold', [], 'Hold') ?></div></div>';
        return div;
    };
    legend.addTo(map);
                ...
                geojsonLayer = L.geoJSON(data, {
                    ...
                    onEachFeature: function(f, layer) {
                        ...
                        var html = '<div class="plot-popup"><h6><?= __('assoc_cm_plot', [], 'Plot') ?> #' + p.plot_number +
                            ...
                            (p.total_price ? '<div class="info-row"><span class="label"><?= __('assoc_cm_price', [], 'Price') ?></span><span class="value fw-bold">à¢—šÂ¹' + Number(p.total_price).toLocaleString() + '</span></div>' : '') +
                            (p.corner_plot ? '<div class="info-row"><span class="label"><?= __('assoc_cm_corner', [], 'Corner Plot') ?></span><span class="value">à¢Å"®</span></div>' : '') +
                            (p.park_facing ? '<div class="info-row"><span class="label"><?= __('assoc_cm_park', [], 'Park Facing') ?></span><span class="value">à¢Å"®</span></div>' : '') +
