<?php
/**
 * Associate My Customers Page - Enhanced with full details
 */
$page_title = $page_title ?? 'My Customers';
    ...
    <div class="col-6 col-md-3">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-3 text-center">
                ...
                <div class="small text-muted">Total Business</div>
            </div>
        </div>
    </div>
    <div class="col-6 col-md-3">
    ...
    <div class="col-6 col-md-3">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-3 text-center">
                ...
                <div class="small text-muted">Became Associates</div>
            </div>
        </div>
    </div>
</div>
...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
        <h5 class="mb-0"><i class="fas fa-address-book me-2 text-primary"></i>My Customers</h5>
        <div>
            <input type="text" id="searchCustomer" class="form-control form-control-sm" placeholder="Search customers..." style="width: 200px;" oninput="filterCustomers()">
        </div>
    </div>
    <div class="card-body">
        <?php if (empty($customers)): ?>
            <div class="text-center py-5">
                ...
                <p class="text-muted">Your customers will appear here once you make sales.</p>
            </div>
        <?php else: ?>
                            ...
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-start mb-3">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            <h6 class="mb-0"><?= htmlspecialchars($c['name'] ?? 'N/A') ?></h6>
                                            <small class="text-muted">Since <?= date('M Y', strtotime($c['registered_date'] ?? '')) ?></small>
                                        </div>
                                    </div>
                                    <?php if ($isAssociate): ?>
                                        <span class="badge bg-success"><i class="fas fa-check me-1"></i>Associate</span>
                                    <?php endif; ?>
                                </div>
                                ...
                                <?php if (!empty($c['plots'])): ?>
                                    <div class="mb-3">
                                        <div class="small text-muted mb-1"><i class="fas fa-map me-1"></i>Properties:</div>
                                        <?php foreach (array_slice($c['plots'], 0, 2) as $plot): ?>
                                            <div class="d-flex justify-content-between small">
                                                <span>Plot #<?= htmlspecialchars($plot['plot_number'] ?? '') ?></span>
                                                <span class="text-muted"><?= htmlspecialchars($plot['colony_name'] ?? '') ?></span>
                                            </div>
                                        <?php endforeach; ?>
                                        <?php if (count($c['plots']) > 2): ?>
                                            <small class="text-primary">+<?= count($c['plots']) - 2 ?> more</small>
                                        <?php endif; ?>
                                    </div>
                                <?php endif; ?>
                                ...
                                <?php if ($pendingAmount > 0): ?>
                                    <div class="alert alert-warning py-1 px-2 mb-3">
                                        <small><i class="fas fa-exclamation-triangle me-1"></i>₹<?= number_format($pendingAmount) ?> pending</small>
                                    </div>
                                <?php endif; ?>
                                ...
                                <?php if (!empty($c['last_booking_date'])): ?>
                                    <div class="small text-muted mb-3">
                                        <i class="fas fa-calendar me-1"></i>Last booking: <?= date('d M Y', strtotime($c['last_booking_date'])) ?>
                                    </div>
                                <?php endif; ?>
                                ...
                                <div class="d-flex flex-wrap gap-1">
                                    <?php if (!empty($c['phone'])): ?>
                                        <a href="tel:<?= $c['phone'] ?>" class="btn btn-outline-primary btn-sm" title="Call">
                                            <i class="fas fa-phone"></i>
                                        </a>
                                        <a href="https://wa.me/<?= preg_replace('/[^0-9]/', '', $c['phone']) ?>" class="btn btn-outline-success btn-sm" target="_blank" title="WhatsApp">
                                            <i class="fab fa-whatsapp"></i>
                                        </a>
                                    <?php endif; ?>
                                    <a href="<?= BASE_URL ?>/associate/customer/<?= $c['id'] ?>" class="btn btn-outline-info btn-sm" title="View Full Details">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <?php if (!$isAssociate): ?>
                                        <button class="btn btn-outline-warning btn-sm" onclick="inviteAsAssociate(<?= $c['id'] ?>, '<?= htmlspecialchars($c['name'] ?? '') ?>', '<?= htmlspecialchars($c['phone'] ?? '') ?>')" title="Invite as Associate">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Invite <strong id="inviteName"></strong> to become an associate and earn together!</p>
                <div class="alert alert-info">
                    <h6 class="alert-heading"><i class="fas fa-info-circle me-1"></i>What they'll get:</h6>
                    <ul class="mb-0 small">
                        <li><strong>5% commission</strong> on every plot sale they make</li>
                        <li><strong>Free Mobile</strong> when they achieve Associate rank</li>
                        <li><strong>Tablet, Laptop, Tour, Bike, Bullet, Car</strong> at higher ranks</li>
                        <li>Build their own team and earn from team sales</li>
                        <li>Full training and support from APS Dream Home</li>
                    </ul>
                </div>
                <div class="mb-3">
                    <label class="form-label fw-bold">Personal Message (Optional)</label>
                    <textarea id="inviteMessage" class="form-control" rows="3" placeholder="Hi! I've been working with APS Dream Home and it's been great. You should check it out too!">Hi! I've been working with APS Dream Home and earning good commissions. You should join as an associate too - let me know if you're interested!</textarea>
                </div>
            </div>
...
function inviteAsAssociate(id, name, phone) {
    ...
    const url = `https://wa.me/${phone.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(message + '\n\nJoin here: ' + window.location.origin + '/register?ref=' + '<?= $_SESSION['referral_code'] ?? '' ?>')}`;
