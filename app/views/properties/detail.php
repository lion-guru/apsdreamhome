...
if ($property) {
    $jsonLd = [
        ...
        'image' => (!empty($property['image']) ? ((strpos($property['image'], 'http') === 0 || strpos($property['image'], '/') === 0) ? $property['image'] : ((defined('BASE_URL') ? BASE_URL : '') . '/' . ltrim($property['image'], '/'))) : ''),
        'offers' => [
            '@type' => 'Offer',
    ...
    <div class="col-lg-8">
        ...
        <div class="pd-gallery mb-4 scroll-reveal">
                ...
                <div class="carousel-inner">
                    <?php foreach ($images as $i => $img): ?>
                        <div class="carousel-item <?= $i === 0 ? 'active' : '' ?>">
                            <?php $raw = $img['image_path'] ?? ''; if (empty($raw) || $raw === 'undefined' || $raw === 'null' || trim((string)$raw) === '') { $src = BASE_URL . '/assets/images/placeholder/property.svg'; } else { $src = $raw; } ?>
                            <img src="<?= htmlspecialchars($src, ENT_QUOTES, 'UTF-8') ?>" class="d-block w-100 gallery-trigger" alt="<?= htmlspecialchars(($property['title'] ?? 'Property') . ' image ' . ($i+1), ENT_QUOTES, 'UTF-8') ?>" loading="lazy" onerror="this.onerror=null;this.src='<?= BASE_URL ?>/assets/images/placeholder/property.svg'" onclick="openLightbox(<?= $i ?>)">
                        </div>
                    <?php endforeach; ?>
                </div>
            ...
            <div class="thumb-strip">
                <?php foreach ($images as $i => $img): ?>
                    <?php $raw = $img['image_path'] ?? ''; if (empty($raw) || $raw === 'undefined' || $raw === 'null' || trim((string)$raw) === '') { $src = BASE_URL . '/assets/images/placeholder/property.svg'; } else { $src = $raw; } ?>
                    <img src="<?= htmlspecialchars($src, ENT_QUOTES, 'UTF-8') ?>" class="<?= $i === 0 ? 'active' : '' ?>" onclick="goToSlide(<?= $i ?>)" alt="<?= htmlspecialchars(($property['title'] ?? 'Property') . ' thumbnail ' . ($i+1), ENT_QUOTES, 'UTF-8') ?>" loading="lazy" onerror="this.onerror=null;this.src='<?= BASE_URL ?>/assets/images/placeholder/property.svg'">
                <?php endforeach; ?>
            </div>
            <?php endif; ?>
        </div>
        ...
        <div class="pd-info-card mb-4 scroll-reveal">
            <div class="pd-title-section">
                <div class="d-flex justify-content-between align-items-start">
                    ...
                    <button class="btn btn-sm" onclick="toggleWishlist(this)" data-id="<?= $property['id'] ?? 0 ?>">
            ...
            <div class="pd-price-bar">
                ...
                <div class="pd-actions">
                    <button class="btn" onclick="document.getElementById('enquiryForm').scrollIntoView({behavior:'smooth'})">
                        <i class="fas fa-envelope me-1"></i> Enquire
                    </button>
                    <a href="tel:+919277121112" class="btn">
            ...
            <div class="card-body-padded">
                <?php foreach (array_slice($related, 0, 4) as $rel): ?>
                    <a href="/property/<?= $rel['id'] ?>" class="pd-related-item">
                        <?php
                        $relRaw = trim((string)($rel['image'] ?? ''));
                        if (empty($relRaw) || $relRaw === 'undefined' || $relRaw === 'null') {
                            $relImg = BASE_URL.'/assets/images/placeholder/property.svg';
                        } elseif (strpos($relRaw, 'http') === 0 || strpos($relRaw, '/') === 0) {
                            $relImg = $relRaw;
                        } else {
                            $relImg = BASE_URL.'/'.ltrim($relRaw, '/');
                        }
                        ?>
                        <img src="<?= htmlspecialchars($relImg, ENT_QUOTES, 'UTF-8') ?>" alt="<?= htmlspecialchars($rel['title'] ?? $rel['name'] ?? 'Property', ENT_QUOTES, 'UTF-8') ?>" loading="lazy" onerror="this.onerror=null;this.src='<?= BASE_URL ?>/assets/images/placeholder/property.svg'">
                        <div>
                            <div class="rel-name"><?= htmlspecialchars($rel['title'] ?? $rel['name'] ?? '') ?></div>
                            <div class="rel-price">₹<?= number_format($rel['price'] ?? 0) ?></div>
                            <small ><?= htmlspecialchars($rel['location'] ?? $rel['address'] ?? '') ?></small>
                        </div>
                    </a>
                <?php endforeach; ?>
            </div>
...
<div class="text-center py-5 scroll-reveal">
    ...
    <a href="/properties" class="btn px-4">
