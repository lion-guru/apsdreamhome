...
try {
    // Count investments
    $stats['investments'] = $db->query("SELECT COUNT(*) FROM properties WHERE owner_id = :uid AND type = 'investment'", ['uid' => $uid])->fetchColumn();

    // Unread Notifications count
    $stats['messages'] = $db->query("SELECT COUNT(*) FROM notifications WHERE user_id = :uid AND is_read = 0", ['uid' => $uid])->fetchColumn();
} catch (Exception $e) {
    ...
    <div class="row mb-4 animate-fade-up">
        <div class="col-md-8 d-flex align-items-center">
            <div class="position-relative me-4">
                <img src="<?= !empty($investor['avatar']) ? htmlspecialchars($investor['avatar']) : (BASE_URL . '/assets/images/default-avatar.png') ?>"
