        ...
        <div class="row align-items-center">
            <div class="col">
                ...
                <ul class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="/admin/sales/show/<?= $sale['id'] ?? 0 ?>">#<?= $sale['id'] ?? 0 ?></a></li>
    ...
    <div class="card shadow-sm border-0">
        <div class="card-body p-4">
            <form method="POST" action="/admin/sales/update/<?= $sale['id'] ?? 0 ?>">
                <div class="row g-3">
                    <div class="col-md-4"><label class="form-label">Customer</label><select name="customer_id" class="form-select"><option value="">Select Customer</option><?php foreach ($customers as $c): ?><option value="<?= $c['id'] ?>" <?= ($sale['customer_id'] ?? '') == $c['id'] ? 'selected' : '' ?>><?= $c['name'] ?></option><?php endforeach; ?></select></div>
                    ...
                    <div class="col-md-4"><label class="form-label">Associate</label><select name="associate_id" class="form-select"><option value="">None</option><?php foreach ($associates as $a): ?><option value="<?= $a['id'] ?>" <?= ($sale['associate_id'] ?? '') == $a['id'] ? 'selected' : '' ?>><?= $a['name'] ?></option><?php endforeach; ?></select></div>
                    ...
                    <div class="col-12"><button type="submit" class="btn btn-primary"><i class="fas fa-save me-1"></i>Update Sale</button> <a href="/admin/sales" class="btn btn-secondary">Cancel</a></div>
                </div>
            </form>
        </div>
    </div>
