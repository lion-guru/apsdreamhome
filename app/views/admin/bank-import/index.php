                            ...
                            <div class="btn-group btn-group-sm">
                                ...
                                <?php if (($imp['status'] ?? '') === 'completed' && (int)($imp['unmatched_rows'] ?? 0) > 0): ?>
                                    <form method="post" action="<?= BASE_URL ?>/admin/bank-import/<?= (int)$imp['id'] ?>/match" class="d-inline">
                                        ...
                                        <button type="submit" class="btn btn-outline-success" title="Auto-Match" onclick="return confirm('Auto-match transactions?')"><i class="fas fa-magic"></i></button>
                                    </form>
                                <?php endif; ?>
                                <form method="post" action="<?= BASE_URL ?>/admin/bank-import/<?= (int)$imp['id'] ?>/delete" class="d-inline">
                                    <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
                                    <button type="submit" class="btn btn-outline-danger" title="Delete" onclick="return confirm('Delete this import and all its transactions?')"><i class="fas fa-trash"></i></button>
                                </form>
                            </div>
                        </td>
                    </tr>
