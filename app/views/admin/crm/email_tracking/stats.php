...
<div class="container-fluid px-4 py-4">
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-md-2"><div class="et-stat"><div class="val"><?= ($overall['opens'] ?? 0) > 0 && ($overall['emails_tracked'] ?? 0) > 0 ? round(($overall['opens'] / $overall['emails_tracked']) * 100, 1) : 0 ?>%</div><div class="label">Open Rate</div></div></div>
    </div>

    <div class="row g-3">
        <div class="col-md-8">
            <div class="card border-0 shadow-sm"><div class="card-header"><h6 class="mb-0"><i class="fas fa-link me-1"></i>Top Clicked Links</h6></div>
        ...
        <div class="col-md-4">
            <div class="card border-0 shadow-sm"><div class="card-header"><h6 class="mb-0"><i class="fas fa-chart-line me-1"></i>Daily Activity</h6></div>
                <div class="card-body">
                ...
                <?php else: $grouped = []; foreach ($daily as $d) { $grouped[$d['day']][] = $d; } krsort($grouped); foreach (array_slice($grouped, 0, 14, true) as $day => $events): ?>
                    <div class="d-flex justify-content-between align-items-center mb-1"><small class="text-muted"><?= date('d M', strtotime($day)) ?></small><div><?php foreach ($events as $e): ?><span class="badge me-1"><?= $e['event_type'] === 'open' ? 'ðŸ‘�' : 'ðŸ–±' ?> <?= $e['cnt'] ?></span><?php endforeach; ?></div></div>
