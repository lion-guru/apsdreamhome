...
<div class="container-fluid px-4 py-4">
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-md-2"><div class="et-stat"><div class="val" style="color:#667eea"><?= ($overall['opens'] ?? 0) > 0 && ($overall['emails_tracked'] ?? 0) > 0 ? round(($overall['opens'] / $overall['emails_tracked']) * 100, 1) : 0 ?>%</div><div class="label">Open Rate</div></div></div>
    </div>

    <div class="row g-3">
        <div class="col-md-8">
            <div class="card border-0 shadow-sm" style="border-radius:14px"><div class="card-header" style="border-radius:14px 14px 0 0"><h6 class="mb-0"><i class="fas fa-link me-1"></i>Top Clicked Links</h6></div>
        ...
        <div class="col-md-4">
            <div class="card border-0 shadow-sm" style="border-radius:14px"><div class="card-header" style="border-radius:14px 14px 0 0"><h6 class="mb-0"><i class="fas fa-chart-line me-1"></i>Daily Activity</h6></div>
                <div class="card-body">
                ...
                <?php else: $grouped = []; foreach ($daily as $d) { $grouped[$d['day']][] = $d; } krsort($grouped); foreach (array_slice($grouped, 0, 14, true) as $day => $events): ?>
                    <div class="d-flex justify-content-between align-items-center mb-1"><small class="text-muted"><?= date('d M', strtotime($day)) ?></small><div><?php foreach ($events as $e): ?><span class="badge me-1" style="background:<?= $e['event_type']==='open' ? '#28a745' : '#17a2b8' ?>"><?= $e['event_type'] === 'open' ? '👁' : '🖱' ?> <?= $e['cnt'] ?></span><?php endforeach; ?></div></div>
