...
<div class="ocr-page">
        ...
        <div class="container-fluid px-4">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                <div>
                    ...
                    <p class="mb-0">Upload, extract, and verify document fields automatically</p>
                </div>
    ...
    <div class="container-fluid px-4">
        <div class="row g-3 mb-4">
            <div class="col-6 col-md-2">
                <div class="ocr-stat-card">
                    <div class="ocr-stat-icon"><i class="fas fa-file-alt"></i></div>
                    <div class="ocr-stat-value"><?= $stats['total'] ?></div>
                    <div class="ocr-stat-label">Total</div>
                </div>
            </div>
            <div class="col-6 col-md-2">
                <div class="ocr-stat-card">
                    <div class="ocr-stat-icon"><i class="fas fa-clock"></i></div>
                    <div class="ocr-stat-value"><?= $stats['pending'] ?></div>
                    <div class="ocr-stat-label">Pending</div>
                </div>
            </div>
            <div class="col-6 col-md-2">
                <div class="ocr-stat-card">
                    <div class="ocr-stat-icon"><i class="fas fa-cog fa-spin"></i></div>
                    <div class="ocr-stat-value"><?= $stats['processing'] ?></div>
                    <div class="ocr-stat-label">Processing</div>
                </div>
            </div>
            <div class="col-6 col-md-2">
                <div class="ocr-stat-card">
                    <div class="ocr-stat-icon"><i class="fas fa-check-circle"></i></div>
                    <div class="ocr-stat-value"><?= $stats['completed'] ?></div>
                    <div class="ocr-stat-label">Completed</div>
                </div>
            </div>
            <div class="col-6 col-md-2">
                <div class="ocr-stat-card">
                    <div class="ocr-stat-icon"><i class="fas fa-shield-alt"></i></div>
                    <div class="ocr-stat-value"><?= $stats['valid'] ?></div>
                    <div class="ocr-stat-label">Verified</div>
                </div>
            </div>
            <div class="col-6 col-md-2">
                <div class="ocr-stat-card">
                    <div class="ocr-stat-icon"><i class="fas fa-times-circle"></i></div>
                    <div class="ocr-stat-value"><?= $stats['failed'] + $stats['invalid'] ?></div>
        ...
        <form class="ocr-filter-bar" method="GET" action="<?= BASE_URL ?>/admin/ocr">
            <input type="text" name="q" placeholder="Search filename..." value="<?= htmlspecialchars($search ?? '') ?>" >
            <select name="status">
            ...
            <button type="submit" class="ocr-btn ocr-btn-outline"><i class="fas fa-search"></i>Filter</button>
            <?php if ($search || $status || $doctype): ?>
                <a href="<?= BASE_URL ?>/admin/ocr" class="ocr-btn ocr-btn-outline"><i class="fas fa-times"></i>Clear</a>
            <?php endif; ?>
        </form>
        ...
        <div class="ocr-card">
            ...
            <?php if (empty($documents)): ?>
                <div class="ocr-empty">
                    ...
                    <p class="mb-3">Upload your first document to start OCR extraction</p>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="ocr-filename" title="<?= htmlspecialchars($doc['original_name'] ?? '') ?>">
                                            <i class="fas fa-file me-1"></i><?= htmlspecialchars($doc['original_name'] ?? $doc['file_name'] ?? 'Unknown') ?>
                                        </span>
                                    </td>
                                    ...
                                    <td ><?= date('d M Y, h:i A', strtotime($doc['created_at'] ?? 'now')) ?></td>
                                    <td>
                                        <div class="d-flex gap-1">
                                            <a href="<?= BASE_URL ?>/admin/ocr/detail/<?= $doc['id'] ?>" class="ocr-btn ocr-btn-outline"><i class="fas fa-eye"></i></a>
                                            <?php if (($doc['ocr_status'] ?? '') === 'pending'): ?>
                                                <form method="POST" action="<?= BASE_URL ?>/admin/ocr/process/<?= $doc['id'] ?>" >
                                                    <input type="hidden" name="csrf_token" value="<?= $csrf_token ?? '' ?>">
                                                    <button type="submit" class="ocr-btn ocr-btn-success" aria-label="Play"><i class="fas fa-play"></i></button>
                                                </form>
                                            <?php endif; ?>
                                            <form method="POST" action="<?= BASE_URL ?>/admin/ocr/delete/<?= $doc['id'] ?>" data-aps-confirm="Delete this document permanently?">
                                                <input type="hidden" name="csrf_token" value="<?= $csrf_token ?? '' ?>">
                                                <button type="submit" class="ocr-btn ocr-btn-danger" aria-label="Delete"><i class="fas fa-trash"></i></button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
