    ...
    <div class="row">
        <div class="col-md-5">
            <div class="card aps-cp-card">
                <div class="card-header aps-cp-card-header"><i class="fas fa-plus"></i> Add Rule</div>
                <div class="card-body aps-cp-card-body">
                    <form method="post" action="<?= BASE_URL ?>/admin/commission/telecaller/rules/store">
                        ...
                        <button type="submit" class="btn"><i class="fas fa-save"></i> Save Rule</button>
                    </form>
                </div>
            </div>
        </div>
                    ...
                    <div class="table-responsive"><table class="table table-striped mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <form method="POST" action="<?= BASE_URL ?>/admin/commission/telecaller/rules/toggle/<?= $r['id'] ?>" >
                                        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                                        <button type="submit" class="btn btn-sm btn-warning"><i class="fas fa-toggle-<?= $r['is_active']?'on':'off' ?>"></i></button>
                                    </form>
                                    <form method="POST" action="<?= BASE_URL ?>/admin/commission/telecaller/rules/delete/<?= $r['id'] ?>" data-aps-confirm="Delete rule #<?= $r['id'] ?>?">
                                        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
