                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                            ...
                            <tr><td class="ps-4"><strong><?= $p['name'] ?></strong></td><td><?= $p['location'] ?? '-' ?></td><td><?= $p['total_plots'] ?? 0 ?></td><td><?= $p['developed_plots'] ?? 0 ?></td><td ><div class="progress"><div class="progress-bar bg-success"></div></div><small class="text-muted"><?= ($p['total_plots'] ?? 0) > 0 ? round(($p['developed_plots'] ?? 0) / $p['total_plots'] * 100) : 0 ?>%</small></td><td class="text-end pe-4"><span class="badge bg-<?= ($p['status'] ?? 'active') === 'completed' ? 'success' : (($p['status'] ?? 'active') === 'active' ? 'primary' : 'secondary') ?>-subtle text-<?= ($p['status'] ?? 'active') === 'completed' ? 'success' : (($p['status'] ?? 'active') === 'active' ? 'primary' : 'secondary') ?> rounded-pill px-3"><?= ucfirst($p['status'] ?? 'Active') ?></span></td></tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </tbody>
                </table>
