...
if (!empty($_POST["id"])) {
    ...
    if ($site_id) {
        ...
        $gata_list = $db->fetchAll("SELECT gata_id, gata_no FROM gata_master WHERE site_id = ? ORDER BY gata_no ASC", [$site_id]);
...
if (!empty($_POST['sid'])) {
    ...
    if ($gata_id) {
        ...
        $db = \App\Core\App::database();
        $plots = $db->fetchAll("SELECT plot_id, plot_no FROM plot_master WHERE gata_a = ? OR gata_b = ? OR gata_c = ? OR gata_d = ? ORDER BY plot_no ASC", [$gata_id, $gata_id, $gata_id, $gata_id]);
