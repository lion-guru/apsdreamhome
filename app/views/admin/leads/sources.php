                        ...
                        <?php foreach ($sourceData as $i => $s): $pct = round(($s['count'] / ($totalLeads ?: 1)) * 100, 1); ?>
                            <div class="mb-3">
                                <div class="d-flex justify-content-between">
                                    <span><i class="fas fa-circle me-2"></i><?= htmlspecialchars(ucfirst($s['source'] ?? 'unknown')) ?></span>
                        ...
                        <div class="d-flex align-items-end gap-2">
                            ...
                            <?php foreach ($monthlyTrend as $m): $h = round(($m['count'] / $maxM) * 180); ?>
                                <div class="flex-fill text-center">
                                    <div title="<?= $m['month'] ?>: <?= $m['count'] ?>"></div>
                                    <small class="text-muted"><?= htmlspecialchars(substr($m['month'], 0, 3)) ?></small>
                                </div>
                            <?php endforeach; ?>
                        </div>
                    <?php else: ?>
