    ...
    <div class="row g-3">
        <div class="col-md-9">
            <div class="card border-0 shadow-sm">
                <div class="card-body aps-cp-card-body" id="messagesContainer" >
                                ...
                                <div class="d-flex mb-2 <?= $isAgent ? 'justify-content-end' : '' ?>">
                                    <div class="p-2 px-3 rounded shadow-sm">
                                        <?php if ($m['sender_type'] !== 'agent' && $m['sender_name']): ?>
                                            <small class="d-block <?= $isAgent ? 'text-white-50' : 'text-muted' ?>"><?= htmlspecialchars($m['sender_name'] ?? '') ?></small>
                                        <?php endif; ?>
                                        <div ><?= htmlspecialchars($m['message'] ?? '') ?></div>
                                        <small class="d-block <?= $isAgent ? 'text-white-50' : 'text-muted' ?>">
            ...
            <div class="card border-0 shadow-sm mb-3">
                <div class="card-body aps-cp-card-body">
                    ...
                    <?php if ($session['page_url']): ?>
                        <p class="small mb-1"><strong>Page:</strong> <a href="<?= htmlspecialchars($session['page_url'] ?? '') ?>" target="_blank" class="text-truncate d-block"><?= htmlspecialchars($session['page_url'] ?? '') ?></a></p>
                    <?php endif; ?>
...
function appendMessage(senderType, senderName, message, isInternal) {
    if (isInternal) {
        ...
        messagesContainer.insertAdjacentHTML('beforeend', `<div class="d-flex mb-2 ${align}"><div class="p-2 px-3 rounded shadow-sm"><div >${escapeHtml(message)}</div><small class="d-block text-white-50">${new Date().toLocaleTimeString()}</small></div></div>`);
    }
