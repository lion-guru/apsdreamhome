...
if (isset($_GET['download']) && $_SESSION['auser'] === 'superadmin') {
    ...
    if (is_file($path)) {
        ...
        $db->execute("INSERT INTO audit_access_log (admin_user, action, details, ip_address) VALUES (:user, 'cloud_download_failed', :details, :ip)", [
                'user' => $user,
                'details' => $details,
                'ip' => $ip
            ]);
...
<body>
    <div class="container py-4">
        ...
        <table class="table table-bordered table-striped">
            ...
            <tbody>
                <?php foreach ($archives as $file): $cloud = get_cloud_url($file);
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php if ($cloud): ?>
                                <a href="<?= h($cloud) ?>" class="btn btn-success btn-sm ms-2" target="_blank">Download from Cloud</a>
        ...
        <p class="mt-3 text-muted">Archives are password-protected ZIPs. Password: <code><?= h($LOG_ARCHIVE_PASSWORD) ?></code></p>
    </div>
</body>
