...
class SuperAdminController {
    ...
    public function updatePageContent($pageId, $content, $layout = null) {
        ...
        try {
            ...
            // Update content
            $result = $this->db->query("UPDATE pages SET content = ?, layout = ?, updated_at = NOW() WHERE id = ?", [$content, $layout, $pageId]);
    ...
    private function backupContent($pageId) {
        ...
        try {
            $this->db->query("INSERT INTO content_backups (page_id, content, created_at) SELECT id, content, NOW() FROM pages WHERE id = ?", [$pageId]);
    ...
    public function updateUserPermissions($userId, $permissions) {
        ...
        try {
            ...
            $result = $this->db->query("UPDATE admin_users SET permissions = ? WHERE id = ?", [$permissionsJson, $userId]);
    ...
    public function uploadMedia($file) {
        ...
        try {
            ...
            // Generate unique filename using cryptographically secure random string
            $filename = \App\Helpers\SecurityHelper::generateRandomString(32, false) . '.' . $fileType;
            $uploadPath = dirname(__DIR__) . '/uploads/' . $filename;

            if(move_uploaded_file($file['tmp_name'], $uploadPath)) {
                // Save to database
                $this->db->query("INSERT INTO media (filename, type, size, uploaded_at) VALUES (?, ?, ?, NOW())", [$filename, $fileType, $fileSize]);
