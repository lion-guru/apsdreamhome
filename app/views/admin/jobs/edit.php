...
<div class="container-fluid">
    ...
    <div class="card shadow-sm border-0">
        <div class="card-body p-4">
            <form method="POST" action="<?= BASE_URL ?>/admin/jobs/manage/<?= $job['id'] ?? 0 ?>/update">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4"><label class="form-label">Employment Type</label><select name="employment_type" class="form-select"><option value="full_time" <?= ($job['employment_type'] ?? '') === 'full_time' ? 'selected' : '' ?>>Full Time</option><option value="part_time" <?= ($job['employment_type'] ?? '') === 'part_time' ? 'selected' : '' ?>>Part Time</option><option value="contract" <?= ($job['employment_type'] ?? '') === 'contract' ? 'selected' : '' ?>>Contract</option><option value="internship" <?= ($job['employment_type'] ?? '') === 'internship' ? 'selected' : '' ?>>Internship</option></select></div>
                    ...
                    <div class="col-12"><button type="submit" class="btn btn-primary"><i class="fas fa-save me-1"></i>Update Job</button> <a href="<?= BASE_URL ?>/admin/jobs" class="btn btn-secondary">Cancel</a></div>
                </div>
            </form>
        </div>
    </div>
