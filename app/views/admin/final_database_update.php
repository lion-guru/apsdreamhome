...
try {
    ...
    $dbName = $db->fetchOne("SELECT DATABASE() as dbname")['dbname'];
...
SELECT `uid`, 0 FROM `user` WHERE NOT EXISTS (
    SELECT 1 FROM `password_security` WHERE `user_id` = `user`.`uid`
);
...
$settings = $db->fetchAll("SELECT * FROM site_settings LIMIT 10");
if (!empty($settings)) {
    ...
    foreach ($settings as $setting) {
        echo "<tr>";
        echo "<td style='padding: 8px;'>" . h($setting['setting_name'] ?? 'N/A') . "</td>";
        echo "<td style='padding: 8px;'>" . h(substr($setting['setting_value'] ?? 'N/A', 0, 100)) . "</td>";
        echo "</tr>";
    }
    echo "</table>";
} else {
