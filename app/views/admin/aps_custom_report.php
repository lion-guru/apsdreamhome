...
if($site_name == '' && $gata_a == '' && $plot_no == '' && $farmer == '')
	...
	<?php
	 ...
	 if($site_name)
	 {
			 ...
			 {
				 $sql_chk = $db->fetchAll("SELECT * from site_master where site_id = ?", [$site_name]);
	 ...
	 <div class="table-wrapper">
         ...
         <table id="example" class="table table-striped" >
					...
					<tbody >
								...
								<?php
									$result_gata = $db->fetchAll("select * from gata_master where site_id = ?", [$site_name]);
									...
									foreach($result_gata as $row_fetch_gata)
									{
										...
										$result_kissan = $db->fetchAll("select * from kissan_master where gata_a = ? or gata_b = ? or gata_c = ? or gata_d = ?", [$gata_id, $gata_id, $gata_id, $gata_id]);
										...
										<!--<td><?php //echo h($row_fetch_gata['area'])."Sqft";?></td>
										<td><?php //echo h($row_fetch_gata['available_area'])." Sqft";?></td>-->
										<td><?php foreach($result_kissan as $row_fetch_kissan)
												...
												?></td>
										<td><?php
										$result_plot = $db->fetchAll("select * from plot_master where gata_a = ? or gata_b = ? or gata_c = ? or gata_d = ?", [$gata_id, $gata_id, $gata_id, $gata_id]);
			...
			<?php
			...
			else if ($gata_a != '')
			 {
				 ...
				 $sql_chk = $db->fetchAll("SELECT * from gata_master where site_id = ? and gata_id = ?", [$site_name, $gata_a]);
         ...
         <!-- Start DEMO HTML (Use the following code into your project)-->
					...
					<tbody>
								...
								<?php
									...
									foreach($sql_chk as $row_fetch_gata)
									{
										...
										$row_fetch_site = $db->fetch("select * from site_master where site_id = ?", [$site_id]);
										$gata_id = $row_fetch_gata['gata_id'];
										$result_kissan = $db->fetchAll("select * from kissan_master where gata_a = ? or gata_b = ? or gata_c = ? or gata_d = ?", [$gata_id, $gata_id, $gata_id, $gata_id]);
				...
				else if ($gata_a != '' && $plot_no != '')
			{
				 ...
				 $sql_chk = $db->fetchAll("SELECT * from plot_master where site_id = ?  and plot_id = ?", [$site_name, $plot_no]);
					...
					<tbody>
								...
								<?php
									...
									foreach($sql_chk as $row_fetch_plot)
									{
										...
										$row_fetch_gata = $db->fetch("select * from gata_master where gata_id = ?", [$gata_id]);
					...
					<tbody>
								...
								<?php
									...
									foreach($sql_chk as $row_fetch_plot)
									{
										...
										$row_fetch_gata_a = $db->fetch("select * from gata_master where gata_id = ?", [$gata_a_id]);
										$gata_no_a = $row_fetch_gata_a['gata_no'] ?? '';

										$row_fetch_gata_b = $db->fetch("select * from gata_master where gata_id = ?", [$gata_b_id]);
										$gata_no_b = $row_fetch_gata_b['gata_no'] ?? '';

										$row_fetch_gata_c = $db->fetch("select * from gata_master where gata_id = ?", [$gata_c_id]);
										$gata_no_c = $row_fetch_gata_c['gata_no'] ?? '';

										$row_fetch_gata_d = $db->fetch("select * from gata_master where gata_id = ?", [$gata_d_id]);
	...
	<div class="table-wrapper">
         ...
         <table id="example" class="table table-striped" style="width:100%">
					...
					else if ($gata_a == '' && $plot_no == '' && $farmer != '')
			{
				 ...
				 $sql_chk = $db->fetchAll("SELECT * from kissan_master where site_id = ?  and kissan_id = ?", [$site_name, $farmer]);
					if(count($sql_chk) > 0)
					{
