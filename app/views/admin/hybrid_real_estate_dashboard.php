...
try {
    ...
    $res = $db->fetch("SELECT COUNT(*) as cnt FROM real_estate_properties");
    $total_properties = $res['cnt'] ?? 0;

    // Company Properties
    $res = $db->fetch("SELECT COUNT(*) as cnt FROM real_estate_properties WHERE property_type='company'");
    $company_properties = $res['cnt'] ?? 0;

    // Resell Properties
    $res = $db->fetch("SELECT COUNT(*) as cnt FROM real_estate_properties WHERE property_type='resell'");
    $resell_properties = $res['cnt'] ?? 0;
    ...
    $res = $db->fetch("SELECT SUM(total_commission) as sum FROM hybrid_commission_records WHERE status='paid'");
    $total_commission = $res['sum'] ?? 0;
    ...
    $res = $db->fetch("SELECT COUNT(*) as cnt FROM real_estate_properties WHERE status='available'");
    $available_properties = $res['cnt'] ?? 0;

    // Sold Properties
    $res = $db->fetch("SELECT COUNT(*) as cnt FROM real_estate_properties WHERE status='sold'");
    $sold_properties = $res['cnt'] ?? 0;
...
$stats = [
    ...
    [
        ...
        'change' => h($company_properties) . ' Company + ' . h($resell_properties) . ' Resell',
        'change_type' => 'positive'
    ],
    [
        'icon' => 'fas fa-home',
        'value' => h($company_properties),
    ...
    [
        'icon' => 'fas fa-coins',
        'value' => '₹' . h(number_format($total_commission, 0)),
    ...
    [
        ...
        'value' => h($sold_properties),
        'label' => 'Properties Sold',
        'change' => 'Available: ' . h($available_properties),
        'change_type' => 'positive'
    ],
    [
