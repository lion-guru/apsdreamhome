...
try {
  $results = $db->fetchAll("SELECT action, COUNT(*) as cnt FROM ai_interactions WHERE created_at >= :from AND created_at <= :to GROUP BY action", [
    'from' => $from_full,
    'to' => $to_full
  ]);
  ...
  error_log("Scheduled Report Error (Summary): " . $e->getMessage());
}

$top_offenders = [];
try {
  $top_offenders = $db->fetchAll("SELECT ip_address, COUNT(*) as cnt FROM ai_interactions WHERE created_at >= :from AND created_at <= :to GROUP BY ip_address HAVING cnt > 10 ORDER BY cnt DESC LIMIT 10", [
    'from' => $from_full,
    'to' => $to_full
  ]);
} catch (Exception $e) {
  error_log("Scheduled Report Error (Offenders): " . $e->getMessage());
}
...
if ($feedback_export_enabled) {
  ...
  if ($feedback_export_from) {
    $sql .= " AND DATE(ts) >= :from";
    $params['from'] = $feedback_export_from;
  }
  if ($feedback_export_to) {
    $sql .= " AND DATE(ts) <= :to";
    $params['to'] = $feedback_export_to;
  }
  if ($feedback_export_action) {
    $sql .= " AND action = :action";
    $params['action'] = $feedback_export_action;
  }
  if ($feedback_export_user) {
    $sql .= " AND user = :user";
    $params['user'] = $feedback_export_user;
  }
  if ($feedback_export_search) {
    $sql .= " AND (suggestion LIKE :search OR feedback LIKE :search)";
    $searchTerm = "%" . $feedback_export_search . "%";
  ...
  try {
    ...
    if ($res) {
      ...
      foreach ($res as $row) {
        if (!$header) {
          $feedback_csv .= implode(',', array_keys($row)) . "\n";
          $header = true;
        }
    ...
    error_log("Scheduled Report Error (Feedback Export): " . $e->getMessage());
  }
}
...
// --- Email with attachments using NotificationManager ---
...
try {
  ...
  if ($send_summary) {
    $f = tempnam(sys_get_temp_dir(), 'summary');
    file_put_contents($f, $summary_csv);
    $attachments[] = ['path' => $f, 'name' => 'summary.csv'];
    $temp_files[] = $f;
  }
  if ($send_audit) {
    $f = tempnam(sys_get_temp_dir(), 'audit');
    file_put_contents($f, $audit_csv);
    $attachments[] = ['path' => $f, 'name' => 'audit.csv'];
    $temp_files[] = $f;
  }
  if ($feedback_export_enabled && $feedback_csv) {
    $f = tempnam(sys_get_temp_dir(), 'feedback');
    file_put_contents($f, $feedback_csv);
    $filename = 'feedback_filtered_log_' . date('Ymd_His') . '.csv';
    $attachments[] = ['path' => $f, 'name' => $filename];
    $temp_files[] = $f;
  }
  if ($anomaly_export_enabled || (isset($argv[1]) && $argv[1] === 'anomaly')) {
    $f = tempnam(sys_get_temp_dir(), 'anomaly');
    file_put_contents($f, $anomaly_csv);
    $filename = 'anomaly_filtered_log_' . date('Ymd_His') . '.csv';
    $attachments[] = ['path' => $f, 'name' => $filename];
    $temp_files[] = $f;
  }

  $summary_text = "- Analytics summary: summary.csv\n- Blocklist audit log: audit.csv\n" . $anomaly_summary;
  ...
  error_log('Scheduled Report Notification Error: ' . $e->getMessage());
}
