...
// Check if user is admin or company_owner
if (!hasRole(['admin', 'superadmin', 'company_owner'])) {
            ...
            foreach ($data['levels'] as $index => $level) {
                if (!empty($level['level_name'])) {
                    ...
                    if (!$db->query($level_query, [
                        ...
                        throw new Exception("Failed to create plan level " . ($index + 1));
                    }
                }
            }
        }
            ...
            foreach ($data['bonuses'] as $bonus) {
                if (!empty($bonus['bonus_name'])) {
                    ...
                    if (!$db->query($bonus_query, [
                        ...
                        throw new Exception("Failed to create bonus " . $bonus['bonus_name']);
                    }
                }
            }
        }
...
$db = \App\Core\App::database();
try {
    $existing_plans = $db->fetchAll($plans_query);
} catch (Exception $e) {
