...
try {
    ...
    foreach ($properties as $index => $property) {
        ...
        $existing = $db->fetchOne($check_sql, [$property['title']]);
    ...
    $type_rows = $db->fetchAll($type_sql);
    $total_value = 0;
    ...
    foreach ($type_rows as $row) {
        echo "<li>" . ucfirst($row['type']) . ": " . $row['count'] . "</li>\n";
    }
    echo "</ul>\n";
    echo "</div>\n";

    // Calculate total value
    $value_sql = "SELECT SUM(price) as total FROM properties WHERE status = 'available'";
    $total_value = $db->fetchOne($value_sql)['total'];

    echo "<div class='col-md-6'>\n";
    echo "<h5>💰 Total Portfolio Value:</h5>\n";
    echo "<h3>₹" . number_format($total_value, 2) . "</h3>\n";
    echo "</div>\n";
    echo "</div>\n";
