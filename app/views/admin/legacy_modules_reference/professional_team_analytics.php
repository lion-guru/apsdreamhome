...
// Get analytics data
$analytics = [
    'total_teams' => $db->fetchOne("SELECT COUNT(DISTINCT team_id) AS c FROM mlm_tree")['c'] ?? 0,
    'active_teams' => $db->fetchOne("SELECT COUNT(DISTINCT team_id) AS c FROM mlm_tree WHERE status = 'active'")['c'] ?? 0,
    'team_performance' => [],
    'rank_distribution' => [],
    'top_teams' => [],
    'growth_metrics' => []
...
$rank_distribution = $db->fetchAll("
    ...
    LEFT JOIN mlm_performance mp ON a.id = mp.associate_id AND mp.month_year = ?
    GROUP BY ml.level_name, ml.level_order
    ORDER BY ml.level_order
", [$current_month]);

// Get growth metrics
$growth_metrics = $db->fetchAll("
    SELECT
...
$top_teams = $db->fetchAll("
    ...
    JOIN mlm_tree mt ON t.team_id = mt.team_id
    LEFT JOIN mlm_performance mp ON mt.associate_id = mp.associate_id AND mp.month_year = ?
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <strong><?php echo h($rank['rank_name']); ?></strong>
                                    </td>
                                    <td>
