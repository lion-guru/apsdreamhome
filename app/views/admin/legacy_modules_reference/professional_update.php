...
try {
    ...
    foreach ($company_updates as $update) {
        ...
        $row = $db->fetchOne($check_sql, [$update['setting_name']]);

        if ($row) {
            // Update existing setting
            $sql = "UPDATE site_settings SET setting_value = ? WHERE setting_name = ?";
            $result = $db->execute($sql, [$update['setting_value'], $update['setting_name']]);
        } else {
            // Insert new setting
            $sql = "INSERT INTO site_settings (setting_name, setting_value) VALUES (?, ?)";
            $result = $db->execute($sql, [$update['setting_name'], $update['setting_value']]);
        }
