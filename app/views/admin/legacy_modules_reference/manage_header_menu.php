...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['menu_json'])) {
    ...
    $sql = "INSERT INTO site_settings (setting_name, value) VALUES ('header_menu_items', :menu) ON DUPLICATE KEY UPDATE value = :menu2";
    $db->execute($sql, ['menu' => $menu, 'menu2' => $menu]);
    $msg = 'Menu updated successfully!';
}

// Fetch menu from DB
$row = $db->fetch("SELECT value FROM site_settings WHERE setting_name='header_menu_items'");
if ($row) {
        ...
        <table class="table table-bordered bg-white" id="menuTable">
            ...
            <?php foreach ($menu_items as $i => $item): ?>
                <tr class="main-menu-row">
                    <td><input type="text" class="form-control" value="<?=h($item['text'])?>"></td>
                    <td><input type="text" class="form-control" value="<?=h($item['url'])?>"></td>
                    <td>
                        ...
                        <table class="table table-sm submenu-table mb-0 mt-2">
                            ...
                            <?php if (!empty($item['children'])): foreach ($item['children'] as $sub): ?>
                                <tr class="submenu-row">
                                    <td><input type="text" class="form-control" value="<?=h($sub['text'])?>" placeholder="Sub-menu Text"></td>
                                    <td><input type="text" class="form-control" value="<?=h($sub['url'])?>" placeholder="Sub-menu URL"></td>
                                    <td>
                                        <select class="form-select">
                                            <?php foreach ($icon_options as $icon): ?>
