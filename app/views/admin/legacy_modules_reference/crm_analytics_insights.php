...
$admin_role = getAuthSubRole() ?? 'admin';
...
function getCustomerAnalytics() {
    ...
    $analytics['acquisition_trends'] = $db->fetchAll($query);
    ...
    $analytics['lifetime_value'] = $db->fetchAll($query);
    ...
    $analytics['segmentation'] = $db->fetchAll($query);
...
function getLeadAnalytics() {
    ...
    $analytics['conversion_funnel'] = $db->fetchAll($query);
    ...
    $analytics['source_performance'] = $db->fetchAll($query);
    ...
    $analytics['response_time'] = $db->fetch($query) ?? [];
...
function getSalesAnalytics() {
    ...
    $analytics['monthly_trends'] = $db->fetchAll($query);
    ...
    $analytics['property_type_sales'] = $db->fetchAll($query);

    // Sales team performance
    $query = "
        SELECT
            u.uname as sales_person,
        ...
        WHERE u.utype IN ('admin', 'sales_manager', 'sales_executive', 'associate', '1', '2', '4')
        GROUP BY u.uid, u.uname
        ORDER BY total_revenue DESC";

    $analytics['sales_team_performance'] = $db->fetchAll($query);
...
function getCommunicationAnalytics() {
    ...
    $analytics['email_campaigns'] = $db->fetchAll($query);
    ...
    $analytics['communication_channels'] = $db->fetchAll($query);
                                ...
                                <div class="insight-card">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-3 mb-3">
                                            <div class="segment-card segment-<?php echo h(strtolower(str_replace(' ', '-', $segment['segment']))); ?>">
                                                <h4><?php echo number_format((int)$segment['customer_count']); ?></h4>
                                                <p class="mb-1"><?php echo h($segment['segment']); ?></p>
                                                <small>Total Value: ₹<?php echo number_format((float)$segment['total_segment_value']/100000, 1); ?>L</small>
                                            </div>
                                        </div>
                                        <?php endforeach; ?>
                                    </div>
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td>
                                                        ...
                                                        <br><small class="text-muted"><?php echo h($customer['email']); ?></small>
                                                    </td>
                                                    <td>₹<?php echo number_format((float)$customer['total_spent']); ?></td>
                                                    <td><?php echo (int)$customer['total_purchases']; ?></td>
                                                    <td>₹<?php echo number_format((float)$customer['avg_order_value']); ?></td>
                                                    <td><?php echo $customer['last_purchase'] ? h(date('M d, Y', strtotime($customer['last_purchase']))) : 'Never'; ?></td>
                                                </tr>
                                                <?php endforeach; ?>
                                            </tbody>
                                        </table>
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="metric-card text-center">
                                    ...
                                    <h4><?php echo number_format((float)($lead_analytics['response_time']['avg_response_time_hours'] ?? 0), 1); ?> hrs</h4>
                            ...
                            <div class="col-md-4">
                                <div class="metric-card text-center">
                                    ...
                                    <h4><?php echo (int)($lead_analytics['response_time']['responded_within_hour'] ?? 0); ?></h4>
                            ...
                            <div class="col-md-4">
                                <div class="metric-card text-center">
                                    ...
                                    <h4><?php echo (int)($lead_analytics['response_time']['total_leads_with_contact'] ?? 0); ?></h4>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-hover">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td><strong><?php echo h($sales_person['sales_person']); ?></strong></td>
                                                    <td><?php echo (int)$sales_person['deals_closed']; ?></td>
                                                    <td>₹<?php echo number_format((float)$sales_person['total_revenue']); ?></td>
                                                    <td>₹<?php echo number_format((float)$sales_person['avg_deal_size']); ?></td>
                                                    <td><?php echo $sales_person['last_sale_date'] ? h(date('M d, Y', strtotime($sales_person['last_sale_date']))) : 'No sales'; ?></td>
                                                </tr>
                                                <?php endforeach; ?>
                                            </tbody>
                                        </table>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-hover">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td><strong><?php echo h($campaign['campaign_name']); ?></strong></td>
                                                    <td><?php echo (int)$campaign['emails_sent']; ?></td>
                                                    <td><?php echo (int)$campaign['emails_opened']; ?></td>
                                                    <td><?php echo (int)$campaign['emails_clicked']; ?></td>
                                                    <td><?php echo (int)$campaign['conversions']; ?></td>
                                                    <td>
                                                        <?php
                                                        $openRate = $campaign['emails_sent'] > 0 ?
                                                            round(((int)$campaign['emails_opened'] / (int)$campaign['emails_sent']) * 100, 1) : 0;
                                                        ?>
                                                        <span class="badge bg-<?php echo $openRate >= 20 ? 'success' : ($openRate >= 10 ? 'warning' : 'danger'); ?>">
                                                            <?php echo number_format($openRate, 1); ?>%
                                                        </span>
                                                    </td>
                                                </tr>
                                                <?php endforeach; ?>
