<?php
$page_title = $page_title ?? 'NOC Detail';
ob_start();
$noc = $noc ?? [];
...
<?php if (empty($noc)): ?>
    <div class="alert alert-danger">NOC not found.</div>
<?php else: ?>
    <div class="row g-4">
        <!-- NOC Info -->
        <div class="col-md-8">
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-header bg-white border-bottom">
                    <div class="d-flex justify-content-between align-items-center">
                        <h6 class="mb-0">NOC Information</h6>
                        <?php
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-6">
                            <div class="text-muted small">NOC Number</div>
        ...
        <div class="col-md-4">
            ...
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-header bg-white border-bottom">
                    <h6 class="mb-0"><i class="fas fa-check-double me-2"></i>Eligibility Checks</h6>
                </div>
                <div class="card-body">
                    <?php if (!empty($eligibility['checks'])): ?>
                        ...
                        <div class="small">
                            <?php if ($eligibility['eligible']): ?>
                                <span class="text-success fw-bold"><i class="fas fa-check me-1"></i>Eligible for NOC</span>
                            <?php else: ?>
                                <span class="text-danger fw-bold"><i class="fas fa-times me-1"></i>Not eligible — <?= $eligibility['fail_count'] ?> check(s) failed</span>
                            <?php endif; ?>
                        </div>
                    <?php else: ?>
                        <div class="text-muted small">No checks available</div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body">
                    <form method="POST" action="<?= BASE_URL ?>/admin/noc-registry/nocs/approve" class="mb-2">
                        ...
                        <button type="submit" class="btn btn-success w-100"><i class="fas fa-check me-1"></i>Approve NOC</button>
                    </form>
                    ...
                    <form method="POST" action="<?= BASE_URL ?>/admin/noc-registry/nocs/reject">
                        ...
                        <button type="submit" class="btn btn-danger w-100"><i class="fas fa-times me-1"></i>Reject NOC</button>
                    </form>
                    ...
                    <form method="POST" action="<?= BASE_URL ?>/admin/noc-registry/nocs/reprocess" class="mt-2">
                        <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?? '' ?>">
                        <input type="hidden" name="noc_id" value="<?= $noc['id'] ?>">
                        <button type="submit" class="btn btn-info w-100"><i class="fas fa-redo me-1"></i>Reprocess</button>
                    </form>
                    <?php endif; ?>
                </div>
            </div>
