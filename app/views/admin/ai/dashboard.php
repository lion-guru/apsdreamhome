    ...
    <div class="container-fluid px-4 style-84072">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
                ...
                <p class="mb-0 opacity-75 style-42715">5 autonomous agents — real estate intelligence engine</p>
            </div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-6 col-md-3">
            <div class="ai-stat-card">
                <div class="stat-icon style-29065"><i class="fas fa-bolt"></i></div>
                <div class="stat-value style-58842"><?= number_format($gateway_stats['total_calls'] ?? 0) ?></div>
                <div class="stat-label">AI Calls Today</div>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="ai-stat-card">
                <div class="stat-icon style-82860"><i class="fas fa-check-circle"></i></div>
                <div class="stat-value style-2154"><?= $gateway_stats['avg_confidence'] ?? '0.00' ?></div>
                <div class="stat-label">Avg Confidence</div>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="ai-stat-card">
                <div class="stat-icon style-64138"><i class="fas fa-clock"></i></div>
                <div class="stat-value style-62735"><?= $gateway_stats['avg_response_ms'] ?? '0' ?>ms</div>
                <div class="stat-label">Avg Response</div>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="ai-stat-card">
                <div class="stat-icon style-10079"><i class="fas fa-database"></i></div>
                <div class="stat-value style-78822"><?= $health['unqualified_leads'] ?? 0 ?></div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-8">
            <div class="style-9697">
                ...
                ?>
                <div class="d-flex gap-3 mb-3 style-44570">
                    <?php foreach ($engines as $e): ?>
                        <div class="gateway-bar flex-grow-1" title="<?= $e['name'] ?>: <?= $e['count'] ?>">
                            <div class="gateway-bar-fill style-40249"></div>
                        </div>
                    <?php endforeach; ?>
                </div>
                <div class="d-flex gap-3 flex-wrap style-86354">
                    <?php foreach ($engines as $e): ?>
            ...
            <div class="style-46899">
                ...
                <div class="d-flex flex-column gap-3">
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        <span class="badge <?= $health['gemini_api'] === 'connected' ? 'bg-success' : 'bg-secondary' ?> style-26285"><?= $health['gemini_api'] === 'connected' ? 'Connected' : 'Not Configured' ?></span>
                    </div>
                    <div class="d-flex align-items-center justify-content-between">
                        <span class="style-87981"><span class="health-dot green"></span>Intent Patterns</span>
                        <span class="badge bg-primary style-26285"><?= $health['intent_patterns'] ?? 0 ?> active</span>
                    </div>
                    <div class="d-flex align-items-center justify-content-between">
                        <span class="style-87981"><span class="health-dot green"></span>Leads Today</span>
                        <span class="badge bg-info style-26285"><?= $health['leads_today'] ?? 0 ?></span>
                    </div>
                    <div class="d-flex align-items-center justify-content-between">
                        <span class="style-87981"><span class="health-dot yellow"></span>Scheduled Visits</span>
                        <span class="badge bg-warning style-26285"><?= $health['scheduled_visits'] ?? 0 ?></span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-12">
            <div class="style-39877">
                <div class="section-title style-61561"><i class="fas fa-bolt style-82740"></i> Free AI Engines (Cost: ₹0)
                    <button id="engineHealthBtn" type="button" class="btn btn-sm style-5967"><i class="fas fa-heartbeat"></i> Live Test</button>
                </div>
        ...
        ?>
            <div class="col-md-4 col-lg-2_4">
                <div class="agent-card">
                    <div class="agent-icon style-91959">
                        <i class="<?= $agent['icon'] ?>"></i>
                    </div>
                    <div class="agent-name"><?= $agent['name'] ?></div>
                    <div class="d-flex justify-content-between mb-3 style-89717">
                        <span><?= $stat['completed'] ?? 0 ?> runs today</span>
                        <?php if ($stat['last_run']): ?>
                            <span class="pulse-dot style-2154" title="Last: <?= $stat['last_run'] ?>">â—�</span>
                        <?php endif; ?>
                    </div>
                    <div class="d-flex gap-2">
                        <form method="POST" action="<?= BASE_URL ?>/admin/ai-system/run" class="style-47240">
                            ...
                            <input type="hidden" name="action" value="batch">
                            <button type="submit" class="agent-run-btn w-100 style-82308">Run</button>
                        </form>
                        <?php if ($key === 'qualifier'): ?>
                            <a href="<?= BASE_URL ?>/admin/ai-system/qualifier" class="agent-run-btn style-74107"><i class="fas fa-eye"></i></a>
                        <?php elseif ($key === 'market'): ?>
                            <a href="<?= BASE_URL ?>/admin/ai-system/market-report" class="agent-run-btn style-74107"><i class="fas fa-eye"></i></a>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="style-9697">
                ...
                <?php if (empty($recent_activity)): ?>
                    <div class="text-center py-4 style-1686">
                        <i class="fas fa-robot style-82058"></i>
                ...
                <?php else: ?>
                    ...
                    ?>
                        <div class="activity-item">
                            <div class="activity-dot style-96004"></div>
    ...
    btn.addEventListener('click', function () {
        ...
        fetch('<?= BASE_URL ?>/admin/ai-system/health')
            ...
            .catch(err => console.error('Request failed:', err));
                ...
                html += '<div><span class="health-dot style-87244"></span>'
