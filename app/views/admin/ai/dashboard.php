    ...
    <div class="container-fluid px-4">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
                ...
                <p class="mb-0 opacity-75">5 autonomous agents — real estate intelligence engine</p>
            </div>
...
<div class="container-fluid px-4">
    ...
    <div class="row g-3 mb-4">
        <div class="col-6 col-md-3">
            <div class="ai-stat-card">
                ...
                <div class="stat-value"><?= number_format($gateway_stats['total_calls'] ?? 0) ?></div>
        ...
        <div class="col-6 col-md-3">
            <div class="ai-stat-card">
                ...
                <div class="stat-value"><?= $gateway_stats['avg_confidence'] ?? '0.00' ?></div>
        ...
        <div class="col-6 col-md-3">
            <div class="ai-stat-card">
                ...
                <div class="stat-value"><?= $gateway_stats['avg_response_ms'] ?? '0' ?>ms</div>
        ...
        <div class="col-6 col-md-3">
            <div class="ai-stat-card">
                ...
                <div class="stat-value"><?= $health['unqualified_leads'] ?? 0 ?></div>
                ...
                <div class="d-flex gap-3 mb-3">
                    <?php foreach ($engines as $e): ?>
                        <div class="gateway-bar flex-grow-1" title="<?= $e['name'] ?>: <?= $e['count'] ?>">
                            <div class="gateway-bar-fill"></div>
                        </div>
                    <?php endforeach; ?>
                </div>
                <div class="d-flex gap-3 flex-wrap">
                    <?php foreach ($engines as $e): ?>
                        <span><span ></span><?= $e['name'] ?>: <?= $e['count'] ?></span>
                    <?php endforeach; ?>
                </div>
            </div>
        </div>
            ...
            <div >
                ...
                <div class="d-flex flex-column gap-3">
                    <div class="d-flex align-items-center justify-content-between">
                        <span ><span class="health-dot <?= $health['gemini_api'] === 'connected' ? 'green' : 'red' ?>"></span>Gemini API</span>
                        <span class="badge <?= $health['gemini_api'] === 'connected' ? 'bg-success' : 'bg-secondary' ?>"><?= $health['gemini_api'] === 'connected' ? 'Connected' : 'Not Configured' ?></span>
                    </div>
                    <div class="d-flex align-items-center justify-content-between">
                        <span ><span class="health-dot green"></span>Intent Patterns</span>
                        <span class="badge bg-primary"><?= $health['intent_patterns'] ?? 0 ?> active</span>
                    </div>
                    <div class="d-flex align-items-center justify-content-between">
                        <span ><span class="health-dot green"></span>Leads Today</span>
                        <span class="badge bg-info"><?= $health['leads_today'] ?? 0 ?></span>
                    </div>
                    <div class="d-flex align-items-center justify-content-between">
                        <span ><span class="health-dot yellow"></span>Scheduled Visits</span>
                        <span class="badge bg-warning"><?= $health['scheduled_visits'] ?? 0 ?></span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-12">
            <div >
                <div class="section-title"><i class="fas fa-bolt"></i> Free AI Engines (Cost: ₹0)
                    <button id="engineHealthBtn" type="button" class="btn btn-sm"><i class="fas fa-heartbeat"></i> Live Test</button>
                </div>
                <div id="engineHealthResult" ></div>
                <div class="row g-3">
                ...
                <div class="agent-card">
                    ...
                    <div class="d-flex justify-content-between mb-3">
                        ...
                        <?php if ($stat['last_run']): ?>
                            <span class="pulse-dot" title="Last: <?= $stat['last_run'] ?>">â—�</span>
                        <?php endif; ?>
                    </div>
                    <div class="d-flex gap-2">
                        <form method="POST" action="<?= BASE_URL ?>/admin/ai-system/run" >
                            ...
                            <input type="hidden" name="action" value="batch">
                            <button type="submit" class="agent-run-btn w-100">Run</button>
                        </form>
                        <?php if ($key === 'qualifier'): ?>
                            <a href="<?= BASE_URL ?>/admin/ai-system/qualifier" class="agent-run-btn"><i class="fas fa-eye"></i></a>
                        <?php elseif ($key === 'market'): ?>
                            <a href="<?= BASE_URL ?>/admin/ai-system/market-report" class="agent-run-btn"><i class="fas fa-eye"></i></a>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
                    ...
                    ?>
                        <div class="activity-item">
                            ...
                            <div class="flex-grow-1">
                                <div ><?= htmlspecialchars($a['agent_type'] ?? '') ?></div>
                                <div ><?= htmlspecialchars($a['details'] ?? $a['action_type'] ?? '') ?></div>
                            </div>
                            <div ><?= date('h:i A', strtotime($a['created_at'] ?? '')) ?></div>
                        </div>
                    <?php endforeach; ?>
                <?php endif; ?>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    btn.addEventListener('click', function () {
        ...
        out.innerHTML = '<span >Pinging AI engines...</span>';
        ...
        fetch('<?= BASE_URL ?>/admin/ai-system/health')
            ...
            .catch(err => console.error('Request failed:', err));
                ...
                var html = '<div >';
                html += '<div><span class="health-dot"></span>'
                ...
                if (s.ollama && s.ollama.test_reply) {
                    html += '<div >test reply: &ldquo;' + s.ollama.test_reply + '&rdquo;</div>';
                }
                html += '<div >Primary engine: <strong>' + (s.primary_engine || 'unknown') + '</strong></div>';
                html += '<div >Gemini: ' + (s.gemini && s.gemini.configured ? 'configured' : 'not set')
                ...
                html += '<div >Checked at ' + (s.checked_at || '') + '</div>';
                html += '</div>';
                out.innerHTML = html;
            })
            .catch(function (e) {
                out.innerHTML = '<span >Health check failed: ' + e + '</span>';
            })
            .finally(function () {
                btn.disabled = false;
