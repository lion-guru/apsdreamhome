...
if (!isAuthenticated() || (getAuthRole() !== 'customer' && getAuthRole() !== 'admin' && getAuthRole() !== 'superadmin')) {
...
// Get customer info
$customer_id = getAuthUserId() ?? 0;
$customer_name = h(getAuthFullName() ?? 'Customer');
...
try {
    ...
    $stats_query = "
        SELECT
            (SELECT COUNT(*) FROM properties WHERE customer_id = :id AND status = 'active') as active_properties,
            (SELECT COUNT(*) FROM property_inquiries WHERE customer_id = :id) as total_inquiries,
            (SELECT COUNT(*) FROM documents WHERE customer_id = :id) as total_documents,
            (SELECT COUNT(*) FROM payments WHERE customer_id = :id AND status = 'completed') as completed_payments,
            (SELECT SUM(amount) FROM payments WHERE customer_id = :id AND status = 'completed') as total_spent,
            (SELECT COUNT(*) FROM properties WHERE customer_id = :id AND status = 'pending') as pending_approvals,
            (SELECT COUNT(*) FROM notifications WHERE user_id = :id AND is_read = 0) as unread_notifications
    ";

    $stats_result = $db->fetchOne($stats_query, ['id' => $customer_id]);
...
// ENHANCED: Add missing features from customer_public_dashboard.php

// Handle form submissions (from customer_public_dashboard.php)
$message = '';
$error = '';
    ...
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        switch ($_POST['action'] ?? '') {
            case 'update_profile':
                try {
                    $db->execute(
                        "UPDATE customers SET name = :name, email = :email, mobile = :mobile, address = :address WHERE id = :id",
                        [
                            'name' => $_POST['name'],
                            'email' => $_POST['email'],
                            'mobile' => $_POST['mobile'],
            ...
            case 'submit_inquiry':
                try {
                    $db->execute(
                        "INSERT INTO customer_inquiries (customer_id, subject, message, inquiry_type, created_at) VALUES (:customer_id, :subject, :message, :inquiry_type, NOW())",
                        [
                            'customer_id' => $customer_id,
                            'subject' => $_POST['subject'],
                            'message' => $_POST['inquiry_message'],
...
// ENHANCED: Fetch customer data (from customer_public_dashboard.php)
$customer = $db->fetchOne("SELECT * FROM customers WHERE id = :id", ['id' => $customer_id]);

// ENHANCED: Fetch customer's bookings (from customer_public_dashboard.php)
$bookings = $db->fetchAll(
    "SELECT b.*, p.property_name, p.property_type FROM bookings b LEFT JOIN properties p ON b.property_id = p.id WHERE b.customer_id = :id ORDER BY b.created_at DESC",
    ['id' => $customer_id]
);

// ENHANCED: Fetch customer's payments (from customer_public_dashboard.php)
$payments = $db->fetchAll(
    "SELECT * FROM payments WHERE customer_id = :id ORDER BY payment_date DESC",
    ['id' => $customer_id]
);

// ENHANCED: Fetch customer's documents (from customer_public_dashboard.php)
$documents = $db->fetchAll(
    "SELECT * FROM customer_documents WHERE customer_id = :id ORDER BY uploaded_at DESC",
    ['id' => $customer_id]
);

// ENHANCED: Fetch EMI details (from customer_public_dashboard.php)
$emi_schedule = $db->fetchAll(
    "SELECT * FROM emi_schedule WHERE customer_id = :id ORDER BY due_date ASC",
    ['id' => $customer_id]
);

// ENHANCED: Fetch customer inquiries (from customer_public_dashboard.php)
$inquiries = $db->fetchAll(
    "SELECT * FROM customer_inquiries WHERE customer_id = :id ORDER BY created_at DESC LIMIT 10",
    ['id' => $customer_id]
);
...
if (!empty($customer['address'])) $filled_fields++;
if (!empty($customer['city'])) $filled_fields++;
if (!empty($customer['state'])) $filled_fields++;
if (!empty($customer['pincode'])) $filled_fields++;
if (!empty($customer['pan'])) $filled_fields++;
...
<head>
    ...
    <link rel="stylesheet" href="<?php echo rtrim($base_url, '/'); ?>/admin/assets/css/bootstrap.min.css">
    <link rel="stylesheet" href="<?php echo rtrim($base_url, '/'); ?>/admin/assets/css/font-awesome.min.css">
...
<body>
    ...
    <div class="dashboard-container">
        ...
        <div class="welcome-section fade-in">
            <div class="welcome-header">
                <div class="greeting">
                    <h1>Welcome back, <?php echo h($customer_name); ?>! 👋</h1>
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card fade-in" style="animation-delay: 0.1s;">
                ...
                <div class="stat-value"><?php echo h($dashboard_stats['active_properties']); ?></div>
            ...
            <div class="stat-card fade-in" style="animation-delay: 0.2s;">
                ...
                <div class="stat-value"><?php echo h($dashboard_stats['total_inquiries']); ?></div>
            ...
            <div class="stat-card fade-in" style="animation-delay: 0.3s;">
                ...
                <div class="stat-value"><?php echo h($dashboard_stats['total_documents']); ?></div>
            ...
            <div class="stat-card fade-in" style="animation-delay: 0.4s;">
                ...
                <div class="stat-value">₹<?php echo h(number_format($dashboard_stats['total_spent'], 2)); ?></div>
            ...
            <div class="stat-card fade-in" style="animation-delay: 0.5s;">
                <div class="stat-icon approvals">
                    ...
                    <?php if ($dashboard_stats['pending_approvals'] > 0): ?>
                        <div class="notification-badge"><?php echo h($dashboard_stats['pending_approvals']); ?></div>
                    <?php endif; ?>
                </div>
                <div class="stat-value"><?php echo h($dashboard_stats['pending_approvals']); ?></div>
            ...
            <div class="stat-card fade-in" style="animation-delay: 0.6s;">
                <div class="stat-icon notifications">
                    ...
                    <?php if ($dashboard_stats['unread_notifications'] > 0): ?>
                        <div class="notification-badge pulse"><?php echo h($dashboard_stats['unread_notifications']); ?></div>
                    <?php endif; ?>
                </div>
                <div class="stat-value"><?php echo h($dashboard_stats['unread_notifications']); ?></div>
        ...
        <div class="quick-actions fade-in" style="animation-delay: 0.7s;">
            ...
            <div class="action-grid">
                <a href="<?php echo h($base_url); ?>properties.php?action=add" class="action-card">
                ...
                <a href="<?php echo h($base_url); ?>documents.php" class="action-card">
                ...
                <a href="<?php echo h($base_url); ?>payments.php" class="action-card">
                ...
                <a href="<?php echo h($base_url); ?>support.php" class="action-card">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            const greetingText = "Welcome back, <?php echo h($customer_name); ?>! 👋";
