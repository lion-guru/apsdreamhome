...
if (!isAuthenticated() || (getAuthRole() !== 'builder' && getAuthRole() !== 'admin')) {
    header('Location: ../../login.php');
    exit();
}

// Get builder info
$builder_id = getAuthUserId() ?? 0;
$builder_name = h(getAuthFullName() ?? 'Builder');
...
<div class="container-fluid py-4">
    <div class="row mb-4">
        <div class="col-12">
            <h2 class="mb-0">Welcome, <?php echo h($builder_name); ?>!</h2>
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-hover" id="projectsTable">
                ...
                <tbody>
                    ...
                    $rows = $db->fetchAll("SELECT * FROM projects WHERE builder_id = :id", ['id' => $builder_id]);
                    foreach ($rows as $row): ?>
                        <tr>
                            ...
                            <td>
                                <a href="edit_project.php?id=<?php echo h($row['bid']); ?>" class="btn btn-sm btn-info">Edit</a>
                                <a href="delete_project.php?id=<?php echo h($row['bid']); ?>" class="btn btn-sm btn-danger">Delete</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
