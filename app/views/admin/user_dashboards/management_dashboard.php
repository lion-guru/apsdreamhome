...
$status_checks = [
    'AI System' => !empty($config->get('ai.gemini_api_key')) || !empty($config->get('ai.openai_api_key')),
    'WhatsApp Integration' => !empty($config->get('whatsapp.instance_id')),
    'Email System' => !empty($config->get('email.smtp_host')),
...
async function testWhatsApp() {
    ...
    try {
        ...
        const response = await fetch('../api/test_notification.php', {
            ...
            body: JSON.stringify({ type: 'sms', recipient: phone, message })
        });
