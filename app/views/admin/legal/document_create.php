                ...
                <div class="aps-cp-card mb-3">
                    ...
                    <div class="aps-cp-card-body">
                        <div class="row g-3">
                            ...
                            <div class="col-md-4 entity-field" data-type="booking" style="display:none"><label class="form-label">Booking</label><select name="entity_id" class="form-select"><?php foreach ($bookings as $b): ?><option value="<?= $b['id'] ?>"><?= htmlspecialchars($b['booking_number'] ?? '#'.$b['id']) ?> - <?= htmlspecialchars($b['customer_name'] ?? '') ?></option><?php endforeach; ?></select></div>
                            <div class="col-md-4 entity-field" data-type="plot" style="display:none"><label class="form-label">Plot</label><select name="entity_id" class="form-select"><?php foreach ($plots as $p): ?><option value="<?= $p['id'] ?>"><?= htmlspecialchars($p['plot_no'] ?? '') ?> - <?= htmlspecialchars($p['colony_name'] ?? '') ?></option><?php endforeach; ?></select></div>
                            <div class="col-md-4 entity-field" data-type="associate" style="display:none"><label class="form-label">Associate</label><select name="entity_id" class="form-select"><?php foreach ($associates as $a): ?><option value="<?= $a['id'] ?>"><?= htmlspecialchars($a['name'] ?? '') ?> (<?= htmlspecialchars($a['associate_code'] ?? '') ?>)</option><?php endforeach; ?></select></div>
                            <div class="col-md-4 entity-field" data-type="colony" style="display:none"><label class="form-label">Colony</label><select name="entity_id" class="form-select"><?php foreach ($colonies as $co): ?><option value="<?= $co['id'] ?>"><?= htmlspecialchars($co['name'] ?? '') ?></option><?php endforeach; ?></select></div>
                        </div>
                    </div>
                </div>
