...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (isset($_POST['create_plan'])) {
            createCommissionPlan($db, $_POST, $associate_id);
        } elseif (isset($_POST['update_plan'])) {
            updateCommissionPlan($db, $_POST);
        } elseif (isset($_POST['activate_plan'])) {
            activatePlan($db, $_POST['plan_id']);
        } elseif (isset($_POST['deactivate_plan'])) {
            deactivatePlan($db, $_POST['plan_id']);
        } elseif (isset($_POST['delete_plan'])) {
            deletePlan($db, $_POST['plan_id']);
        }
    }
}

// Get all plans
$plans = getAllCommissionPlans($db);

// Get active plan
$active_plan = getActiveCommissionPlan($db);

function createCommissionPlan($db, $data, $associate_id) {
    try {
        $db->beginTransaction();
        ...
        createDefaultLevels($db, $plan_id);

        $db->commit();

        log_admin_activity('Commission Plan Created', "Plan: {$data['plan_name']} (ID: $plan_id)");
...
function updateCommissionPlan($db, $data) {
    try {
        $query = "UPDATE mlm_commission_plans 
                  SET plan_name = ?, plan_code = ?, description = ?, plan_type = ?
                  WHERE id = ?";
        ...
        log_admin_activity('Commission Plan Updated', "Plan ID: {$data['plan_id']}");
...
function createDefaultLevels($db, $plan_id) {
    $default_levels = [
        ['Associate', 5.00, 2.00, 0.00, 0.00, 0.00, 0.00, 100000],
        ['Bronze', 7.00, 3.00, 2.00, 5.00, 0.00, 0.00, 250000],
        ['Silver', 10.00, 4.00, 3.00, 8.00, 1.00, 0.00, 500000],
        ['Gold', 12.00, 5.00, 4.00, 10.00, 2.00, 1.00, 1000000],
        ['Platinum', 15.00, 6.00, 5.00, 12.00, 3.00, 2.00, 2000000],
        ['Diamond', 18.00, 7.00, 6.00, 15.00, 4.00, 3.00, 3500000],
        ['Executive', 20.00, 8.00, 7.00, 18.00, 5.00, 5.00, 5000000],
        ['Senior Executive', 22.00, 9.00, 8.00, 20.00, 6.00, 6.00, 7500000],
        ['Director', 25.00, 10.00, 9.00, 22.00, 7.00, 7.00, 10000000],
        ['Global Director', 30.00, 12.00, 10.00, 25.00, 10.00, 10.00, 999999999]
    ];
...
function getAllCommissionPlans($db) {
    $query = "SELECT p.*, u.uname as created_by_name,
              (SELECT COUNT(*) FROM mlm_plan_levels WHERE plan_id = p.id) as level_count
              FROM mlm_commission_plans p
              LEFT JOIN associates a ON p.created_by = a.id
...
function activatePlan($db, $plan_id) {
    try {
        ...
        $db->execute($query, [$plan_id]);

        $db->commit();

        log_admin_activity('Commission Plan Activated', "Plan ID: $plan_id");
...
function deactivatePlan($db, $plan_id) {
    try {
        $query = "UPDATE mlm_commission_plans SET status = 'inactive', deactivated_at = NOW() WHERE id = ?";
        $db->execute($query, [$plan_id]);

        log_admin_activity('Commission Plan Deactivated', "Plan ID: $plan_id");
...
function deletePlan($db, $plan_id) {
    try {
        ...
        log_admin_activity('Commission Plan Deleted', "Plan ID: $plan_id");
...
<?php
// ... after PHP logic ...
$page_title = "Commission Plan Manager";
                                    ...
                                    <div class="card plan-card h-100">
                                        <div class="card-body">
                                            <div class="d-flex justify-content-between align-items-start mb-3">
                                                <div>
                                                    <h5 class="card-title"><?php echo h($plan['plan_name']); ?></h5>
                                                    <p class="card-text small text-muted">
                                                        Code: <?php echo h($plan['plan_code']); ?>
                                                    </p>
                                                </div>
                                                <span class="status-badge status-<?php echo h($plan['status']); ?>">
                                                    <?php echo ucfirst(h($plan['status'])); ?>
                                                </span>
                                            </div>

                                            <p class="card-text"><?php echo h($plan['description']); ?></p>

                                            <div class="mb-3">
                                                <small class="text-muted">
                                                    <i class="fas fa-calendar me-1"></i>Created: <?php echo date('M d, Y', strtotime($plan['created_at'])); ?><br>
                                                    <i class="fas fa-user me-1"></i>By: <?php echo h($plan['created_by_name']); ?><br>
                                                    <i class="fas fa-layer-group me-1"></i><?php echo (int)$plan['level_count']; ?> levels
                                                </small>
                                            </div>
    ...
    <script>
        ...
        function activatePlan(planId) {
            if (confirm('Are you sure you want to activate this plan? This will deactivate the current active plan.')) {
                ...
                form.innerHTML = `
                    ...
                    <input type="hidden" name="activate_plan" value="1">
        ...
        function deactivatePlan(planId) {
            if (confirm('Are you sure you want to deactivate this plan?')) {
                ...
                form.innerHTML = `
                    <input type="hidden" name="csrf_token" value="${CSRF_TOKEN}">
                    <input type="hidden" name="deactivate_plan" value="1">
                    <input type="hidden" name="plan_id" value="${planId}">
                `;
                document.body.appendChild(form);
                form.submit();
