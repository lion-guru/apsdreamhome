<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0"><?= htmlspecialchars($page_heading ?? 'All Notifications') ?></h1>
        <div>
            <button class="btn btn-outline-secondary btn-sm mark-all-read" data-url="<?= BASE_URL ?>/admin/notifications/mark-all-read">
        ...
        <div class="card-body p-0">
            <?php if (!empty($notifications)): ?>
                <div class="list-group list-group-flush">
                    <?php foreach ($notifications as $n): ?>
                        ...
                        ?>
                        <div class="list-group-item list-group-item-action py-3 px-3 d-flex align-items-start gap-3 <?= empty($n['is_read']) ? 'bg-light' : '' ?>" data-id="<?= $n['id'] ?>">
                            <div class="mt-1">
                                <span class="badge rounded-circle p-2 <?= $badgeClass ?> d-inline-flex align-items-center justify-content-center">
                            ...
                            <div class="flex-grow-1 min-w-0">
                                ...
                                <div class="d-flex gap-2 mt-1">
                                    ...
                                    <?php if (empty($n['is_read'])): ?>
                                        <button class="btn btn-sm btn-outline-secondary px-2 py-0 mark-read" data-id="<?= $n['id'] ?>" data-url="<?= BASE_URL ?>/admin/notifications/mark-read/<?= $n['id'] ?>">
            ...
            <?php else: ?>
                <div class="text-center py-5">
                    ...
                    <p class="text-muted small mb-0">You have no notifications yet.</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
