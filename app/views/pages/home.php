...
<main id="main-content">
    ...
    <script>
        function calcEMI() {
            ...
            document.getElementById('loanAmtDisplay').textContent = 'Rs' + P.toLocaleString('en-IN');
            ...
            if (R === 0) {
                document.getElementById('emiResult').textContent = 'Rs' + Math.round(P / N).toLocaleString('en-IN');
            } else {
                const emi = P * R * Math.pow(1 + R, N) / (Math.pow(1 + R, N) - 1);
                const totalPay = emi * N;
                const totalInt = totalPay - P;
                document.getElementById('emiResult').textContent = 'Rs' + Math.round(emi).toLocaleString('en-IN');
                document.getElementById('totalInterest').textContent = 'Rs' + Math.round(totalInt).toLocaleString('en-IN');
                document.getElementById('totalPayment').textContent = 'Rs' + Math.round(totalPay).toLocaleString('en-IN');
            }
        }
        calcEMI();
    </script>
    ...
    <script>
        ...
        function openServiceModal(service) {
            ...
            svc.features.forEach(function(f) {
                featuresHtml += '<li class="mb-2"><i class="fas fa-check text-' + svc.ctaColor + ' me-2"></i>' + f + '</li>';
            });
            featuresHtml += '</ul>';
            ...
            if (svc.prices) {
                ...
                svc.prices.forEach(function(p) {
                    featuresHtml += '<div class="d-flex justify-content-between mb-1"><span>' + p.tier + '</span><span class="fw-bold text-warning">' + p.price + '</span></div>';
                });
                featuresHtml += '</div>';
            }
            featuresHtml += '</div>';
            ...
            if (svc.ctaBtn) {
                featuresHtml += '<a href="#" class="btn btn-' + svc.ctaColor + '" onclick="' + svc.ctaAction + ';return false;"><i class="fas fa-calculator me-2"></i>' + svc.ctaBtn + '</a>';
            }
            if (svc.phone) {
                featuresHtml += '<hr class="my-3"><p class="small text-muted mb-1">\uD83D\uDCDE Call: ' + svc.phone + '</p>';
            }
            if (svc.whatsapp) {
                featuresHtml += '<p class="small text-muted mb-0">\uD83D\uDCAC <a href="' + svc.whatsapp + '" target="_blank">WhatsApp</a></p>';
            }
                        ...
                        <div class="col-lg-7">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="fw-bold h4 mb-0 pt-1" id="growthResult">Rs40,45,558</div>
                                </div>
                            </div>
                            <div class="mt-3">
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    <span class="small text-white-50"><?= __('home_re_cagr') ?></span>
                                    <span class="small fw-bold text-success" id="reValue">Rs52,33,855</span>
                                </div>
                                <div class="progress mb-2" style="height: 6px;">
                                ...
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    ...
                                    <span class="small fw-bold text-warning" id="fdValue">Rs17,90,848</span>
                                </div>
                                <div class="progress mb-2" style="height: 6px;">
                                ...
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    ...
                                    <span class="small fw-bold text-primary" id="goldValue">Rs23,67,364</span>
                                </div>
    ...
    <script>
        function calcGrowth() {
            ...
            document.getElementById('reValue').textContent = 'Rs' + Math.round(re).toLocaleString('en-IN');
            document.getElementById('fdValue').textContent = 'Rs' + Math.round(fd).toLocaleString('en-IN');
            document.getElementById('goldValue').textContent = 'Rs' + Math.round(gold).toLocaleString('en-IN');
            document.getElementById('growthResult').textContent = 'Rs' + Math.round(re).toLocaleString('en-IN');
