<?php
$page_title = $page_title ?? __('auction_page_title', [], 'Property Auctions');
$page_heading = $page_heading ?? __('auction_page_heading', [], 'Property Auctions');
$content = $content ?? '';
$live = $live ?? [];
$upcoming = $upcoming ?? [];
$closed = $closed ?? [];
...
<div class="container py-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="mb-1"><?= __('auction_page_heading', [], 'Property Auctions') ?></h1>
            <p class="text-muted mb-0"><?= __('auction_hero_subtitle', [], 'Bid on exclusive properties in real-time') ?></p>
        </div>
    </div>

    <h3 class="mb-3"><i class="fas fa-bolt text-warning me-2"></i><?= __('auction_live_now', [], 'Live Now') ?></h3>
    <?php if (empty($live)): ?>
        <div class="alert alert-info"><?= __('auction_no_live', [], 'No live auctions right now. Check back soon!') ?></div>
    <?php else: ?>
                        ...
                        <div class="card-body aps-cp-card-body">
                            ...
                            <div class="d-flex justify-content-between my-3">
                                <div>
                                    ...
                                    <strong class="text-success">â‚¹<?= number_format($a['current_bid'] ?? $a['start_price']) ?></strong>
                                </div>
                                <div>
    ...
    <h3 class="mb-3"><i class="fas fa-calendar text-info me-2"></i><?= __('auction_upcoming', [], 'Upcoming Auctions') ?></h3>
    <?php if (empty($upcoming)): ?>
        <p class="text-muted"><?= __('auction_no_upcoming', [], 'No upcoming auctions scheduled.') ?></p>
    <?php else: ?>
        <div class="row g-3 mb-5">
                ...
                <div class="col-md-4">
                    <div class="card border-0 shadow-sm h-100">
                        <div class="card-body aps-cp-card-body">
                            ...
                            <p class="mb-2"><strong><?= __('auction_start_price', [], 'Start:') ?> â‚¹<?= number_format($a['start_price']) ?></strong></p>
    ...
    <h3 class="mb-3"><i class="fas fa-trophy text-warning me-2"></i><?= __('auction_recent_winners', [], 'Recent Winners') ?></h3>
    <?php if (empty($closed)): ?>
        <p class="text-muted"><?= __('auction_no_closed', [], 'No closed auctions yet.') ?></p>
    <?php else: ?>
        <div class="row g-3">
                ...
                <div class="col-md-4">
                    <div class="card border-0 shadow-sm h-100">
                        <div class="card-body aps-cp-card-body">
                            ...
                            <h6><?= htmlspecialchars($a['title']) ?></h6>
                            <p class="text-success mb-0"><strong><?= __('auction_final_price', [], 'Final:') ?> â‚¹<?= number_format($a['winning_bid'] ?? 0) ?></strong></p>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
