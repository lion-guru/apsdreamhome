<?php
$page_title = $page_title ?? 'Booking Detail';
...
$docTypeLabels = [
    ...
    'noc' => 'NOC',
    'receipt' => 'Receipt',
    'registry_deed' => 'Registry Deed',
    'mutation_letter' => 'Mutation Letter',
    'other' => 'Other',
];
?>
...
<div class="aps-cp-hero">
    <div class="row align-items-center">
        <div class="col-md-8">
            <h2><i class="fas fa-file-invoice-dollar me-2"></i>Booking Detail</h2>
            <?php if ($booking): ?>
                <p>Booking <?= htmlspecialchars($booking['booking_number'] ?? '') ?> &mdash; <?= htmlspecialchars($booking['colony_name'] ?? '') ?></p>
            <?php else: ?>
...
<div class="row g-4 mb-4">
    <div class="col-lg-4">
        <div class="aps-cp-card">
            ...
            <div class="aps-cp-card-body">
                ...
                <div class="mb-2">
                    <span class="badge bg-info"><i class="fas fa-star me-1"></i> Corner Plot</span>
                </div>
                <?php endif; ?>
                <?php if (!empty($booking['road_width_ft']) && $booking['road_width_ft'] > 0): ?>
                <div class="mb-2">
                    <small class="text-muted d-block">Road Width</small>
    ...
    <div class="col-lg-8">
        <div class="aps-cp-card mb-4">
            <div class="aps-cp-card-header">
                <h5><i class="fas fa-info-circle text-primary"></i> Booking Info</h5>
            </div>
            <div class="aps-cp-card-body">
                <div class="row g-3">
                    <div class="col-sm-6">
                ...
                <div class="mt-3">
                    ...
                    <div class="d-flex justify-content-between mt-1">
                        <small class="text-success">Paid: ₹<?= number_format($total_paid) ?></small>
                        <?php if ($pendingAmt > 0): ?>
                            <small class="text-danger">Pending: ₹<?= number_format($pendingAmt) ?></small>
                        <?php else: ?>
                            <small class="text-success"><i class="fas fa-check-circle"></i> Fully Paid</small>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div class="aps-cp-card mb-4">
            <div class="aps-cp-card-header">
                <h5><i class="fas fa-history text-info"></i> Status Timeline</h5>
            </div>
            <div class="aps-cp-card-body">
                    ...
                    <div class="position-relative mb-3">
                        ...
                        <div>
                            ...
                            <?php if (!empty($h['changed_by_name'])): ?>
                                <small class="text-muted ms-2">by <?= htmlspecialchars($h['changed_by_name']) ?></small>
                            <?php endif; ?>
...
<div class="aps-cp-card mb-4">
    <div class="aps-cp-card-header">
        <h5><i class="fas fa-calendar-alt text-warning"></i> Payment Schedule (<?= count($payments) ?> installments)</h5>
    </div>
                ...
                <tbody>
                    ...
                    <tr class="<?= $isOverdue ? 'table-danger' : '' ?>">
                        ...
                        <td>
                            ...
                            <?php if ($isOverdue): ?>
                                <br><small class="text-danger"><i class="fas fa-exclamation-triangle"></i> <?= round((strtotime('now') - strtotime($p['due_date'])) / 86400) ?> days overdue</small>
                            <?php endif; ?>
                        </td>
...
<div class="aps-cp-card mb-4">
    <div class="aps-cp-card-header">
        <h5><i class="fas fa-receipt text-success"></i> Payment Receipts (<?= count($receipts) ?>)</h5>
    </div>
    <div class="aps-cp-card-body p-0">
        <div class="table-responsive">
            <table class="aps-cp-table">
...
<div class="aps-cp-card mb-4">
    <div class="aps-cp-card-header">
        <h5><i class="fas fa-folder-open text-info"></i> Documents (<?= count($documents) ?>)</h5>
    </div>
    <div class="aps-cp-card-body p-0">
        <div class="table-responsive">
            <table class="aps-cp-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><strong><i class="fas fa-file-alt me-2"></i><?= htmlspecialchars($doc['document_name'] ?? 'Document') ?></strong></td>
