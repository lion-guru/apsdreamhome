...
<div class="container py-5">
  <?php if (empty($property)): ?>
    <div class="alert alert-warning text-center">
      ...
      <a href="<?= BASE_URL ?>/resell" class="btn btn-primary mt-3">Back to listings</a>
    </div>
  <?php else: ?>
    <nav aria-label="breadcrumb" class="mb-3">
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="<?= BASE_URL ?>">Home</a></li>
        <li class="breadcrumb-item"><a href="<?= BASE_URL ?>/resell">Resell Properties</a></li>
          ...
          <div class="card-body">
            ...
            <div class="row g-3 mb-4">
              ...
              <?php if (!empty($property['bedrooms'])): ?>
                <div class="col-md-3"><div class="border rounded p-3 text-center"><i class="fas fa-bed fa-2x text-muted"></i><div class="mt-2 small text-muted">Bedrooms</div><strong><?= htmlspecialchars($property['bedrooms']) ?></strong></div></div>
              <?php endif; ?>
              <?php if (!empty($property['bathrooms'])): ?>
                <div class="col-md-3"><div class="border rounded p-3 text-center"><i class="fas fa-bath fa-2x text-muted"></i><div class="mt-2 small text-muted">Bathrooms</div><strong><?= htmlspecialchars($property['bathrooms']) ?></strong></div></div>
              <?php endif; ?>
              <?php if (!empty($property['age_years'])): ?>
                <div class="col-md-3"><div class="border rounded p-3 text-center"><i class="fas fa-calendar fa-2x text-muted"></i><div class="mt-2 small text-muted">Age</div><strong><?= htmlspecialchars($property['age_years']) ?> years</strong></div></div>
              <?php endif; ?>
            </div>
            ...
            <?php if (!empty($property['images'])): ?>
              ...
              <div class="row g-2">
                <?php foreach ($property['images'] as $img): ?>
                  <div class="col-md-4"><img src="<?= htmlspecialchars($img['image_url']) ?>" class="img-fluid rounded" alt=""></div>
                <?php endforeach; ?>
              </div>
            <?php endif; ?>
          </div>
      ...
      <div class="col-md-4">
        <div class="card shadow-sm mb-3">
          <div class="card-body">
            ...
            <?php endif; ?>
            <hr>
            <a href="<?= BASE_URL ?>/contact?property_id=<?= (int)$property['id'] ?>" class="btn btn-primary w-100 mb-2"><i class="fas fa-envelope me-1"></i> Send Inquiry</a>
            <a href="<?= BASE_URL ?>/schedule-visit?property_id=<?= (int)$property['id'] ?>" class="btn btn-outline-primary w-100"><i class="fas fa-calendar me-1"></i> Schedule Visit</a>
          </div>
        </div>
