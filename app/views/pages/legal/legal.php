...
<section class="legal-hero-section section-padding bg-primary text-white text-center rounded-bottom-4 py-5" data-aos="fade-down">
    <div class="container py-4">
        <h1 class="display-5 fw-bold mb-2"><?php echo __('legal_docs_heading', [], 'Legal Documents'); ?></h1>
        <p class="lead mb-0"><?php echo __('legal_docs_subtitle', [], 'Transparency and trust are our foundations. View our official certifications and legal papers.'); ?></p>
    </div>
</section>
            ...
            <div class="col-lg-4 col-md-6" data-aos="fade-up" data-aos-delay="<?= ($index + 1) * 100 ?>">
                <div class="legal-card p-4 rounded-4 shadow-sm h-100 border text-center transition-hover">
                    ...
                    <a href="<?= get_asset_url('legal/' . $doc->file_path, 'docs') ?>" class="btn btn-outline-<?= $color ?> rounded-pill px-4" target="_blank">
                        <i class="fas fa-download me-2"></i><?php echo __('download_pdf', [], 'Download PDF'); ?>
                    </a>
                </div>
            </div>
            <?php endforeach; ?>
            ...
            <div class="col-lg-12">
                <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                    <div class="card-header bg-light border-0 py-3 px-4">
                        <h5 class="mb-0 fw-bold"><?php echo __('additional_legal_papers', [], 'Additional Legal Papers'); ?></h5>
                    </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover align-middle mb-0">
                                <thead class="table-light">
                                    <tr>
                                        <th class="ps-4"><?php echo __('doc_name', [], 'Document Name'); ?></th>
                                        <th><?php echo __('category', [], 'Category'); ?></th>
                                        <th><?php echo __('date_published', [], 'Date Published'); ?></th>
                                        <th class="text-end pe-4"><?php echo __('action', [], 'Action'); ?></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <?php else: ?>
                                        <tr>
                                            <td colspan="4" class="text-center py-4 text-muted"><?php echo __('no_additional_docs', [], 'No additional documents available.'); ?></td>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
...
<style>
    .legal-hero-section { background: linear-gradient(135deg, #0d6efd 0%, #0a58ca 100%) !important; }
    .legal-card { transition: all 0.3s ease; background-color: #fff; }
    .legal-card:hover { transform: translateY(-5px); box-shadow: 0 1rem 3rem rgba(0,0,0,.175) !important; }
    .bg-primary-soft { background-color: rgba(13, 110, 253, 0.1); }
    .bg-success-soft { background-color: rgba(25, 135, 84, 0.1); }
    .bg-info-soft { background-color: rgba(13, 202, 240, 0.1); }
    .icon-box { width: 70px; height: 70px; border-radius: 50%; display: flex; align-items: center; justify-content: center; }
</style>
