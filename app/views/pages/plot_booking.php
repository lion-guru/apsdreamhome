...
<div class="container py-4">
    <nav aria-label="breadcrumb" class="mb-3">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Book Plot <?= htmlspecialchars($plot['plot_number'] ?? '') ?></li>
        </ol>
    </nav>

    <h2 class="fw-bold mb-4"><i class="fas fa-file-contract"></i> Book a Plot</h2>

    <div class="row">
        <!-- Left: Booking Form -->
        <div class="col-lg-7">
            <div class="booking-form-section">
                <h5 class="fw-bold mb-3"><i class="fas fa-user"></i> Your Information</h5>
                <div class="row mb-3">
                    <div class="col-md-6"><strong>Name:</strong> <?= htmlspecialchars($user['name'] ?? '') ?></div>
                    <div class="col-md-6"><strong>Email:</strong> <?= htmlspecialchars($user['email'] ?? '') ?></div>
                    <div class="col-md-6 mt-2"><strong>Phone:</strong> <?= htmlspecialchars($user['phone'] ?? $user['mobile'] ?? '') ?></div>
                </div>
            </div>
                ...
                <form method="POST" action="<?= BASE_URL ?>/plot/book">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="booking_type">
                            <option value="site_visit">I want to visit the site first</option>
                            <option value="online_consultation">Online consultation / Direct booking</option>
                        </select>
                    </div>

                    <?php if (!empty($plot['negotiated_price']) && $plot['negotiated_price'] != $plot['total_price']): ?>
                    <div class="alert alert-success">
                        <i class="fas fa-tag"></i> <strong>Special Deal Price Available!</strong>
                        This plot has a special negotiated price of <strong>₹<?= number_format(intval($plot['negotiated_price'])) ?></strong>
                        (Original: ₹<?= number_format(intval($plot['total_price'])) ?>)
                        <input type="hidden" name="negotiated_price" value="<?= $plot['negotiated_price'] ?>">
                    </div>
                    <?php endif; ?>

                    <div class="mb-3">
                        <label class="form-label">Notes / Special Requests (optional)</label>
                    ...
                    <div class="emi-note mb-3">
                        <i class="fas fa-info-circle"></i> <strong>Payment Terms:</strong> 25% token amount (₹<?= number_format(intval($plot['total_price'] * 0.25)) ?>) required within 15 days of booking confirmation. Balance as per agreed schedule.
                    </div>
        ...
        <div class="col-lg-5">
            <div class="plot-summary-card">
                ...
                <p class="detail mb-3">
                    ...
                    <?= !empty($plot['block']) ? '| Block ' . htmlspecialchars($plot['block']) : '' ?>
                </p>
                <hr class="border-light">
                <div class="row detail">
                    <div class="col-6">₹<?= number_format(floatval($plot['price_per_sqft'] ?? 0)) ?>/sqft</div>
                    <div class="col-6">
                        <?php if ($plot['corner_plot'] ?? false): ?>Corner Plot<?php endif; ?>
                        <?php if ($plot['park_facing'] ?? false): ?><?= ($plot['corner_plot'] ?? false) ? ' | ' : '' ?>Park Facing<?php endif; ?>
                    </div>
                </div>
            ...
            <div class="booking-form-section mt-3">
                <h6 class="fw-bold"><i class="fas fa-history"></i> Your Recent Bookings</h6>
