            ...
            <div class="card border-0 shadow-sm mb-3">
                ...
                <div class="card-body aps-cp-card-body">
                    <div class="table-responsive"><table class="table table-sm table-borderless table-responsive">
                        <tr><td ><strong>Level</strong></td><td><span class="badge bg-<?= ($log['level'] ?? 'info') === 'error' ? 'danger' : (($log['level'] ?? 'info') === 'warning' ? 'warning' : 'info') ?>"><?= htmlspecialchars($log['level'] ?? 'info') ?></span></td></tr>
                        ...
                        <tr><td><strong>User Agent</strong></td><td class="text-truncate"><?= htmlspecialchars($log['user_agent'] ?? '-') ?></td></tr>
        ...
        <div class="col-lg-4">
            <div class="card border-0 shadow-sm mb-3">
                ...
                <div class="card-body p-0">
                    <pre class="mb-0 p-3"><?= htmlspecialchars($log['trace'] ?? 'No stack trace available.') ?></pre>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body p-0">
                    <pre class="mb-0 p-3"><?= htmlspecialchars(json_encode($log['context'], JSON_PRETTY_PRINT) ?: '{}') ?></pre>
                </div>
            </div>
            <?php endif; ?>
        </div>
