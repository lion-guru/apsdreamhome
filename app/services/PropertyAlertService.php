...
class PropertyAlertService
{
    ...
    public function subscribe(array $data): int
    {
        ...
        $stmt = $this->pdo->prepare("INSERT INTO property_alert_subscriptions
            (user_id, email, phone, name, property_type, listing_type, city, state, min_price, max_price, min_area_sqft, max_area_sqft, bedrooms, notify_email, notify_sms, notify_whatsapp, frequency, unsubscribe_token)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([
            $data['user_id'] ?? null,
            $data['email'],
    ...
    {
        $stmt = $this->pdo->prepare("UPDATE property_alert_subscriptions SET is_active = 0 WHERE unsubscribe_token = ?");
        $stmt->execute([$token]);
    ...
    {
        $stmt = $this->pdo->prepare("INSERT INTO property_alert_log (subscription_id, property_id, user_id, channel, status, message, sent_at) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$subId, $propId, $userId, $channel, $status, $message, $status === 'sent' ? date('Y-m-d H:i:s') : null]);
        return (int)$this->pdo->lastInsertId();
    }

    public function markNotified(int $subId): void
    {
        $stmt = $this->pdo->prepare("UPDATE property_alert_subscriptions SET last_notified_at = NOW(), total_notifications = total_notifications + 1 WHERE id = ?");
        $stmt->execute([$subId]);
    }
