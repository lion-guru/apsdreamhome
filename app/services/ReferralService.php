<?php
require_once __DIR__ . '/RankService.php';
/**
 * Referral Service
...
class ReferralService {
                        ...
                        if ($status === 'success' && !$dryRun) {
                            ...
                            try {
                                ...
                                if ($referralCodeOverride) {
                                    ...
                                    $stmtOverride->bind_param('si', $referralCodeOverride, $userId);
    ...
    public function generateReferralCode($name, $email, $user_type = null) {
        ...
        $type_prefix = strtoupper(substr($user_type, 0, 1)) ?? 'U';
    ...
    private function codeExists($code) {
        $stmt = $this->conn->prepare("SELECT COUNT(*) FROM mlm_profiles WHERE referral_code = ?");
    ...
    public function getReferralLink($user_id, $role = null) {
        ...
        if ($result) {
            ...
            return BASE_URL . 'register?' . http_build_query($params);
        }
        return null;
    }
    ...
    public function trackReferral($referrer_user_id, $referred_user_id, $referral_type, $channel = 'direct_link') {
        ...
        $stmt->bind_param("iiss", $referrer_user_id, $referred_user_id, $referral_type, $channel);
        return $stmt->execute();
    }
    ...
    public function getNetworkTree($user_id, $max_depth = 5, array $options = []) {
        ...
        ");
        $stmt->bind_param("ii", $user_id, $max_depth);
        $stmt->execute();
    ...
    public function getNetworkStats($user_id) {
        ...
        $stmt->bind_param('i', $user_id);
        $stmt->execute();
        $level_breakdown = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
        $stmt->close();
    ...
    private function introducesCircularReference(int $userId, int $potentialSponsorId): bool
    {
        ...
        $stmt->bind_param('i', $potentialSponsorId);
        $stmt->execute();
        $ancestors = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
        $stmt->close();
