 ...
 * RoyaltyPoolService
 * ──────────────────
 * Manages the 2% royalty pool that is collected outside the 20% global cap.
 *
 * Collection:
 *   - 2% of every payment received is contributed via contributeToPool().
 *   - Called by HybridCommissionEngine::contributeToRoyaltyPool() which already writes
 *     to royalty_pool_contributions.
 *
 * Distribution:
 *   - Monthly cron calls distributeMonthlyRoyalty() to split the pool among
 *     all agents at rank Vice President or higher, proportional to their GBV.
 *
 * Tables used:
 *   royalty_pool_contributions  (id, booking_id, amount, contributed_at)
 *   royalty_pool_distributions  (id, user_id, month_year, share_pct, amount, distributed_at)
 *   mlm_commission_ledger       (written via HybridCommissionEngine::writeLedger equivalent)
 *   mlm_profiles                (lifetime_sales, rank)
 */
