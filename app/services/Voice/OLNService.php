...
class OLNService
{
        ...
        try {
            $unassigned = $this->db->fetchAll(
                "SELECT l.*, u.name as lead_name, u.phone, u.email
                 ...
                 AND l.status NOT IN ('closed', 'dnd', 'not_interested')
            ...
            foreach ($unassigned as $lead) {
                ...
                $this->db->execute(
                    "INSERT INTO ai_calling_schedule (lead_id, user_id, phone, agent_type, assigned_agent, scheduled_date, status, priority, created_at)
                ...
                $this->db->execute(
                    "UPDATE leads SET assigned_to = ?, assigned_agent_type = 'followup' WHERE id = ?",
                    [$agent['id'], $lead['id']]
                );
    ...
    public function getLeadJourney($leadId)
    {
        try {
            ...
            $sessions = $this->db->fetchAll(
                "SELECT * FROM ai_call_sessions WHERE lead_id = ? ORDER BY started_at DESC",
                [$leadId]
            );

            $schedule = $this->db->fetchAll(
                "SELECT * FROM ai_calling_schedule WHERE lead_id = ? ORDER BY created_at DESC",
                [$leadId]
            );
    ...
    public function advanceStage($leadId, $newStage, $notes = '')
    {
        try {
            ...
            $current = $this->db->fetch("SELECT status FROM leads WHERE id = ?", [$leadId]);
            if (!$current) {
                return ['success' => false, 'message' => 'Lead not found'];
            }

            $this->db->execute(
                "UPDATE leads SET status = ?, last_stage_change = NOW() WHERE id = ?",
                [$newStage, $leadId]
            );
            ...
            if ($newStage === 'viewing') {
                $lead = $this->db->fetch("SELECT user_id, property_interest FROM leads WHERE id = ?", [$leadId]);
                if ($lead) {
                    $this->db->execute(
                        "INSERT INTO ai_calling_schedule (lead_id, user_id, agent_type, scheduled_date, priority, status, created_at)
                         VALUES (?, ?, 'site_visit', DATE_ADD(CURDATE(), INTERVAL 1 DAY), 1, 'scheduled', NOW())",
                        [$leadId, $lead['user_id']]
                    );
                }
            }
            ...
            if ($newStage === 'not_interested') {
                $this->db->execute(
                    "UPDATE leads SET dnd_until = DATE_ADD(NOW(), INTERVAL 30 DAY) WHERE id = ?",
                    [$leadId]
                );
            }
    ...
    public function calculateLeadScore($leadId)
    {
        try {
            ...
            $sessions = $this->db->fetchAll(
                "SELECT * FROM ai_call_sessions WHERE lead_id = ?",
                [$leadId]
            );
        ...
        try {
            ...
            $leadsByStage = $this->db->fetchAll(
                "SELECT COALESCE(status, 'new') as stage, COUNT(*) as count
                 FROM leads WHERE created_at >= ? OR last_stage_change >= ?
            ...
            $avgTimeToConvert = $this->db->fetch(
                "SELECT AVG(DATEDIFF(COALESCE(last_stage_change, updated_at), created_at)) as avg_days
                 FROM leads WHERE status = 'closed' AND created_at >= ?",
                [$since]
            );
            ...
            $bestCallingTimes = $this->db->fetchAll(
                "SELECT HOUR(started_at) as hour, COUNT(*) as calls,
                 ...
                 FROM ai_call_sessions WHERE started_at >= ?
            ...
            $agentPerformance = $this->db->fetchAll(
                "SELECT a.id, a.name, COUNT(s.id) as total_calls,
                 ...
                 LEFT JOIN ai_call_sessions s ON a.id = s.agent_id AND s.started_at >= ?
    ...
    public function bulkScheduleFollowUps($leadIds, $agentType = 'followup')
    {
        try {
            ...
            foreach ($leadIds as $leadId) {
                $lead = $this->db->fetch("SELECT id, user_id, phone, assigned_to FROM leads WHERE id = ?", [$leadId]);
                if (!$lead) continue;

                $this->db->execute(
                    "INSERT INTO ai_calling_schedule (lead_id, user_id, phone, agent_type, assigned_agent, scheduled_date, status, created_at)
                     VALUES (?, ?, ?, ?, ?, DATE_ADD(CURDATE(), INTERVAL 1 DAY), 'scheduled', NOW())",
                    [$leadId, $lead['user_id'], $lead['phone'] ?? '', $agentType, $lead['assigned_to']]
                );
                $scheduled++;
            }
