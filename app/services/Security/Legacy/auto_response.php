...
class AutoResponseSystem {
    ...
    private function scheduleUnblock($ip, $duration) {
        ...
        $query = "INSERT INTO scheduled_tasks (task_type, task_data, execute_at)
                 VALUES ('unblock_ip', ?, DATE_ADD(NOW(), INTERVAL ? SECOND))";

        $this->db->execute($query, [$ip, $duration]);
    }
    ...
    private function getState($key) {
        $row = $this->db->fetchOne("SELECT value FROM system_state WHERE `key` = ?", [$key]);
    ...
    private function setState($key, $value) {
        $query = "INSERT INTO system_state (`key`, value)
                 VALUES (?, ?)
                 ON DUPLICATE KEY UPDATE value = ?";

        $this->db->execute($query, [$key, $value, $value]);
    }
