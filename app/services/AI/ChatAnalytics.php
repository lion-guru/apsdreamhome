...
class ChatAnalytics
{
    ...
    public function logEvent(string $event, string $action, string $sessionId, ?int $userId = null, array $meta = []): void
    {
        try {
            $this->db->query(
                "INSERT INTO chat_analytics (event_type, action, session_id, user_id, meta, created_at) VALUES (?, ?, ?, ?, ?, NOW())",
                [$event, $action, $sessionId, $userId, json_encode($meta)]
            );
        } catch (\Exception $e) {
    ...
    public function getDashboard(int $days = 30): array
    {
        ...
        $dropoffs = $this->db->fetchAll(
            "SELECT action, 
                    ...
                    COUNT(*) as count
             FROM chat_analytics 
             WHERE event_type = 'dropoff' AND created_at >= ?
