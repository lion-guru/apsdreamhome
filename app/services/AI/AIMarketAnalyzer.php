...
class AIMarketAnalyzer {
    ...
    private function calculatePriceTrend($location, $propertyType, $days) {
        try {
            ...
            $currentData = $this->db->fetch($sql, $params);

            if (!$currentData) {
                $currentData = ['avg_price' => 0, 'property_count' => 0];
            }
            ...
            // Add the last ? for the final DATE_SUB
            $prevParams[] = $days;
            $prevData = $this->db->fetch($prevSql, $prevParams);
...
//
// This file contains 885 lines. Consider optimizations:
//
