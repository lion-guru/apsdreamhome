...
class SecurityManager {
    ...
    private function initializeEncryptionKey() {
        ...
        if (!$key) {
            $key = \bin2hex(\App\Helpers\SecurityHelper::secureRandomBytes(32)); // Generate a secure random key
    ...
    public function encrypt($data) {
        $data = \serialize($data);
        $iv = \App\Helpers\SecurityHelper::secureRandomBytes(\openssl_cipher_iv_length($this->encryption_cipher));
    ...
    public function checkRateLimit($identifier) {
        ...
        try {
            ...
            // Check and clean up old rate limit entries using modern PDO-based API
            $this->db->executeQuery("
                DELETE FROM rate_limits
                WHERE timestamp < ? OR identifier = ?
            ", [$time_window_start, $identifier]);
            ...
            if ($result && $result['request_count'] >= $this->rate_limit_config['max_requests']) {
                // Block IP
                ...
                ", [$identifier, $block_time, $block_time]);
