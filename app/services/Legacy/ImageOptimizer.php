...
class ImageOptimizer {
    ...
    public static function generateResponsiveImage($src, $alt, $options = []) {
        ...
        $html = '<img ';
        $html .= 'src="' . h($options['placeholder']) . '" ';
        $html .= 'data-src="' . h($src) . '" ';

        if ($options['srcset']) {
            $html .= 'data-srcset="' . h($options['srcset']) . '" ';
            $html .= 'sizes="' . h($options['sizes']) . '" ';
        }

        $html .= 'alt="' . h($alt) . '" ';
        $html .= 'loading="' . $options['loading'] . '" ';
        $html .= 'class="lazyload ' . h($options['class']) . '" ';
    ...
    public static function optimizeImageAttributes($src, $alt, $options = []) {
        ...
        if ($lqip) {
            $html .= 'src="' . h($lqip) . '" ';
            $html .= 'data-src="' . h($src) . '" ';
            $html .= 'class="lazyload blur-up ' . h($options['class']) . '" ';
        } else {
            $html .= 'src="' . h($src) . '" ';
            $html .= 'class="' . h($options['class']) . '" ';
        }
    ...
    public static function generatePictureElement($src, $alt, $options = []) {
        ...
        if (file_exists($_SERVER['DOCUMENT_ROOT'] . $webp_src)) {
            $html .= '<source srcset="' . h($webp_src) . '" type="image/webp">';
        }

        // Fallback image
        $html .= '<img ';
        $html .= 'src="' . h($fallback_src) . '" ';
        $html .= 'alt="' . h($alt) . '" ';
        $html .= 'loading="' . $options['loading'] . '" ';
        $html .= 'class="' . h($options['class']) . '" ';
        $html .= 'sizes="' . h($options['sizes']) . '" ';
