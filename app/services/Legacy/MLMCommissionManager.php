...
class MLMCommissionManager
{
            ...
            if ($row && $row['count'] == 0) {
                ...
                foreach ($levels as $level) {
                    $sql = "INSERT INTO associate_levels (level_name, level_number, min_team_size, min_personal_sales, commission_percentage, bonus_percentage, override_percentage, status)
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                    $this->db->execute($sql, $level);
                }
            }
        } catch (\Exception $e) {
            if ($this->logger) {
                $this->logger->log("Error inserting default commission levels: " . $e->getMessage(), 'error', 'commission');
            }
        }
    }
    ...
    public function calculateBookingCommission($bookingId)
    {
        ...
        try {
            ...
            if ($this->logger) {
                $this->logger->log("Error calculating booking commission: " . $e->getMessage(), 'error', 'commission');
            }
            return false;
        }
    }
    ...
    private function insertCommissionRecord($bookingId, $commission)
    {
        ...
        try {
            ...
            if ($this->logger) {
                $this->logger->log("Error inserting commission record: " . $e->getMessage(), 'error', 'commission');
            }
        }
    }
    ...
    private function getAssociateLevelInfo($associateId)
    {
        ...
        try {
            ...
            if ($this->logger) {
                $this->logger->log("Error getting associate level info: " . $e->getMessage(), 'error', 'commission');
            }
            return null;
        }
    }
    ...
    private function getAssociateInfo($associateId)
    {
        ...
        try {
            ...
            if ($this->logger) {
                $this->logger->log("Error getting associate info: " . $e->getMessage(), 'error', 'commission');
            }
            return null;
        }
    }
    ...
    private function getAssociateHierarchy($associateId, $maxLevels = 5)
    {
        ...
        try {
            while ($currentId && $level < $maxLevels) {
                ...
                $associate = $this->db->fetch($sql, [$currentId]);
            ...
            if ($this->logger) {
                $this->logger->log("Error getting associate hierarchy: " . $e->getMessage(), 'error', 'commission');
            }
        }
    ...
    private function getAssociateMonthlySales($associateId)
    {
        ...
        try {
            $row = $this->db->fetch($sql, [$associateId, $currentMonth, $nextMonth]);
            return $row['monthly_sales'] ?? 0;
        } catch (\Exception $e) {
            if ($this->logger) {
                $this->logger->log("Error getting associate monthly sales: " . $e->getMessage(), 'error', 'commission');
            }
            return 0;
        }
    }
    ...
    private function getAssociateTeamSize($associateId)
    {
        ...
        try {
            ...
            return $row['team_size'] ?? 0;
        } catch (\Exception $e) {
            if ($this->logger) {
                $this->logger->log("Error getting associate team size: " . $e->getMessage(), 'error', 'commission');
            }
            return 0;
        }
    }
    ...
    public function processCommissionPayouts($associateId, $periodStart, $periodEnd)
    {
        ...
        try {
            $commissions = $this->db->fetchAll($sql, [$associateId, $periodStart, $periodEnd]);
            ...
            if ($this->logger) {
                $this->logger->log("Error processing commission payouts: " . $e->getMessage(), 'error', 'commission');
            }
    ...
    public function getAssociateDashboard($associateId)
    {
        ...
        $sql = "SELECT ct.*, pb.booking_number, pb.total_amount as booking_amount
                ...
                LIMIT 10";
        $dashboard['recent_commissions'] = $this->db->fetchAll($sql, [$associateId]);
    ...
    private function getAssociateAchievements($associateId)
    {
        ...
        return $this->db->fetchAll($sql, [$associateId]);
    }
    ...
    public function getAssociatePerformance($associateId)
    {
        ...
        try {
            // Total direct sales
            $sql = "SELECT COUNT(*) as direct_sales, SUM(total_amount) as total_volume
                    FROM plot_bookings WHERE associate_id = ? AND status IN ('confirmed', 'completed')";
            $row = $this->db->fetch($sql, [$associateId]);
            $performance['direct_sales'] = $row['direct_sales'] ?? 0;
            $performance['direct_volume'] = $row['total_volume'] ?? 0;

            // Total team sales
            $sql = "SELECT COUNT(*) as team_sales, SUM(total_amount) as team_volume
                    FROM plot_bookings pb
                    JOIN associates a ON pb.associate_id = a.id
                    WHERE a.sponsor_id = ? AND pb.status IN ('confirmed', 'completed')";
            $row = $this->db->fetch($sql, [$associateId]);
            $performance['team_sales'] = $row['team_sales'] ?? 0;
            $performance['team_volume'] = $row['team_volume'] ?? 0;

            // Level progression
            $performance['current_level'] = $this->getAssociateLevelInfo($associateId);
    ...
    public function generateCommissionReport($associateId, $startDate, $endDate)
    {
        ...
        try {
            ...
            $report['breakdown_by_type'] = $this->db->fetchAll($sql, [$associateId, $startDate, $endDate]);
            ...
            $sql = "SELECT DATE_FORMAT(created_at, '%Y-%m') as month,
                    ...
                    ORDER BY month";
            $report['monthly_trend'] = $this->db->fetchAll($sql, [$associateId, $startDate, $endDate]);
