...
class TwoFactorManager {
    ...
    private function sendToken($user_id, $token, $method, $user_type = 'user') {
        ...
        if ($user_type === 'admin') {
            $sql = "SELECT email as uemail, NULL as uphone FROM admin WHERE id = ?";
        } else {
            $sql = "SELECT uemail, uphone FROM user WHERE uid = ?";
        }
        ...
        switch ($method) {
            case self::METHOD_EMAIL:
                if (!empty($user['uemail'])) {
                    $this->sendEmailToken($user['uemail'], $token);
                }
                break;
            case self::METHOD_SMS:
                if (!empty($user['uphone'])) {
                    $this->sendSmsToken($user['uphone'], $token);
                }
                break;
