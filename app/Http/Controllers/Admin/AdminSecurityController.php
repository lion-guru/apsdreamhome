...
class AdminSecurityController extends AdminController
{
    public function index()
    {
        ...
        try {
            ...
            $failed24h = (int)($this->db->query("SELECT COUNT(*) FROM failed_login_attempts WHERE attempt_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR)")->fetchColumn());
            $failed7d = (int)($this->db->query("SELECT COUNT(*) FROM failed_login_attempts WHERE attempt_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)")->fetchColumn());
            $tfaEnabled = (int)($this->db->query("SELECT COUNT(*) FROM users WHERE two_factor_enabled = 1")->fetchColumn());
            $totalUsers = (int)($this->db->query("SELECT COUNT(*) FROM users WHERE deleted_at IS NULL")->fetchColumn());
            ...
            $topIPs = $this->db->query("SELECT ip_address, COUNT(*) as cnt FROM failed_login_attempts WHERE attempt_at >= DATE_SUB(NOW(), INTERVAL 7 DAY) GROUP BY ip_address ORDER BY cnt DESC LIMIT 10")->fetchAll(\PDO::FETCH_ASSOC);
