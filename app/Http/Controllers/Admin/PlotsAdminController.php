...
class PlotsAdminController
{
    ...
    public function index()
    {
        ...
        // Get filters data
        $colonies = $this->db->query("SELECT c.*, d.name as district_name, s.name as state_name FROM colonies c LEFT JOIN districts d ON c.district_id = d.id LEFT JOIN states s ON d.state_id = s.id WHERE c.is_active = 1 ORDER BY s.name, d.name, c.name")->fetchAll(\PDO::FETCH_ASSOC);
