...
class HomePageController extends BaseController
{
    ...
    public function home()
    {
        ...
        $hero_properties = \App\Services\Cache\HotPathCacheService::getHomeHeroProperties(
            function () {
                $stmt = $this->db->prepare("SELECT p.id, p.title, p.price, p.city, p.location, p.type, p.area_sqft, p.bedrooms, p.featured, (SELECT image_path FROM property_images WHERE property_id = p.id ORDER BY is_primary DESC, id ASC LIMIT 1) AS image FROM properties p WHERE p.status = 'active' AND p.featured = 1 ORDER BY p.created_at DESC LIMIT 3");
                $stmt->execute();
