...
class AuthMiddleware
{
    ...
    public static function validateAdminCredentials($username, $password)
    {
        ...
        try {
            ...
            $sql = "SELECT id, name, email, password, role, status FROM users WHERE name = :username OR email = :email";
            $user = $db->fetch($sql, ['username' => $username, 'email' => $username]);

            if (!$user) {
                // Fallback check in admin table if not found in users
                // PERFORMANCE: Select only needed columns instead of SELECT *
                $sqlAdmin = "SELECT id, auser, email, apassword, role, status FROM admin WHERE auser = :username OR email = :email";
...
// Standalone auth helper functions
function adminAuth()
{
    if (!isset($_SESSION['admin_logged_in']) || $_SESSION['admin_logged_in'] !== true) {
        if (
...
{
    if (
        !isset($_SESSION['user_id']) && !isset($_SESSION['admin_logged_in']) &&
