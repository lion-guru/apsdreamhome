<?php
/**
 * PHASE 10: Drop tables with 0-3 rows AND <=3 code refs AND no FKs
 * These are "feature scaffolding" with barely any data
 */
...
foreach ($tables as $t) {
    ...
    if ($isView === 'VIEW') {
        // Try to verify view works
        try {
            $pdo->query("SELECT * FROM `$t` LIMIT 1");
        } catch (Exception $e) {
            // Broken view - drop it
            try {
                $pdo->exec("DROP VIEW IF EXISTS `$t`");
                $dropped++;
            } catch (Exception $ex) {}
        }
        continue;
...
echo "Dropped: $dropped (tables with <=3 rows, <=3 refs, 0 FKs, all refs in try/catch)\n";
