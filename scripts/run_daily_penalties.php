...
try {
    ...
    if ($result['success']) {
        echo "✅ Penalties applied successfully" . PHP_EOL;
        echo "   Installments affected: {$result['penalties_applied']}" . PHP_EOL;
        echo "   Total penalty accrued: ₹" . number_format($result['total_penalty'], 2) . PHP_EOL;
        ...
        if (!empty($result['installments'])) {
            ...
            foreach ($result['installments'] as $inst) {
                echo "   - Installment #{$inst['id']} ({$inst['booking_number']}, #{$inst['installment_no']}): " .
                     ...
                     "₹" . number_format($inst['new_penalty'], 2) . " penalty, " .
                     "total accrued: ₹" . number_format($inst['total_accrued'], 2) . PHP_EOL;
            }
        }
    } else {
        echo "⚠️  Penalty engine returned: " . ($result['error'] ?? 'unknown error') . PHP_EOL;
    }
