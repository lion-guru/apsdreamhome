...
find_duplicates() {
    ...
    find /c/xampp/htdocs/apsdreamhome -type f -name "*.$file_type" | while read -r file; do
        # Get the filename without path
        filename=$(basename "$file")
        
        # Count occurrences of this filename
        count=$(find /c/xampp/htdocs/apsdreamhome -type f -name "$filename" | wc -l)
        
        if [ "$count" -gt 1 ]; then
            echo "Duplicate found: $filename (Occurrences: $count)" | tee -a "$DUPLICATE_LOG"
            find /c/xampp/htdocs/apsdreamhome -type f -name "$filename" | tee -a "$DUPLICATE_LOG"
