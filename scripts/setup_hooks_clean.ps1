...
"@
        ...
        Write-Log "Git hooks path already configured" -ForegroundColor Yellow
    }
}
...
function New-TaskSchedulerJob {
    ...
    if ($existingTask -and !$Force) {
        Write-Log "Task '$taskName' already exists. Use -Force to recreate." -ForegroundColor Yellow
        return
    }
    ...
    if ($existingTask -and $Force) {
        ...
        Write-Log "Existing task removed" -ForegroundColor Yellow
    }
...
try {
    ...
    if (!(Test-AdminPrivileges)) {
        Write-Log "Not running as administrator. Some features may not work." -ForegroundColor Yellow
    }
