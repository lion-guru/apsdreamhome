...
foreach ($rows as $r) {
    echo "User {$r['user_id']}: level={$r['current_level']}, team={$r['total_team_size']}, directs={$r['direct_referrals']}, sales=₹{$r['lifetime_sales']}\n";
}
...
foreach ($rows as $r) {
    echo "User {$r['beneficiary_user_id']}: {$r['cnt']} entries, ₹{$r['total']}\n";
}
...
foreach ($rows as $r) {
    echo "User {$r['user_id']}: {$r['cnt']} bookings, ₹{$r['total']}\n";
}
