...
define('DB_NAME', 'apsdreamhomefinal_prod');
```
...
chmod 755 /var/www/html/apsdreamhomefinal/admin/
chmod 755 /var/www/html/apsdreamhomefinal/includes/
chmod 777 /var/www/html/apsdreamhomefinal/uploads/ (if exists)
...
GRANT SELECT, INSERT, UPDATE, DELETE ON apsdreamhomefinal_prod.* TO 'aps_prod_user'@'localhost';
FLUSH PRIVILEGES;
```
...
mysqldump -u root -p apsdreamhomefinal_prod > backup_$(date +%Y%m%d).sql

# Weekly full backup
tar -czf aps_backup_$(date +%Y%m%d).tar.gz /var/www/html/apsdreamhomefinal/
```
