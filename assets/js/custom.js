    ...
    .then((data) => {
      if (data.success) {
        ...
        if (favoriteCount) {
          ...
          favoriteCount.textContent = isFavorite
            ? currentCount - 1
            : currentCount + 1;
        }
      } else {
...
function shareProperty(propertyId, title = "", text = "") {
  ...
  if (navigator.share) {
    navigator
      ...
      .catch((error) => {
        // Debug: console.log('Error sharing:', error);
...
function submitVisitRequest(form) {
  ...
  setTimeout(() => {
    ...
    // Debug: console.log('Submitting visit request:', jsonData);
    ...
    if (success) {
      ...
      showToast(
        "Visit scheduled successfully! Our agent will contact you soon.",
        "success",
      );
...
function updatePropertyResults() {
  ...
  setTimeout(() => {
    // In a real application, you would make an AJAX request here
    // Debug: console.log('Updating property results with filters:', Object.fromEntries(searchParams.entries()));
...
function initNewsletterForm() {
  ...
  newsletterForm.addEventListener("submit", function (e) {
    ...
    // Simulate API call (replace with actual API call)
    setTimeout(() => {
      // Debug: console.log('Subscribing email:', emailInput.value);
...
// Schedule visit modal function
function showScheduleVisitModal(propertyId) {
  // Create modal HTML
...
function submitVisitRequest(propertyId) {
  ...
  // Debug: console.log('Submitting visit request:', formData);

  // Here you would typically make an AJAX call to submit the form
  // For now, we'll just show a success message
  const modal = bootstrap.Modal.getInstance(
    document.getElementById("scheduleVisitModal"),
  );
  modal.hide();

  // Show success message
  alert(
    "Your visit has been scheduled! We will contact you shortly to confirm the details.",
  );
}
