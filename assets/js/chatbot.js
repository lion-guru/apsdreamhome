...
(function() {
    ...
    window.toggleChat = function() {
        ...
        if (floatBtn) floatBtn.style.display = isOpen ? 'none' : 'flex';
    };

    window.toggleChatLanguage = function() {
        lang = (lang === 'en') ? 'hi' : 'en';
        var btn = document.getElementById('langToggle');
        if (btn) btn.textContent = lang === 'hi' ? '\u{1F1EE}\u{1F1F3} EN' : '\u{1F1EE}\u{1F1F3} HI';
    };
    ...
    function getBotResponse(userMsg) {
        ...
        fetch(apiUrl, {
            ...
            body: JSON.stringify({ message: userMsg, language: lang, context: context })
        })
        ...
        .catch(function() {
            typingDiv.remove();
            appendMessage('Namaste! \u{1F91D} Welcome to APS Dream Home! How can I help you today?', 'bot');
        });
    }
    ...
    // Show a quick greeting hint after 5s on first visit
    if (!sessionStorage.getItem('chatbot_shown')) {
        setTimeout(function() {
            if (!isOpen && floatBtn) {
                floatBtn.style.animation = 'pulse-ring 1.5s ease 3';
                setTimeout(function() { floatBtn.style.animation = ''; }, 4500);
            }
            sessionStorage.setItem('chatbot_shown', '1');
        }, 5000);
    }
