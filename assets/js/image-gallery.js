/**
 * ImageGallery Lightbox - pure vanilla JS (no jQuery)
 * Features:
 *  - Open on click of [data-gallery] img or .property-image
 *  - Full-size image with caption
 *  - Thumbnail strip at bottom (clickable)
 *  - Keyboard nav: arrows, ESC
 *  - Touch gestures: swipe left/right on mobile
 *  - Zoom: click to zoom in, double-click to zoom 2x
...
(function() {
    ...
    class ImageGallery {
        ...
        bindDocumentClicks() {
            document.addEventListener('click', (e) => {
                const target = e.target.closest('img[data-gallery-group], .lightbox-trigger, [data-gallery] img, [data-gallery] .thumb, .property-image');
                if (!target) return;
                e.preventDefault();
        ...
        build() {
            ...
            this.overlay.innerHTML = `
                ...
                <div class="lightbox-controls">
                    ...
                    <button type="button" class="lightbox-btn" data-action="reset" title="Reset zoom">1x</button>
                    <button type="button" class="lightbox-btn" data-action="play" title="Play slideshow">&#9658;</button>
                    <button type="button" class="lightbox-btn" data-action="download" title="Download">&#11015;</button>
                </div>
            ...
            this.overlay.querySelector('.lightbox-image').addEventListener('click', () => this.zoomIn());
            this.overlay.querySelector('.lightbox-image').addEventListener('dblclick', () => {
            ...
            stage.addEventListener('touchstart', (e) => { this.touchStartX = e.changedTouches[0].screenX; }, { passive: true });
        ...
        handleAction(action) {
            switch (action) {
                ...
                case 'reset': this.zoom = 1; this.applyZoom(); break;
                case 'play': this.toggleSlideshow(); break;
                case 'download': this.download(); break;
            }
        }

        zoomIn() { this.zoom = Math.min(this.zoom + 0.25, 4); this.applyZoom(); }
        ...
        toggleSlideshow() {
            const btn = this.overlay.querySelector('[data-action="play"]');
            if (this.slideshow) {
                ...
                btn.innerHTML = '&#10074;&#10074;';
                this.slideshow = setInterval(() => this.next(), 4000);
            }
        }
        ...
        hide() {
            ...
            if (this.slideshow) { clearInterval(this.slideshow); this.slideshow = null; }
            this.zoom = 1;
            setTimeout(() => { if (this.overlay && !this.isOpen) { this.overlay.remove(); this.overlay = null; } }, 300);
        }
        ...
        render() {
            ...
            this.images.forEach((entry, idx) => {
                ...
                t.loading = 'lazy';
                t.addEventListener('click', () => { this.current = idx; this.render(); });
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => { window.imageGallery = new ImageGallery(); });
    } else {
