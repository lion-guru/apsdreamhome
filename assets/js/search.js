...
function createPropertyCard(property) {
    ...
    card.innerHTML = `
        <div class="property-card card h-100 border-0 shadow-sm" 
             data-id="${property.id || ''}"
             data-title="${property.title ? property.title.replace(/"/g, '&quot;') : ''}"
            ...
            <div class="position-relative">
                <img src="${property.thumbnail || 'assets/images/properties/placeholder.jpg'}" 
                     class="card-img-top property-thumbnail" 
                     alt="${property.title || 'Property'}">
                <div class="property-badge">${property.type || 'For Sale'}</div>
                <div class="property-price">${formatPrice(property.price)}</div>
                <button class="btn btn-icon btn-light rounded-circle property-favorite" data-property-id="${property.id || ''}">
                    <i class="far fa-heart"></i>
                </button>
            </div>
            <div class="card-body">
                <h5 class="card-title text-truncate">${property.title || 'Property'}</h5>
            ...
            <div class="card-footer bg-transparent border-top-0">
                <a href="property-details.php?id=${property.id || ''}" class="btn btn-outline-primary w-100">
                    View Details
                </a>
            </div>
        </div>
...
function displaySearchResults(properties, total, append = false) {
    ...
    if (resultsCount) {
        resultsCount.textContent = `${total} propert${total === 1 ? 'y' : 'ies'} found`;
    }
    ...
    if (properties && properties.length > 0) {
        ...
        if (loadMoreBtn) {
            ...
            loadMoreBtn.innerHTML = remaining > 0 
                ? `<span class="loading-spinner d-none"></span> Load ${Math.min(remaining, itemsPerPage)} More`
                : 'No More Results';
        }
        
        // If we're in map view, update the map with the new properties
        if (currentView === 'map') {
            const propertyData = Array.from(document.querySelectorAll('.property-card')).map(card => ({
                id: card.dataset.id,
    ...
    <div class="card property-card h-100">
      ...
      <div class="card-body">
        ...
        <div class="property-features mb-3">
          ...
          ${property.area ? `<span><i class="fas fa-ruler-combined me-1"></i> ${property.area} sq.ft</span>` : ''}
        </div>
        <div class="d-flex justify-content-between align-items-center">
          <h6 class="mb-0 text-primary">${property.price ? formatPrice(property.price) : 'Contact for price'}</h6>
          <a href="/property-details.php?id=${property.id}" class="btn btn-sm btn-outline-primary">View Details</a>
        </div>
      </div>
    </div>
  `;
...
function showError(message) {
  const searchResultsContainer = document.getElementById('searchResultsContainer') || document.getElementById('searchResults');
  if (!searchResultsContainer) return;
            ...
            const marker = L.marker([parseFloat(property.latitude), parseFloat(property.longitude)])
                ...
                .bindPopup(`
                    <div class="map-popup">
                        ...
                        <p class="mb-1"><strong>${formatPrice(property.price)}</strong></p>
                        <p class="mb-1 text-muted small">${property.bedrooms || 0} Beds | ${property.bathrooms || 0} Baths</p>
                        <a href="property-details.php?id=${property.id}" class="btn btn-sm btn-primary w-100 mt-2">View Details</a>
                    </div>
                `);
...
function toggleView(view) {
    ...
    if (view === 'map') {
        ...
        const properties = Array.from(document.querySelectorAll('.property-card')).map(card => ({
            id: card.dataset.id,
            title: card.dataset.title,
            price: card.dataset.price,
            bedrooms: card.dataset.bedrooms,
...
async function fetchProperties() {
    ...
    if (isLoading || !searchForm) return;
    ...
    try {
        ...
        if (data.success && data.properties) {
            ...
            // Display results
            displaySearchResults(properties, total, currentPage > 1);
            
            // Update pagination state
            hasMoreResults = (currentPage * itemsPerPage) < total;
            
            // Show/hide load more button
            if (loadMoreBtn) {
                loadMoreBtn.classList.toggle('d-none', !hasMoreResults);
                loadMoreBtn.disabled = false;
                loadMoreBtn.innerHTML = 'Load More Results';
            }
        } else {
            throw new Error(data.message || 'No properties found');
        }
    } catch (error) {
        console.error('Error fetching properties:', error);
...
async function loadSavedSearches() {
    try {
        ...
        savedSearchesList.innerHTML = data.data.map(search => `
            <div class="list-group-item list-group-item-action">
                <div class="d-flex w-100 justify-content-between align-items-center">
                    <h6 class="mb-1">${escapeHtml(search.name)}</h6>
                    <small class="text-muted">${formatDate(search.updated_at)}</small>
                </div>
                <div class="d-flex mt-2">
                    <button class="btn btn-sm btn-outline-primary me-2 load-search" data-id="${search.id}">
                        <i class="fas fa-search me-1"></i> Load
                    </button>
                    <button class="btn btn-sm btn-outline-danger delete-search" data-id="${search.id}">
...
async function loadSavedSearch(event) {
    try {
        ...
        if (typeof updatePriceRangeDisplay === 'function' && searchParams.min_price && searchParams.max_price) {
        ...
        if (searchParams.view && (searchParams.view === 'list' || searchParams.view === 'map')) {
    ...
    if (searchForm) {
        ...
        if (toggleBtn && advancedOptions) {
            toggleBtn.addEventListener('click', (e) => {
                ...
                const icon = toggleBtn.querySelector('i');
                if (icon) {
                    icon.className = isHidden ? 'fas fa-chevron-up me-1' : 'fas fa-sliders-h me-1';
                }
