...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (
      e.target.matches(".select-all-searches") ||
      e.target.closest("#selectAllSearches") ||
      e.target.closest("#deselectAllSearches")
    ) {
    ...
    // Handle bulk actions
    if (
      e.target.matches(".bulk-action-btn") ||
    ...
    if (
      e.target.matches(".search-checkbox") ||
      e.target.closest(".search-checkbox")
    ) {
      const checkbox = e.target.matches(".search-checkbox")
        ? e.target
        : e.target.closest(".search-checkbox");
    ...
    if (selectedCount > 0) {
      ...
      if (selectAllCheckbox) {
        const totalCheckboxes =
          document.querySelectorAll(".search-checkbox").length;
...
function handleBulkAction(action) {
  ...
  switch (action) {
    case "delete":
      if (
        confirm(
          `Are you sure you want to delete ${selectedIds.length} selected searches?`,
        )
      ) {
...
async function deleteMultipleSearches(ids) {
  try {
    ...
    if (data.success) {
      showToast(
        `Successfully deleted ${data.deleted_count} searches`,
        "success",
      );
      loadSavedSearches();
    } else {
...
function exportSearches(ids) {
  ...
  // Debug: console.log('Exporting searches:', ids);
...
async function saveSearch(name) {
  ...
  try {
    ...
    showToast(
      error.message || "Failed to save search. Please try again.",
      "error",
    );
  } finally {
    // Reset button state
...
async function loadSavedSearches() {
  ...
  try {
    ...
    if (data.success && data.searches && data.searches.length > 0) {
      ...
      const sortedSearches = data.searches.sort(
      ...
      savedSearchesList.innerHTML = recentSearches
        .map(
          (search) => `
                        ...
                        <div class="flex-grow-1">
                            ...
                            <div class="d-flex align-items-center">
                                ...
                                ${
                                  ...
                                  search.search_params?.max_price
...
async function loadSavedSearch(event) {
  ...
  try {
    ...
    if (data.success && data.search) {
      ...
      if (
        typeof priceRangeSlider !== "undefined" &&
      ...
      if (searchItem) {
        ...
        setTimeout(() => {
          ...
          const savedSearchesList =
            document.getElementById("savedSearchesList");
