...
class APS_AI_Client {
  constructor(apiKey = null) {
    this.apiKey =
      apiKey ||
      "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";
  ...
  async generateResponse(messages, options = {}) {
    try {
      ...
      return {
        ...
        error: error.message,
        response:
          "I'm sorry, I'm having trouble connecting to my AI brain right now. Please try again in a moment! 🤖",
      };
    }
  }
    ...
    const messages = [
      {
        role: "system",
        content:
          "You are an expert real estate copywriter for APS Dream Home. Create compelling, professional property descriptions that highlight key features and appeal to potential buyers.",
      },
      {
        role: "user",
    ...
    const messages = [
      {
        role: "system",
        content:
          "You are a real estate valuation expert. Provide accurate market value estimates based on current market trends and property characteristics.",
      },
      {
        role: "user",
    ...
    const messages = [
      {
        role: "system",
        content:
          "You are a strategic technology consultant helping improve development workflows and system architecture. Provide actionable recommendations with implementation priorities.",
      },
      {
        role: "user",
...
async function analyzePropertyWithAI(propertyData) {
  try {
    ...
    return {
      ...
      valuation: valuationResult.success
...
class AIDashboardFeatures {
  ...
  async initialize() {
    try {
      ...
      const recommendations =
        await this.aiClient.getPersonalizedRecommendations();
  ...
  updateAgentStatus(status) {
    ...
    const moodElement = document.querySelector(
      ".ai-agent-status .mood-indicator",
    );
  ...
  displayRecommendations(recommendations) {
    ...
    // Debug: console.log('Personalized recommendations:', recommendations);
  }
  ...
  displayLiveSuggestions(suggestions) {
    ...
    // Debug: console.log('Live suggestions:', suggestions);
  }
}
  ...
  const activityTracker = setInterval(() => {
    ...
    if (userActivity.length >= 5) {
      ...
      if (lastActivities.every((activity) => activity.includes(currentPage))) {
        dashboardFeatures.generateLiveSuggestions(
          `User spending time on ${currentPage}`,
        );
      }
    }
