...
class UnifiedAuthService
{
    ...
    {
        // Use existing session instance if available to ensure shared state
        if (class_exists('App\Core\App') && ($app = \App\Core\App::getInstance()) && $app->session()) {
    ...
    {
        // If session is already started via App, respect its configuration
        ...
        $options = [
            'cookie_lifetime' => $this->config['session_timeout'],
            'cookie_path' => '/',
            'cookie_domain' => $domain,
            'cookie_secure' => $secure,
            'cookie_httponly' => true,
            'cookie_samesite' => 'Lax' // Default to Lax for better UX
        ];
