            ...
            <div class="row mb-4">
                <div class="col-12 text-center">
                    <div class="btn-group flex-wrap" role="group">
                        <a href="<?= BASE_URL ?>downloads" class="btn btn-outline-primary <?= !isset($_GET['category']) || $_GET['category'] == 'all' ? 'active' : '' ?>">All</a>
                        <?php foreach ($categories as $cat): ?>
                            <a href="<?= BASE_URL ?>downloads?category=<?= urlencode($cat) ?>" class="btn btn-outline-primary <?= isset($_GET['category']) && $_GET['category'] == $cat ? 'active' : '' ?>">
                    ...
                    <ul class="pagination justify-content-center">
                        <?php for ($i = 1; $i <= $pagination['total_pages']; $i++): ?>
                            <li class="page-item <?= $pagination['current_page'] == $i ? 'active' : '' ?>">
                                <a class="page-link" href="?page=<?= $i ?><?= isset($_GET['category']) ? '&category=' . urlencode($_GET['category']) : '' ?>">
                                    <?= $i ?>
                                </a>
                            </li>
                        <?php endfor; ?>
