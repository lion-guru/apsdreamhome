...
TRUNCATE TABLE property_feature_mappings;

-- Enable foreign key checks
SET FOREIGN_KEY_CHECKS = 1;

-- Insert sample property features
INSERT INTO property_features (name, icon, status) VALUES
('Swimming Pool', 'fa-swimming-pool', 'active'),
('Gym', 'fa-dumbbell', 'active'),
('Parking', 'fa-parking', 'active'),
('Garden', 'fa-tree', 'active'),
('Security', 'fa-shield-alt', 'active'),
('Lift', 'fa-elevator', 'active'),
('Power Backup', 'fa-bolt', 'active'),
('Water Supply', 'fa-tint', 'active'),
('Club House', 'fa-home', 'active'),
('Play Area', 'fa-child', 'active');
...
VALUES
('Customer One', 'customer6@example.com', '9874532101', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'customer', 'active', '2024-01-10 10:00:00'),
('Customer Two', 'customer7@example.com', '987232102', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'customer', 'active', '2024-01-15 11:30:00'),
('Customer Three', 'customer8@example.com', '9676532103', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'customer', 'active', '2024-02-05 14:15:00'),
('Customer Four', 'customer9@example.com', '9898532104', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'customer', 'active', '2024-02-20 16:30:00'),
('Customer Five', 'customer10@example.com', '98745932105', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'customer', 'active', '2024-03-01 09:45:00');
...
INSERT IGNORE INTO property_types (name, description, icon, status, created_at, updated_at)
VALUES
('Apartment', 'Residential apartment units in a building', 'fa-building', 'active', NOW(), NOW()),
('Villa', 'Independent house with private garden', 'fa-home', 'active', NOW(), NOW()),
('Plot', 'Empty land for construction', 'fa-vector-square', 'active', NOW(), NOW()),
('Penthouse', 'Luxury top-floor apartment', 'fa-building', 'active', NOW(), NOW()),
('Studio', 'Single room apartment with kitchenette', 'fa-home', 'active', NOW(), NOW());
...
INSERT INTO property_feature_mappings (property_id, feature_id, value)
SELECT p.id, f.id, 
  CASE 
    WHEN f.name = 'Swimming Pool' THEN 'Yes'
    WHEN f.name = 'Gym' THEN 'Yes'
    WHEN f.name = 'Parking' THEN '2 Covered'
    WHEN f.name = 'Garden' THEN 'Yes'
    WHEN f.name = 'Security' THEN '24/7 Security'
    WHEN f.name = 'Lift' THEN '2 Lifts'
    WHEN f.name = 'Power Backup' THEN 'Full Backup'
    WHEN f.name = 'Water Supply' THEN '24/7 Supply'
    WHEN f.name = 'Club House' THEN 'Yes'
    WHEN f.name = 'Play Area' THEN 'Children''s Play Area'
  END
FROM properties p
CROSS JOIN property_features f
WHERE (f.name IN ('Swimming Pool', 'Gym', 'Parking', 'Garden', 'Security', 'Lift', 'Power Backup', 'Water Supply', 'Club House', 'Play Area'));
...
-- Create sample MLM tree structure
INSERT INTO mlm_tree (user_id, parent_id, level, path, created_at)
SELECT id, sponsor_id, 1, CONCAT(',', id, ','), NOW() FROM associates WHERE sponsor_id IS NULL
UNION ALL
SELECT a2.id, a2.sponsor_id, 2, CONCAT(',', a2.sponsor_id, ',', a2.id, ','), NOW() 
FROM associates a1
JOIN associates a2 ON a1.id = a2.sponsor_id
WHERE a2.sponsor_id IS NOT NULL;

-- Create sample MLM commissions
INSERT INTO mlm_commissions (user_id, from_user_id, level, amount, percentage, description, status, created_at)
SELECT 
    a1.id as user_id,
    a2.id as from_user_id,
    1 as level,
    b.amount * 0.1 as amount, -- 10% commission for direct referral
    10.0 as percentage,
    'Direct referral commission' as description,
    'pending' as status,
    NOW() as created_at
FROM associates a1
JOIN associates a2 ON a1.sponsor_id = a2.id
JOIN bookings b ON a1.id = b.associate_id
WHERE b.status = 'completed';

-- Create sample MLM payouts
INSERT INTO mlm_payouts (user_id, amount, payment_method, status, processed_at, created_at)
SELECT 
    a.id as user_id,
    ...
    NOW() as created_at
FROM associates a
JOIN mlm_commissions mc ON a.id = mc.user_id
WHERE mc.status = 'approved'
GROUP BY a.id
HAVING SUM(mc.amount) > 0;
