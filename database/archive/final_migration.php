...
function get_primary_key($conn, $db, $table) {
    $result = $conn->prepare("SHOW KEYS FROM `$db`.`$table` WHERE Key_name = 'PRIMARY'");
...
function migrate_table_data($conn, $source_db, $target_db, $table) {
    ...
    $source_columns = [];
    $source_result = $conn->prepare("SHOW COLUMNS FROM `$source_db`.`$table`");
    ...
    $target_columns = [];
    $target_result = $conn->prepare("SHOW COLUMNS FROM `$target_db`.`$table`");
    ...
    $count_result = $conn->prepare("SELECT COUNT(*) as count FROM `$source_db`.`$table`");
    $count_result->execute();
    $result = $count_result->get_result();
    ...
    if ($row_count > 0) {
        ...
        $source_data = $conn->prepare("SELECT $columns_sql FROM `$source_db`.`$table`");
        $source_data->execute();
        $result = $source_data->get_result();
        ...
        while ($row = $source_data->fetch_assoc()) {
            ...
            if ($primary_key && isset($row[$primary_key])) {
                $check = $conn->prepare("SELECT 1 FROM `$target_db`.`$table` WHERE `$primary_key` = '" . $conn->real_escape_string($row[$primary_key]) . "'");
                $check->execute();
