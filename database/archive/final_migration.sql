-- Final Database Migration Script for APS Dream Homes
-- This script updates the database schema to support the new user and associates structure
-- while maintaining compatibility with existing code

-- Backup existing data
-- First check which user table exists (user or users)
SET @user_table_exists = (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'user');
SET @users_table_exists = (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'users');

-- Create appropriate backup based on which table exists
SET @backup_query = IF(@user_table_exists > 0, 'CREATE TABLE IF NOT EXISTS user_backup AS SELECT * FROM user', 
                      IF(@users_table_exists > 0, 'CREATE TABLE IF NOT EXISTS user_backup AS SELECT * FROM users', 
                      'SELECT "No user table found" AS message'));
PREPARE stmt FROM @backup_query;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Backup associates table if it exists
SET @associates_exists = (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'associates');
SET @backup_associates_query = IF(@associates_exists > 0, 'CREATE TABLE IF NOT EXISTS associates_backup AS SELECT * FROM associates', 
                               'SELECT "No associates table found" AS message');
PREPARE stmt FROM @backup_associates_query;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
BEGIN
    ...
    IF user_backup_exists > 0 THEN
        ...
        IF @has_uid > 0 AND @has_uname > 0 THEN
            -- Cursor for user table with old column names
...
-- Create trigger for team hierarchy maintenance (only if it doesn't exist)
DELIMITER //

-- First check if trigger already exists
SET @trigger_exists = (SELECT COUNT(*) FROM information_schema.triggers 
                      WHERE trigger_schema = DATABASE() 
                      AND trigger_name = 'after_associate_insert');
...
BEGIN
    ...
    IF NEW.sponsor_id IS NOT NULL THEN
        ...
        SET @sponsor_exists = (SELECT COUNT(*) FROM associates WHERE uid = NEW.sponsor_id);
...
-- Create a view for easier access to associate data with user information
CREATE OR REPLACE VIEW associate_details AS
SELECT 
    a.associate_id,
    a.uid,
    u.id AS user_id,
...
-- Create a procedure to update associate levels based on business
DELIMITER //
CREATE PROCEDURE update_associate_levels()
BEGIN
    UPDATE associates a
    JOIN associate_levels al ON 
        a.total_business BETWEEN al.min_business AND al.max_business
    SET a.level = al.level_id
    WHERE a.total_business BETWEEN al.min_business AND al.max_business;
    
    -- Log the level changes
    INSERT INTO associate_level_history (associate_id, old_level_id, new_level_id, change_date)
    SELECT a.associate_id, a.level, al.level_id, NOW()
    FROM associates a
    JOIN associate_levels al ON 
        a.total_business BETWEEN al.min_business AND al.max_business
    WHERE a.level != al.level_id;
END //
DELIMITER ;

-- Create a table to track level changes
CREATE TABLE IF NOT EXISTS `associate_level_history` (
  ...
  PRIMARY KEY (`id`),
  KEY `idx_level_history_associate` (`associate_id`),
  CONSTRAINT `fk_level_history_associate` FOREIGN KEY (`associate_id`) REFERENCES `associates` (`associate_id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
DROP PROCEDURE IF EXISTS migrate_user_data;

-- Keep backup tables for safety (comment out if you want to drop them)
...
SELECT COUNT(*) AS associates_migrated FROM associates;