...
-- SKIP: `accounting_payments`.`payment_date` already indexed
-- SKIP: `accounting_payments`.`party_id` already indexed
-- SKIP: `accounting_payments`.`bank_account_id` already indexed
-- SKIP: `accounting_payments`.`transaction_id` already indexed
-- SKIP: `accounting_payments`.`invoice_id` already indexed
-- SKIP: `accounting_payments`.`status` already indexed
-- SKIP: `accounting_payments`.`created_at` already indexed
-- SKIP: `accounting_payments`.`updated_at` already indexed

-- SKIP: `accounting_settings`.`updated_at` already indexed

-- SKIP: `activities`.`activity_id` already indexed
-- SKIP: `activities`.`lead_id` already indexed
-- SKIP: `activities`.`opportunity_id` already indexed
-- SKIP: `activities`.`due_date` already indexed
-- SKIP: `activities`.`completed_date` already indexed
-- SKIP: `activities`.`created_at` already indexed
-- SKIP: `activities`.`updated_at` already indexed

-- SKIP: `activity_log`.`created_at` already indexed

-- SKIP: `activity_logs`.`user_id` already indexed
-- SKIP: `activity_logs`.`created_at` already indexed

-- SKIP: `addresses`.`user_id` already indexed
-- SKIP: `addresses`.`property_id` already indexed
-- SKIP: `addresses`.`created_at` already indexed
-- SKIP: `addresses`.`updated_at` already indexed

-- SKIP: `admin`.`status` already indexed
-- SKIP: `admin`.`email` already indexed
-- SKIP: `admin`.`phone` already indexed

-- SKIP: `admin_activity_log`.`admin_id` already indexed
-- SKIP: `admin_activity_log`.`created_at` already indexed


-- SKIP: `ai_chatbot_config`.`created_at` already indexed

-- SKIP: `ai_chatbot_interactions`.`user_id` already indexed
-- SKIP: `ai_chatbot_interactions`.`created_at` already indexed

-- SKIP: `ai_config`.`updated_at` already indexed

-- SKIP: `ai_lead_scores`.`lead_id` already indexed
-- SKIP: `ai_lead_scores`.`scored_at` already indexed

-- SKIP: `ai_logs`.`user_id` already indexed
-- SKIP: `ai_logs`.`created_at` already indexed

-- SKIP: `api_developers`.`email` already indexed
-- SKIP: `api_developers`.`status` already indexed
-- SKIP: `api_developers`.`created_at` already indexed

-- SKIP: `api_integrations`.`status` already indexed
-- SKIP: `api_integrations`.`created_at` already indexed

-- SKIP: `api_keys`.`user_id` already indexed
-- SKIP: `api_keys`.`status` already indexed
-- SKIP: `api_keys`.`created_at` already indexed
-- SKIP: `api_keys`.`updated_at` already indexed
-- SKIP: `api_keys`.`last_used_at` already indexed


-- SKIP: `api_request_logs`.`api_key_id` already indexed

-- SKIP: `api_sandbox`.`status` already indexed
-- SKIP: `api_sandbox`.`created_at` already indexed


-- SKIP: `app_store`.`created_at` already indexed

-- SKIP: `ar_vr_tours`.`property_id` already indexed
-- SKIP: `ar_vr_tours`.`uploaded_at` already indexed

-- SKIP: `associate_levels`.`status` already indexed
-- SKIP: `associate_levels`.`created_at` already indexed
-- SKIP: `associate_levels`.`updated_at` already indexed

-- SKIP: `associate_mlm`.`created_at` already indexed
-- SKIP: `associate_mlm`.`updated_at` already indexed

-- SKIP: `associates`.`user_id` already indexed
-- SKIP: `associates`.`created_at` already indexed
-- SKIP: `associates`.`updated_at` already indexed

-- SKIP: `associates_backup`.`associate_id` already indexed
-- SKIP: `associates_backup`.`user_id` already indexed
-- SKIP: `associates_backup`.`sponsor_id` already indexed
-- SKIP: `associates_backup`.`created_at` already indexed

-- SKIP: `attendance`.`employee_id` already indexed
-- SKIP: `attendance`.`status` already indexed

-- SKIP: `audit_access_log`.`accessed_at` already indexed
-- SKIP: `audit_access_log`.`user_id` already indexed

-- SKIP: `audit_log`.`user_id` already indexed
-- SKIP: `audit_log`.`created_at` already indexed

-- SKIP: `audit_trail`.`record_id` already indexed
-- SKIP: `audit_trail`.`user_id` already indexed
-- SKIP: `audit_trail`.`created_at` already indexed

-- SKIP: `bank_accounts`.`status` already indexed
-- SKIP: `bank_accounts`.`created_at` already indexed
-- SKIP: `bank_accounts`.`updated_at` already indexed

-- SKIP: `bank_reconciliation`.`bank_account_id` already indexed
-- SKIP: `bank_reconciliation`.`reconciliation_date` already indexed
-- SKIP: `bank_reconciliation`.`status` already indexed
-- SKIP: `bank_reconciliation`.`created_at` already indexed
-- SKIP: `bank_reconciliation`.`updated_at` already indexed

-- SKIP: `bank_transactions`.`bank_account_id` already indexed
-- SKIP: `bank_transactions`.`transaction_date` already indexed
-- SKIP: `bank_transactions`.`payment_id` already indexed
-- SKIP: `bank_transactions`.`reconciled_date` already indexed
-- SKIP: `bank_transactions`.`created_at` already indexed

-- SKIP: `booking_payments`.`payment_id` already indexed
-- SKIP: `booking_payments`.`booking_id` already indexed
-- SKIP: `booking_payments`.`payment_date` already indexed
-- SKIP: `booking_payments`.`transaction_id` already indexed
