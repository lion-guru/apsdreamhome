...
$conn->query("CREATE TABLE IF NOT EXISTS db_migrations (
...
$result = $conn->query("SELECT version FROM db_version ORDER BY id DESC LIMIT 1");
        ...
        if (empty($migrationsToRun)) {
            ...
            try {
                foreach ($migrationsToRun as $migration) {
                    ...
                    $migrationName = $conn->real_escape_string($migration['file']);
                    $migrationVersion = $conn->real_escape_string($migration['version']);
                    $status = $success ? 'success' : 'failed';
                    
                    $conn->query("INSERT INTO db_migrations (version, migration_name, status) VALUES ('$migrationVersion', '$migrationName', '$status')");
                ...
                if ($failCount === 0) {
                    ...
                    $conn->query("INSERT INTO db_version (version) VALUES ('$targetVersion')");
            ...
            if (!file_exists($rollbackPath)) {
                ...
                try {
                    ...
                    if ($success) {
                        ...
                        $migrationName = $conn->real_escape_string($rollbackMigration['file']);
                        ...
                        // Update database version
                        $conn->query("INSERT INTO db_version (version) VALUES ('$prevVersion')");
