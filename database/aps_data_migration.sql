...
-- Run this after importing the new schema and restoring your old data to a temp database (e.g., realestatephp_old).

-- 1. ADMIN TABLE MIGRATION
INSERT INTO admin (aid, auser, apass, role, status, email, phone, created_at)
SELECT aid, auser, apass, role, status, aemail, aphone, NOW() FROM realestatephp_old.admin;

-- 2. ASSOCIATES TABLE MIGRATION
INSERT INTO associates (id, name, email, phone, level, created_at)
SELECT associate_id, NULL, NULL, NULL, level, created_at FROM realestatephp_old.associates;

-- 3. ASSOCIATE LEVELS MIGRATION
INSERT INTO associate_levels (id, level, commission_percent, description)
SELECT level_id, level_name, commission_percentage, reward_description FROM realestatephp_old.associate_levels;
...
-- SELECT uid, uname, uemail, uphone, 'customer', 'active', NULL, NOW() FROM realestatephp_old.users;

-- 5. BOOKINGS TABLE MIGRATION
INSERT INTO bookings (id, plot_id, customer_id, booking_date, status, amount, created_at)
SELECT booking_id, property_id, NULL, booking_date, status, amount, NOW() FROM realestatephp_old.bookings;

-- 6. LEADS TABLE MIGRATION
INSERT INTO leads (id, name, email, phone, source, status, notes, created_at)
SELECT lead_id, name, email, phone, NULL, status, notes, created_at FROM realestatephp_old.leads;
...
SELECT activity_id, lead_id, opportunity_id, type, subject, description, due_date, completed, completed_date, created_by, assigned_to, created_at, updated_at FROM realestatephp_old.activities;
...
SELECT campaign_id, name, description, type, status, start_date, end_date, budget, expected_revenue, actual_cost, actual_revenue, created_at, updated_at FROM realestatephp_old.campaigns;
...
SELECT member_id, campaign_id, lead_id, status, created_at, updated_at FROM realestatephp_old.campaign_members;

-- 10. CONTACT BACKUP TABLE MIGRATION
INSERT INTO contact_backup (cid, name, email, phone, subject, message, status, created_at)
SELECT cid, name, email, phone, subject, message, status, created_at FROM realestatephp_old.contact_backup;

-- 11. AUDIT LOG MIGRATION
INSERT INTO audit_log (id, user_id, action, entity_type, entity_id, changes, ip_address, created_at)
SELECT id, user_id, action, entity_type, entity_id, changes, ip_address, created_at FROM realestatephp_old.audit_log;
...
SELECT id, name, phone, email, file_name, file_type, file_size, comments, created_at, file_data FROM realestatephp_old.career_applications;

-- 13. CITY TABLE MIGRATION
INSERT INTO city (cid, cname, sid)
SELECT cid, cname, sid FROM realestatephp_old.city;
...
SELECT transaction_id, associate_id, booking_id, business_amount, commission_amount, commission_percentage, level_difference_amount, upline_id, transaction_date, status FROM realestatephp_old.commission_transactions;
...
SELECT communication_id, lead_id, opportunity_id, type, direction, subject, content, communication_date, user_id, created_at FROM realestatephp_old.communications;

-- 16. COMPONENTS MIGRATION
INSERT INTO components (id, name, type, content, is_active, created_at, updated_at, created_by)
SELECT id, name, type, content, is_active, created_at, updated_at, created_by FROM realestatephp_old.components;
...
SELECT payment_id, booking_id, payment_amount, payment_date, payment_method, transaction_id, payment_notes FROM realestatephp_old.booking_payments;
