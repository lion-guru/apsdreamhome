...
class NotificationService 
{
    ...
    public function sendBulkEmail($recipients, $subject, $templateName = null, $data = [], $attachments = []) {
        $results = [];
        $batchSize = $this->settings["batch_size"] ?? 100;
        $delay = $this->settings["retry_delay_minutes"] ?? 5;
        
        foreach (array_chunk($recipients, $batchSize) as $batch) {
            foreach ($batch as $recipient) {
                $results[] = $this->sendEmail($recipient, $subject, $templateName, $data, $attachments);
            }
    ...
    public function sendBulkSMS($recipients, $message, $templateName = null, $data = []) {
        $results = [];
        $batchSize = $this->settings["batch_size"] ?? 100;
        $delay = $this->settings["retry_delay_minutes"] ?? 5;
        ...
        foreach (array_chunk($recipients, $batchSize) as $batch) {
            foreach ($batch as $recipient) {
                $results[] = $this->sendSMS($recipient, $message, $templateName, $data);
            }
    ...
    public function createTemplate($data, $type) {
        $tableName = $type == "email" ? "email_templates" : "sms_templates";
    ...
    public function updateTemplate($id, $data, $type) {
        $tableName = $type == "email" ? "email_templates" : "sms_templates";
    ...
    public function deleteTemplate($id, $type) {
        $tableName = $type == "email" ? "email_templates" : "sms_templates";
...
<div class="container-fluid">
                                ...
                                <table class="table table-sm">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>Property Inquiry Received</td>
    ...
    if ($viewName == 'templates') {
        $baseContent .= '
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        ...
                        <a href="/admin/notification/create-template" class="btn btn-primary">
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        <a href="/admin/notification/edit-template/1" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                                        <a href="/admin/notification/preview/1" class="btn btn-sm btn-info"><i class="fas fa-eye"></i></a>
                                        <button class="btn btn-sm btn-success"><i class="fas fa-paper-plane"></i> Test</button>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <a href="/admin/notification/edit-template/2" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                                        <a href="/admin/notification/preview/2" class="btn btn-sm btn-info"><i class="fas fa-eye"></i></a>
                                        <button class="btn btn-sm btn-success"><i class="fas fa-paper-plane"></i> Test</button>
                                    </td>
                                </tr>
                                <tr>
                                    <td>property_inquiry</td>
                                    <td><span class="badge bg-info">Inquiry</span></td>
                                    <td>Property Inquiry Received</td>
                                    <td><span class="badge bg-success">Active</span></td>
                                    <td><i class="far fa-star text-muted"></i></td>
                                    <td>
                                        <a href="/admin/notification/edit-template/3" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                                        <a href="/admin/notification/preview/3" class="btn btn-sm btn-info"><i class="fas fa-eye"></i></a>
                                        <button class="btn btn-sm btn-success"><i class="fas fa-paper-plane"></i> Test</button>
                                    </td>
                                </tr>
                            </tbody>
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    <tr>
                                        <td>welcome_sms</td>
                                        <td><span class="badge bg-primary">Registration</span></td>
                                        <td>Welcome to APS Dream Home! Your account has been created...</td>
                                        <td><span class="badge bg-success">Active</span></td>
                                        <td><i class="fas fa-star text-warning"></i></td>
                                        <td>
                                            <a href="/admin/notification/edit-template/4" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                                            <button class="btn btn-sm btn-success"><i class="fas fa-paper-plane"></i> Test</button>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>payment_success_sms</td>
                                        <td><span class="badge bg-success">Payment</span></td>
                                        <td>Payment successful! Amount: ₹59000...</td>
                                        <td><span class="badge bg-success">Active</span></td>
                                        <td><i class="far fa-star text-muted"></i></td>
                                        <td>
                                            <a href="/admin/notification/edit-template/5" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                                            <button class="btn btn-sm btn-success"><i class="fas fa-paper-plane"></i> Test</button>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>otp_verification</td>
                                        <td><span class="badge bg-warning">Verification</span></td>
                                        <td>Your APS Dream Home OTP is: {{otp}}...</td>
                                        <td><span class="badge bg-success">Active</span></td>
                                        <td><i class="far fa-star text-muted"></i></td>
                                        <td>
                                            <a href="/admin/notification/edit-template/6" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                                            <button class="btn btn-sm btn-success"><i class="fas fa-paper-plane"></i> Test</button>
                                        </td>
                                    </tr>
                                </tbody>
    ...
    if ($viewName == 'send_test') {
        $baseContent .= '
                    <h4>Send Test Notification</h4>
                    <form method="POST" action="/admin/notification/send-test">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="recipient" name="recipient" placeholder="Email or Phone Number" required>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" id="template" name="template">
                                        ...
                                        <option value="payment_success_sms">Payment Success SMS</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" id="message" name="message" rows="3" placeholder="Or enter custom message"></textarea>
                                </div>
                            </div>
                        </div>
