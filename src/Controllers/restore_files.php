...
// लॉग फाइल सेट करें
$log_file = $root_dir . '/restore_files_log.txt';
...
function move_file($source, $destination) {
    ...
    if (copy($source, $destination)) {
        log_message("फाइल कॉपी की गई: $source -> $destination");
        return true;
    } else {
...
create_directory($root_dir . '/js/tinymce/plugins');
create_directory($root_dir . '/js/tinymce/skins');
...
foreach ($template_files as $file) {
    ...
    // हेडर, फुटर और फंक्शन्स फाइल्स को include फोल्डर में मूव करें
...
function update_paths_in_file($file_path) {
    ...
    // 6. updated-config-paths.php के लिए पाथ अपडेट करें
    $content = preg_replace('/include\("includes\/templates\/updated-config-paths\.php"/i', 'include("config.php"', $content);
    $content = preg_replace('/require\("includes\/templates\/updated-config-paths\.php"/i', 'require("config.php"', $content);
    
    // 7. common-functions.php के लिए पाथ अपडेट करें
    $content = preg_replace('/include\("includes\/templates\/common-functions\.php"/i', 'include("include/functions.php"', $content);
    $content = preg_replace('/require\("includes\/templates\/common-functions\.php"/i', 'require("include/functions.php"', $content);
