...
foreach ($tables as $tableName => $sql) {
    if (!createTableIfNotExists($conn, $tableName, $sql)) {
...
if ($allTablesCreated) {
    ...
    $checkTypes = $conn->query("SELECT COUNT(*) as count FROM property_types");
    $typeCount = $checkTypes->fetch_assoc()['count'];
    ...
    if ($typeCount == 0) {
        ...
        $typeStmt = $conn->prepare("INSERT INTO property_types (type, description) VALUES (?, ?)");
        foreach ($defaultTypes as $type) {
            $typeStmt->bind_param("ss", $type[0], $type[1]);
            $typeStmt->execute();
        }
