...
class _MyTeamPageState extends ConsumerState<MyTeamPage> {
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => ref.refresh(myTeamProvider),
          ),
        ],
      ),
  ...
  List<Map<String, dynamic>> _filteredMembers(
    ...
    if (_searchQuery.isEmpty) return teamMembers;
  ...
  Widget _buildContent(
    ...
    final filtered = _filteredMembers(teamMembers);
    return RefreshIndicator(
      onRefresh: () async => ref.refresh(myTeamProvider),
  ...
  Widget _buildStatsHeader(Map<String, dynamic> stats) {
    final total = (stats['total_team_size'] ?? stats['total'] ?? 0) as int;
    final active = (stats['active_members'] ?? stats['active'] ?? 0) as int;
    final totalSales =
        (stats['total_team_business'] ?? stats['totalSales'] ?? 0.0) as double;
