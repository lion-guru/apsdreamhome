...
class _LeadListPageState extends ConsumerState<LeadListPage> {
            ...
            child: SingleChildScrollView(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  _buildFilterChip('Contacted', _selectedStatus == 'Contacted',
                      (value) {
                    setState(() {
                      _selectedStatus = value;
                    });
                  ...
                  const SizedBox(width: 8),
                  _buildFilterChip(
                      'Interested', _selectedStatus == 'Interested', (value) {
                    setState(() {
                      _selectedStatus = value;
                    });
                  }),
                  const SizedBox(width: 8),
                  _buildFilterChip('Converted', _selectedStatus == 'Converted',
                      (value) {
                    setState(() {
                      _selectedStatus = value;
                    });
            ...
            child: leadsAsync.when(
              data: (leads) {
                ...
                return RefreshIndicator(
                  onRefresh: () =>
                      ref.read(leadsProvider.notifier).refreshLeads(),
  ...
  void _updateLeadStatus(Lead lead, String newStatus) async {
    try {
      await ref.read(leadsProvider.notifier).updateLeadStatus(lead, newStatus);
      if (mounted) {
  ...
  void _processVoiceResult(String text) async {
    ...
    try {
      // final apiService = ref.read(apiServiceProvider); // Temporarily removed
